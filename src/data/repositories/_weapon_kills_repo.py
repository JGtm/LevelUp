...
Fournit les méthodes de lecture des kills par arme pour un match
ou un ensemble de matchs, et les méthodes d'écriture (upsert, backfill).
"""
...
class WeaponKillsMixin:
    ...
    def load_weapon_kills_for_match(self, match_id: str) -> pl.DataFrame:
        """Charge les kills par arme pour tous les joueurs d'un match.

        Returns:
            DataFrame (xuid, weapon_id, kills).
        """
        try:
            conn = self._get_connection()
            result = conn.execute(
                "SELECT xuid, weapon_id, kills "
            ...
            return pl.DataFrame(schema={"xuid": pl.Utf8, "weapon_id": pl.Int32, "kills": pl.Int16})
    ...
    def load_top_weapon_per_player(self, match_id: str) -> dict[str, tuple[int, int]]:
        ...
        try:
            ...
            rows = conn.execute(
                ...
                "  SELECT xuid, weapon_id, kills,"
                "    ROW_NUMBER() OVER (PARTITION BY xuid ORDER BY kills DESC) AS rn"
                "  FROM shared.weapon_kills"
                "  WHERE match_id = ?"
                ") sub WHERE rn = 1",
                (match_id,),
    ...
    def load_weapon_kills_for_player(
        ...
        Args:
            xuid: XUID du joueur.
            match_ids: Liste de match_id. Si ``None``, tous les matchs.
        ...
        try:
            ...
            if match_ids:
                ...
                result = conn.execute(
                    f"SELECT match_id, weapon_id, kills "
                ...
                result = conn.execute(
                    "SELECT match_id, weapon_id, kills "
                    "FROM shared.weapon_kills "
                    "WHERE xuid = ? "
    ...
    def load_weapon_kills_aggregated(
        ...
        try:
            ...
            if match_ids:
                placeholders = ", ".join("?" for _ in match_ids)
                result = conn.execute(
                    f"SELECT weapon_id, SUM(kills)::INTEGER AS total_kills "
                    f"FROM shared.weapon_kills "
                    f"WHERE xuid = ? AND match_id IN ({placeholders}) "
                ...
                result = conn.execute(
                    "SELECT weapon_id, SUM(kills)::INTEGER AS total_kills "
                    "FROM shared.weapon_kills "
                    "WHERE xuid = ? "
            ...
            return pl.DataFrame(schema={"weapon_id": pl.Int32, "total_kills": pl.Int32})

    # ── Write operations (accept explicit connection) ────────────────────

    @staticmethod
    def upsert_weapon_kills(
        conn: duckdb.DuckDBPyConnection,
        match_id: str,
        xuid: str,
        counts: Counter,
    ) -> int:
        """Upsert les lignes (match_id, xuid, weapon_id, kills).
        ...
        if not counts:
            return 0
        rows = 0
        for weapon_id, kills in counts.items():
            conn.execute(
                "INSERT INTO weapon_kills (match_id, xuid, weapon_id, kills) "
                "VALUES (?, ?, ?, ?) "
                "ON CONFLICT (match_id, xuid, weapon_id) "
                "DO UPDATE SET kills = EXCLUDED.kills",
                (match_id, xuid, weapon_id, kills),
            )
            rows += 1
        logger.debug("upsert_weapon_kills %s : %d lignes", match_id[:8], rows)
        return rows
    ...
    def load_player_kills_for_match(
        ...
        """Charge les kills POV du joueur + médailles proches depuis highlight_events.

        Retourne une liste de dicts avec time_ms, gamertag, xuid,
        ...
        kill_rows = conn.execute(
            ...
            "WHERE he.match_id = ? AND he.gamertag ILIKE ? "
