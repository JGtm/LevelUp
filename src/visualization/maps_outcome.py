...
def _add_timeline_traces(fig: go.Figure, sub: pl.DataFrame, color: str, label: str) -> None:
    """Ajoute les traces historique (petits) + session (grands, contour blanc) pour un outcome."""
...
def plot_map_lollipop(
    ...
    Chaque carte = tige grise + cercle coloré (vert >= 50 %, rouge sinon).
...
def plot_map_winrate_bullet(
    ...
    Barre grise = win rate historique (reference). Barre fine coloree = session.
    Vert si meilleur que l'historique, rouge si pire, ambre si equivalent (+-5 %).

    Args:
        bd_current: Breakdown session (map_name, win_rate, matches).
        bd_history: Breakdown historique (map_name, win_rate, matches).
        lang: Langue.
    ...
    joined = joined.sort("_hist_wr", descending=True)
    d = to_pandas_for_plotly(joined)
    colors = HALO_COLORS.as_dict()
    hist_lbl = "Historique" if lang == "fr" else "History"
    sess_lbl = "Session"

    def _color(curr: float, hist: float) -> str:
    ...
    curr_colors = [_color(c, h) for c, h in zip(d["win_rate"], d["_hist_wr"], strict=False)]
    fig = go.Figure()
    fig.add_trace(
        go.Bar(
            x=d["_hist_wr"],
            ...
            orientation="h",
            name=hist_lbl,
            marker={"color": "rgba(120,120,120,0.25)", "line": {"width": 0}},
            width=0.55,
            customdata=list(zip(d["_hist_wr"], d["_hist_n"], strict=False)),
            hovertemplate=f"%{{y}}<br>{hist_lbl}=%{{customdata[0]:.0%}} (N=%{{customdata[1]}})<extra></extra>",
        )
    )
    fig.add_trace(
        go.Bar(
            ...
            orientation="h",
            name=sess_lbl,
            marker={"color": curr_colors, "line": {"width": 0}},
            width=0.22,
            customdata=d["matches"],
            hovertemplate=f"%{{y}}<br>{sess_lbl}=%{{x:.0%}} (N=%{{customdata}})<extra></extra>",
        )
    )
...
def plot_map_perf_vs_history(
    ...
    """Barres horizontales groupees : performance session vs historique par carte.
