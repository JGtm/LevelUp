...
par ti. Fixtures : `.ai/re_dump/kf_slot0_live.bin` + `kf_capture_sample.txt`.
...
`.ai/re_dump/keyframe_buffer_live.bin`. **Byte-for-byte IDENTIQUE** à la capture #1 (autre lancement,
...
On a un ORACLE bit-exact : `.ai/re_dump/keyframe_buffer_live.bin` (11485 o, déterministe). Deux voies :
...
- **Oracle** : `.ai/re_dump/keyframe_buffer_live.bin` (buffer keyframe game-interne, déterministe, bit-exact).
