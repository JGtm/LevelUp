...
| M5 | Libellés L2 (accueil), L3 (modes/playlists → `assets.toml`), L4 (armes), L5 (rangs → `mappings/ranks.go`) | Libellés | Sonnet, effort moyen, un lot par famille | parité FR/EN à l'écran, ratchet qui baisse, `no_slug_comparison_test` — **L2+L5 [x] 2026-09-07** : `feat/libelles-accueil-rangs` (base `feat/v75` 6a1496e30) ; L2 : `home_locale.go` déjà conforme (0 littéral, vérifié — les `labelForLocale` restants résolvent des noms d'asset dynamiques, famille L3), seul composite en dur trouvé = `RecentMatchItem.Title` (assemblé côté Go via `RecentMatchesOptions.OutcomeText`) → SUPPRIMÉ, web compose désormais le repli depuis une clé i18n déjà existante ; L5 : `csrUnrankedLabel = "Non classé"` → clé canonique `"unranked"` (D5), web étend `lib/skillTiers.ts::TIER_NAME_BY_KEY` (canal existant, pas de nouveau channel) ; ratchet `compare_service.go` 3→2 ; découverte : `mappings/ranks.go` sert le rang de CARRIÈRE, pas le tier CSR (carte du plan inexacte, corrigée au plan libellés §11) ; gates Go (build/vet/tests ciblés/openapi-gen -check/generate-types sans diff/golangci-lint 0 issue) et web (typecheck/vitest 656 fichiers/lint 0 erreur/lint:colors/lint:fields) verts ; **L3+L4 restent `[ ]`** (seconde moitié du lot, hors périmètre de cette session). — **L3 [x] 2026-09-08** (reprise `feat/libelles-modes-playlists`, base `feat/libelles-accueil-rangs` 92ae3fe3d) : `mode_category.go` et les `labelForLocale` de l'accueil vérifiés `[~]` déjà conformes (clés/données, pas de littéral) ; `rankedplaylists.go` (`NameEN`/`NameFR`) migré de champs littéraux vers des méthodes lisant `ranked_playlists_labels.toml` (embarqué — condition de reprise consignée au plan libellés §11) ; `match_history_service.go::expTypePVPRanked/Unranked` `[!]` NON traité (CONTRAT testé GH5-2 avec la cascade de filtres web, ~80 fichiers `apps/web/src` — lot dédié requis, cf. plan libellés §11) ; `migrations/mode_playlist_fr.go` `[~]` hors périmètre confirmé (migration, CLAUDE.md §2.H). Ratchet `no_french_label_literal_test.go` : rankedplaylists.go 15→0 (retiré) ; nouveau ratchet `no_bare_resolve_mode_ui_test.go::TestNoNewModePlaylistLabelLiteral` (interdit un nouveau champ `NameEN:`/`NameFR:` littéral). Gates Go et web (mêmes commandes que L2/L5, node_modules réinstallé dans le worktree) verts. — **L4 [x] 2026-09-08** (reprise `feat/libelles-armes`, base `feat/libelles-modes-playlists` 2f190c543, worktree `LevelUp-wt-m5c-armes`) : découverte majeure (doctrine RE-VÉRIFIER) — les 2 sites cités (§2.C) ne sont PAS des libellés à migrer vers TOML : `registry.go::weaponRegistryFamilies` (22 littéraux) seedait `weapon_families` (metadata.duckdb) SANS AUCUN lecteur (Go ni web) → champ mort supprimé par une migration CTAS-swap dédiée (`purge_weapon_families_labels_columns`, calquée sur le précédent direct du même paquet `purge_weapons_name_fr_column`/V721-05.1) plutôt que migré (règle « 0 code mort », pas de TOML pour du contenu mort) ; `labels.go` (table `weapon_labels`, 12 littéraux) `[!]` NON traité — DONNÉES seedées au sens de la consigne d'exécution, vérifié LIVE (repli de nom pour sentinelles grenade/mêlée/véhicule + ids sans weapon_key dans `weapon_resolver.go`, architecture V72-06 volontaire). Ratchet `no_french_label_literal_test.go` : registry.go 22→0 (retiré) ; nouveau ratchet dédié `no_weapon_family_label_literal_test.go::TestNoNewWeaponFamilyLabelLiteral` (interdit la réintroduction d'un champ `en, fr string` dans games/weapons) — 131→130 fichiers, 522→500 littéraux. Découvertes consignées, non traitées : `WeaponLabel`/`TopWeaponLabel` (match_view.go) restent en option 2 (serveur localise) au lieu de l'option 1/D5 — préexistant V72-06, chantier fullstack séparé ; `weapons.manufacturer` également mort (même famille que weapon_families avant purge), hors périmètre libellés. Gates Go (build/vet/tests ciblés + `-tags=integration` sur migration/weapons + openapi-gen -check 0 diff + generate-types 0 diff + golangci-lint 0 issue après extraction de 4 constantes goconst) et web (aucun fichier web touché — 0 DTO API modifié — gates relancés par prudence : typecheck/lint 0 erreur 29 warnings/lint:colors/lint:fields/vitest 656 fichiers 6993 tests) verts. **Lot M5 entièrement clos (L2+L3+L4+L5)**. |
