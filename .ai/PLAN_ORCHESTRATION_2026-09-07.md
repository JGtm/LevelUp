...
| M3 | Budget de candidats du calage (adaptatif ou dédoublonnage par fin de vie) ; test de documentation RETOURNÉ ; bump 49 partagé avec le lot suivant qui bumpe | v2 R7 | Opus, effort moyen | `d9781168` / `51ebbc0f` identiques hors numéro — [x] 2026-09-07 : `feat/v2-restes-r7` (worktree `LevelUp-wt-m3-calage`, base R6, poussée) ; **dédoublonnage par FIN DE VIE** retenu (le vote comptait les morts appariables quand l'affinage apparie 1:1 : un amas de 20 morts déposait 20 voix sur chaque fin isolée pour 1 paire réalisable) — la voix d'un panier devient `min(morts, fins)`, borne de Hall/König, **aucun seuil nouveau** ; budget adaptatif écarté (classement resté faux + 15 affinages inutiles). Fixture adversariale `off=216350 n=2` → `off=200000 n=15 second=2`, alarme éteinte ; test retourné en mutation `TestUnAmasPlusGrosQueLeVraiCalageNEmportePasLeBudget` (teste `voteDeathOffsets` seul), baseline inchangée (le test date du 07/09, la baseline du 26/06). **AUCUN golden ne bouge → PAS de bump, `SchemaVersion` reste 48**, le 49 reste pour P2. `make replay-corpus-gate` et les témoins `d9781168`/`51ebbc0f` restent À JOUER PAR LE SUPERVISEUR (worktree sans film) : commandes au §6 de `.ai/V7.5/v2/RESTES_R7_2026-09-07.md` |
| M1 | Lien « voir dans le rejeu » depuis une cellule : contrat `POST …/tactical/{map_id}/cellule` (contributions `{match_id, instant_ms, xuid}` filtrées par ownership ADR 0029 + `matchs_non_ouvrables`), service, handler, `TacticalCellCard` avec lien `?frame=` (`playbackStore`), instant → frame en logique pure | Tactique S.1 (5.5 + 5.6 complet) | Opus effort bas pour le contrat + Sonnet pour le web | test « un match d'un autre joueur n'apparaît pas mais compte », `?frame=` positionne le rejeu, contrat régénéré, typecheck, vitest — [x] 2026-09-08 : reprise (exécutant précédent coupé par quota) sur `feat/tactique-lien-rejeu` (base `feat/v75` @ `6a1496e30`), worktree `LevelUp-wt-m1-cellule`. Contrat, service, handler, web (`TacticalCellCard`, `useTacticalCellule`, clé de query) déjà posés à la reprise ; complété dans cette passe : tests web manquants (`instantToFrame` dans `tacticalView.logic.test.ts`, rendu `TacticalCellCard.test.tsx` — lien `?frame=`, footer conditionnel, placeholder/chargement/vide) et l'entrée découvertes du décalage d'horloge match/film (non traitée, réserve documentée dans le code depuis la première passe). Gates verts : `gofmt`/`go build`/`go vet` propres, `go test` service+api+duckdb+domain vert, `NoRaw`/`Mesuree` vert, `archlint`+`sync` (no_slug_comparison, no_art_patterns) vert, `openapi-gen -check` à jour, `generate-types` sans dérive (92 lignes stables), `golangci-lint --new-from-merge-base=origin/main` 0 issue, web `tsc -b` propre, `eslint` 0 erreur (29 warnings préexistants hors périmètre), `vitest run --pool=forks` 657/658 fichiers verts (7006 tests, 17 skip préexistants), `lint:colors`/`lint:fields` propres. |
...
| M1b | Corriger le décalage d'horloge match/film sur le lien « voir dans le rejeu » (découverte du lot M1, décision utilisateur ferme du 2026-09-08) : `coverage.bridge.deathOffsetMs` publié (additif, pointeur, jamais un zéro pour « inconnu »), bump SchemaVersion 48→49 (assumé hors-règle : clé de reprise du backfill, pas un changement de forme cassant), `TacticalContribution.clock` (`match`/`film`) publié par question, lien `?t=&clock=` (remplace `?frame=`), la route du rejeu convertit une fois le document chargé (`resolveTacticalReplayInstant`/`resolveTacticalOpenAtFrame`, `lib/replay/replayLogic.ts`) et affiche un avis si le calage est inconnu — `instantToFrame` retiré (mort) | Découverte M1 (`.ai/DECOUVERTES_TACTIQUE_2026-09-07.md`) | Sonnet, effort moyen | golden (version + champ seulement), cliquet `TestStructureIsOptionalInDocument` mis à jour + sa chronique, cliquet de parité `service/replayview`, test service par question (clock), route (offset connu/inconnu/film), `msToFrames` aller-retour — **[x] 2026-09-08** : worktree `LevelUp-wt-m1b-offset`, branche `feat/tactique-lien-rejeu-exact` (base `feat/v2-restes-r7`, `feat/tactique-lien-rejeu` fusionnée sans conflit de code). Gates verts : `gofmt`/`go build`/`go vet`/`go test` (paquets du lot) propres, `golangci-lint --new-from-merge-base=origin/main` 0 issue, `openapi-gen -check` à jour, `generate-types` sans dérive (6 lignes), web `tsc -b` propre, `eslint` 0 erreur (29 warnings préexistants), `vitest run --pool=forks` 658/659 fichiers verts (7023 tests, 17 skips préexistants), `lint:colors`/`lint:fields` propres. Découverte au passage : `?frame=` de M1 n'était PAS câblé côté route (`replay.tsx` n'avait aucun `validateSearch`) — corrigé dans ce lot (`openAtFrame` sur `ReplayCanvas`/`useReplayPlayback`), sinon le lien tactique n'aurait jamais rien ouvert. Parc d'artefacts < 49 : recuisson nécessaire (registre). |
