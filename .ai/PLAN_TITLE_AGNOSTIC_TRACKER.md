...
| 1.5 | DDL par titre (sortir `migration/steps_*`) | 🟡 | 55 | **oui** (2e titre) — mécanisme B complet + **5 steps migrés** (1 PvE + 4 Shared additifs) ; reste 2 tiers (voir b3) |
...
| **1.5.1 — voie B (MigrationSteps explicite, ADR 0025)** | 🟡 | **Mécanisme COMPLET (no-op prouvé)** : helpers exportés (`519f2e7c4`) + `RunSteps` primitive (`a40f28c53`) + provider `SetTitleStepsProvider`/`combineSteps`, `RunForDB` combine global+title dédup par Name, package `halo_infinite/migrations` (vide), wiring boot serveur (`ad44a5ede`). 14 appelants inchangés. **b3 en cours** : 5 steps migrés (pilote `add_pve_schema` `cd80688b9` + batch Shared additif `d1b76b7d1`). CLIs Shared câblés (diag_bot, apply_shared_migrations). **Pattern** : copier dans `Steps()` (`execScript`→`migration.ExecScript`), `git rm`, câbler les CLIs du target, `order_audit` valide. **Reste = 2 TIERS** : **(A) additif (facile)** — `ADD COLUMN`/index/table secondaire : aucun test ne les asserte (tests cœur assertent le schéma de base / `EnsureSharedSchema`) → déplacement direct ; les CLIs Shared sont déjà câblés. **(B) cœur + entanglement** — migrations créant `match_registry`/vues etc. : `migration_test.go` (package migration) les asserte via `RunForDB` MAIS ne peut PAS poser le provider Halo (**cycle d'import**) → il faut **relocaliser ces tests cœur vers `halo_infinite/migrations`** (qui peut poser le provider) avant de bouger les steps cœur. Câblage restant par target : Metadata (seed-assists, populate-playlists, cmd/levelup), Player (repair_data_consistency + mains ouvrant le pool — le provider global suffit), SharedSocial (pool via mains) |
