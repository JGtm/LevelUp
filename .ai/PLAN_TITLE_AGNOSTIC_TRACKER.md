...
| 1.5 | DDL par titre (sortir `migration/steps_*`) | 🟡 | 45 | **oui** (2e titre) — mécanisme B complet (ordre ✅, helpers ✅, `RunSteps` ✅, provider ✅) ; reste = déplacer les fichiers (b3) |
...
| **1.5.1 — voie B (MigrationSteps explicite, ADR 0025)** | 🟡 | **Mécanisme COMPLET (no-op prouvé)** : helpers exportés (`519f2e7c4`) + `RunSteps` primitive (`a40f28c53`) + provider `SetTitleStepsProvider`/`combineSteps`, `RunForDB` combine global+title dédup par Name, package `halo_infinite/migrations` (vide), wiring boot serveur (`ad44a5ede`). 14 appelants inchangés. **Reste b3** : déplacer les steps `package migration` → `halo_infinite/migrations.Steps()` un par un (+ adapter les order tests pour compter les steps title-owned, + câbler `SetTitleStepsProvider` dans les CLIs qui migrent le target déplacé) ; golden ordre valide chaque étape |
