...
| 1.5 | DDL par titre (sortir `migration/steps_*`) | 🟡 | 60 | **oui** (2e titre) — mécanisme B complet + **8 steps migrés** (PvE + 5 Shared + 2 metadata leaf b4) + garde-fou direction-de-dépendance ; reste tier B (~158 steps, familles ordering-sensibles atomiques + reorder inversion escaladé) |
