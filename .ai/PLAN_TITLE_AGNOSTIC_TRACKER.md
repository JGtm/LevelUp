...
| 1.5 | DDL par titre (sortir `migration/steps_*`) | 🟡 | 50 | **oui** (2e titre) — mécanisme B complet + **pilote b3 prouvé bout-en-bout** (1 step migré, `pve_match_stats` créée via provider) ; reste = répéter pour ~50 fichiers (mécanique) |
...
| **1.5.1 — voie B (MigrationSteps explicite, ADR 0025)** | 🟡 | **Mécanisme COMPLET (no-op prouvé)** : helpers exportés (`519f2e7c4`) + `RunSteps` primitive (`a40f28c53`) + provider `SetTitleStepsProvider`/`combineSteps`, `RunForDB` combine global+title dédup par Name, package `halo_infinite/migrations` (vide), wiring boot serveur (`ad44a5ede`). 14 appelants inchangés. **Pilote b3 ✅** (`cd80688b9`) : `add_pve_schema` migré → `halo_infinite/migrations.Steps()`, prouvé bout-en-bout (table créée via provider). **Pattern de déplacement établi** : (1) copier la migration dans `Steps()` en remplaçant `execScript`→`migration.ExecScript` etc. ; (2) `git rm` le `steps_*.go` ; (3) si un test/CLI lance `RunForDB(target)` pour ce target sans garde `len==0`, y poser `SetTitleStepsProvider(halomigrations.StepsFor)` ; (4) l'audit `order_audit_test.go` valide complétude global+title. **Reste** : répéter pour ~50 fichiers (mécanique). Note câblage : seul `SharedPvE`=serveur-only ; les autres targets (Player/Shared/Metadata/Social) ont des CLIs à câbler quand on déplace leurs steps |
