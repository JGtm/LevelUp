...
| **Killfeed VISIBLE — 2/2 : la bascule des 20 lecteurs** + `seed_demo` (S1) + `rebuild_mp` (S2) | **BLOQUÉ sur 3 arbitrages superviseur, mesures faites 2026-08-03** — `PLAN_BRANCHEMENT_KILLSOURCE.md` §2.4. La vue de compat est INFAISABLE (la table est devenue la base crédit) ; `publishable` ne doit pas filtrer les lignes (coût mesuré : 47 037 morts sur 366 matchs) ; la bascule retire un opposant fantôme des agrégats carrière **Halo 5** (161 frags / 127 morts sur le joueur le plus actif) | **LE vrai visible** : armes de kill, assists nommés dans les pages existantes, agrégats carrière dégonflés (« 101 → 29 », re-vérifié au chiffre près) | 2-3 |
