...
**Statut couverture actuelle** : gap — `app_settings.json` est un fichier global unique ; `AppSettings`/`NotifyConfig`/`CurrentCSRSeasonID` sont des singletons process sans dimension titre, donc un 2e titre hérite forcément de la saison CSR, des webhooks et des toggles d'Halo (mauvaise saison CSR = bloquant runtime : sync CSR pointe une saison inexistante).
