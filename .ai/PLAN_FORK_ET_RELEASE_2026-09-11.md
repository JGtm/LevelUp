...
- [x] A.1 API : la réponse du document de rejeu porte la version COURANTE du producteur à côté
      ...
      `analysis/replay` ↔ `domain/replaydoc`. FAIT en en-tête HTTP
      `X-Replay-Latest-Schema-Version` (pas dans le corps) : `domain/replaydoc` interdit
      explicitement tout numéro de version et tout import d'`analysis/replay` (cf. son
      `doc.go`), et `replayview/parity_test.go` exige une source stockée pour chaque champ
      servi — la version courante n'en a pas, elle vient du producteur, pas de l'artefact.
      Assemblée à la frontière HTTP (`api/handlers/replay.go`, champ `LatestSchemaVersion`
      taggé `header:"..."`). Commit 40bf29a5c.
- [x] A.2 Web : badge visible SEULEMENT si `isAdmin` : « Schéma N · à jour » ou « Schéma N ·
      ...
      `ReplaySchemaBadge.tsx` (features/match-replay/ui/), logique pure
      `replaySchemaStatusLogic.ts`, `api.getWithHeader()` ajouté au client pour lire l'en-tête
      sans toucher au corps JSON. Posé dans le `h1` de `replay.tsx`, à côté du titre. Commit
      097a764ca.
- [x] A.3 Tests : Go (handler/service), vitest (admin/non-admin, à jour/périmé), contrat
      `make generate-types` sans dérive, tsc, lint. Tout vert (détail dans le thought_log et
      le rapport de session) : `go test ./...` + `go vet ./...` complets sur apps/go-api,
      `TestOpenAPIYAMLIsUpToDate` vert après régénération, `npm run typecheck` (cache purgé)
      + `npm run lint` (0 erreur) + `vitest run src/features/match-replay` (2720 tests) +
      `vitest run src/lib/replay src/lib/api` (295 tests) sur apps/web.
- [ ] A.4 Journal + revue sur pièces par le pilote.
...
- [ ] B.1 Mesure grenades sur 3 films (médiane distance lancer→lanceur, part > 4 m).
- [ ] B.2 Correctif `locateThrow` (auteur d'abord, toutes les naissances, rayon 4 m, `Slot` publié).
- [ ] B.3 Garde-fou pas projectile > 10 m (`Rest=false`, compteur tronqués) ; mesure |Δy| − étendue Y.
- [ ] B.4 Props Forge par carte (`MapGeometryDir(slug, module)`, `UNATTRIBUTED/` + README, test PathResolver).
...
- 2026-09-11 : Lot A (A.1-A.3) exécuté sur `wt/badge-schema` — commits 40bf29a5c (Go, en-tête
  `X-Replay-Latest-Schema-Version`) et 097a764ca (Web, `ReplaySchemaBadge`). A.4 laissé à la
  revue du pilote.
