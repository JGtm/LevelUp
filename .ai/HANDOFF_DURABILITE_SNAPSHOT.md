...
- **Fait + testé** : Phase 0 (B-swap instrumentation), Phase 1.b (`CommitThenAdvance`), **Phase 2 producteur+monitoring (`3600007f4`)**, **Phase 3 lecture snapshot + cutover MatchView (`083f22eea`)**, **Phase 4 cutover GLOBAL des lectures shared — COMPLÈTE** (cf. thought_log [2026-06-25] « Phase 4 »).
- **Câblage Phase 4 = GLOBAL** (subsume le scoped MatchView, reverté) : `OpenSnapshotShared` reconstruit le schéma shared COMPLET (8 tables matérialisées + toutes les vues via `migration.ApplyResolutionViews`/`ApplyMvPlayerMatchesView` + inline v_weapon_kills/match_csrs_latest) → sûr par construction (schéma complet OU `ErrSnapshotIncomplete`→fallback live). Câblé via `config.sharedReaderForTitle` + hook `AppConfig.SnapshotReaderWrapper` (impl cmd/server, singleton par titre). Producteur exporte `weapon_kills`/`match_csrs` en RAW. Médias (SharedSocial) + lectures player (ReadDB) restent live.
- **Reste** : **déploiement prod = décision utilisateur** (downtime ; merge main = auto-deploy). Au deploy : producteur peuple les snapshots → TOUTES les lectures shared basculent (fallback live avant), mesurable via `snapshot_read_served/live_fallback_total` + `shared_provider_reader_stall_ns_total`. Revert = redéployer le binaire précédent (additif, zéro migration destructive). Suivi non bloquant : régénérer `apps/web` generated-types (front ne consomme pas encore la section snapshot du monitoring).
- **Commit Phase 4** : implémenté + vert, **PAS encore committé** (en attente d'autorisation du tour courant).
