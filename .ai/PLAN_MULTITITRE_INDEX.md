...
## ✅ STATUT CONSOLIDÉ — 2026-06-18 (LIRE CECI D'ABORD — source unique)

> Les tableaux registre A/B ci-dessous restent le détail ; ce bloc est le résumé à jour. En cas de divergence, ce bloc fait foi (les statuts datés des tables ont pu prendre du retard).

**Le multi-titre est branchable day-one côté DONNÉES.** Toute l'infra title-agnostic (boot, DB, capabilities, gating data) est livrée et prouvée. Ajouter un 2ᵉ titre = déposer un dossier `config/titles/<slug>/` (title.toml + mappings + constants + auth) → découvert au boot, exposé dans `bootstrap.available_titles`, DB provisionnées. Reste, pour un VRAI 2ᵉ titre : (a) écrire son **adapter data + ses vraies sources** (intrinsèquement title-spécifique, hors infra) ; (b) **câbler le sélecteur de titre UI** — la plomberie front est prête mais aucun composant ne la rend (cf. axe UI ci-dessous). Donc multi-titre côté données = oui, côté **navigation** = pas encore.
...
**🟡 RESTE — 2 items architecturaux / externes (raison dure, pas du soft-deferral)** :
1. **Huma — génération auto de l'openapi.yaml** : migration des routes = 100% ; la **génération** est bloquée par l'archi (133 instances `NewAPI` à chemins relatifs). → on garde le **YAML manuel** (contract-testé). Même nœud que [PLAN_WEB_API_TYPES_MIGRATION](PLAN_WEB_API_TYPES_MIGRATION.md) (basculer `types.ts`→`generated.ts` est gaté sur **compléter** l'openapi.yaml, aujourd'hui sous-spécifié vs le backend réel).
2. **Phase 1.9 — pool d'auth par titre** : le watcher **détecte + route** un 2ᵉ titre, mais le pool partagé tient des tokens Spartan d'**audience Halo** ; le keying par titre est une jambe **PMT-2 inexerçable sans un vrai 2ᵉ titre**.

**🖥️ RESTE — UI multi-titre (audit 2026-06-18 vérifié + adversarial)** :
- **Sélecteur de titre (switcher) — NON câblé** 🔴 *bloquant-2e-titre*, = PMT-8 / MT-22. La plomberie complète existe (store `switchTitle` + `buildTitleSwitcherEntries` gérant `coming_soon`/`archived`, `setApiTitleSlug` → header `X-LevelUp-Title`, backend `BuildAvailableTitles`) MAIS **aucun composant ne la rend** → avec >1 titre l'utilisateur reste verrouillé sur halo_infinite. C'est du code mort côté rendu. Petit (1 dropdown dans NavL1 branché sur `buildTitleSwitcherEntries` + `switchTitle`). **C'est LE point « minimum de travail le jour d'un nouveau Halo » à poser.**
- **Gating capability — ~95%** 🟢. Explorer (bloc CSR saison + filtre skill-tier) et Timeseries (charts rang `ranked||lusr`) **gatés le 2026-06-18** (étaient oubliés par la passe Phase 5). Reste **cosmétique, non bloquant** : vocabulaire Halo en dur non gaté (admin Sync « API Halo »/« Watcher présence Xbox », Lab libellés « Waypoint », HomeHeroBanner images, `csrRankImageURL` motif `HINF-CSR`) → fallback Halo pour un 2e titre, à sortir vers adapters par-titre quand un vrai 2e titre arrive.
