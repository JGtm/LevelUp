...
| Migration Huma (~79 fichiers / ~139 routes, pas 113) | Phase 3b | master | 🟡 **en cours 2026-06-17** : fondations + **3/139 routes migrées**. (a) coexistence chi.Walk PROUVÉ (`TestHumaCoexistsWithChiWalk`) ; (b) **format JSON byte-identique à writeJSON** (`humaJSONFormat` : NaN-safe + HTML-escape + `\n`) → couvre les ~100 routes à floats sans recâblage par-route ; (c) **3 des 4 shapes prouvées** avec golden chacune : no-param GET (`/changelog`), path-param GET (`/jobs/{job_id}`), **query-param GET (`/directory/gamertags/search`)**. Reste shape **cached/ETag** (niche : `writeJSONCached` = `home.go` seul) + ~136 routes (multi-session mécanique) + bascule openapi.yaml généré |
