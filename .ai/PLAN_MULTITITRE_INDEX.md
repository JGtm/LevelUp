...
| Migration Huma (~79 fichiers / ~139 routes, pas 113) | Phase 3b | master | 🟡 **en cours 2026-06-18** : socle + **5 routes migrées**, 2 patterns structurels prouvés. Inventaire ultracode (13 agents) : 169 routes → trivial=90, needs_care=67, exclude=12 (redirects/binary/CSV/multipart/SPA restent chi). (a) **socle `humacore`** (sans cycle, partagé api+handlers : factory + erreur writeError + format byte-identique writeJSON NaN-safe) ; (b) **sous-routeur imbriqué PROUVÉ** (`TestHumaNestedSubrouterProbe` : middleware ownership/title hérité + path param parent `{player_slug}` lu, pas de bypass) → débloque ~70 routes player-scoped ; (c) **TOUTES les shapes prouvées+committées** : GET no-param/path/query, POST-body→200, body optionnel, **204** (Output `Status int`), PATCH/DELETE, **erreur+header** (`ErrorWithHeaders` 503 Retry-After), path param custom-parsé (string→400). **24 routes migrées** : changelog, jobs, gamertag, progression×3, notifications×9, + **fan-out codegen workflow** (campaign×7, coach×3, patterns×1, profile×1, agents parallèles + vérif adversariale vs HEAD). TOUS les handlers self-register player-scoped faits. Écart délibéré unique : body malformé → 422 validation_error (vs 400 invalid_body). Reste : routes INLINE server.go (player-group/admin/watcher/top-level, câblage humaAPI par sous-groupe) + needs_care + bascule openapi.yaml généré |
