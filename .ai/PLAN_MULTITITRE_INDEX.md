...
| MT-03 | World-stats / leaderboard (repos, enricher, CLIs, `DEFAULT 'halo_infinite'`) | major | partiel | [PMT-7](PLAN_MULTITITRE_PERIPHERY.md) — **read-path livré** : migration colonne `title_slug` + `CapWorldLeaderboard` + port/repo/service threadés + gating + oracle b (isolation). Reste write-path (writers/cron/CLIs `-title`) différé byte-identique | `GetCSRWorldLeaderboard(titleSlug,…)` + `leaderboard_title_isolation_test` |
