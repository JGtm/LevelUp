...
| MT-26 | Discord : config globale (→PMT-4) + contenu 100% Halo (→PMT-11) | major | partiel | [PMT-11](PLAN_MULTITITRE_PERIPHERY.md) ✅ contenu/outcomes b571f1df5 (seam `NotifyLabels`) + footer dérivé du descripteur (`discordFooterText()`, source unique, byte-identique) ; [PMT-4](PLAN_MULTITITRE_PERIPHERY.md) config = gap | `notify.NotifyLabels`+`OutcomeSource`+`discordFooterText` ; threading par-titre des embeds + libellés backfill restent différés |
...
| MT-19 | Progression handlers `defaultProgressionTitleSlug` + PrestigeBundle 1 DB | major | partiel (de-magic) | [EXT-2](PLAN_MULTITITRE_PERIPHERY.md) — `defaultProgressionTitleSlug()` corps `"halo_infinite"` → `titlePkg.DefaultSlug` (byte-identique, seam déjà centralisé 8 callers) ; threading `ctxkeys.TitleSlug` + PrestigeBundle multi-DB restent Phase 2 | `internal/api/post_sync_deltas.go:112` |
| MT-10 | CLI/ops hors backup-restore-diagnose (healthcheck, gate, cmd/* sans `--title`) | major | partiel (de-magic) | [EXT-1.5](PLAN_MULTITITRE_PERIPHERY.md) — de-magic slug `"halo_infinite"` → `titlePkg.DefaultSlug` livré (healthcheck ×7, gate ×5, byte-identique) ; itérer `registry.All()` (diagnostic multi-titre) reste différé (CHANGE le format pour 0 valeur single-titre) ; `cmd/diag_*` throwaway hors scope | `internal/ops/healthcheck.go` + `validation/gate.go` |
