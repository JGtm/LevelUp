...
| Migration Huma (~79 fichiers / ~139 routes, pas 113) | Phase 3b | master | 🟡 **en cours 2026-06-17** : fondations + **2/139 routes migrées**. (a) coexistence chi.Walk PROUVÉ (`TestHumaCoexistsWithChiWalk`) ; (b) **format JSON byte-identique à writeJSON** (`humaJSONFormat` : NaN-safe + HTML-escape + `\n`) → couvre les ~100 routes à floats sans recâblage par-route ; (c) shapes prouvées : no-param GET (`/changelog`) + **path-param GET (`/jobs/{job_id}`)** avec golden contrat chacun. Reste ~137 routes (multi-session) + shapes query-param/cached(ETag) + bascule openapi.yaml généré |
