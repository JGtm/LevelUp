...
> **Statut clôture (2026-06-15, branche `feat/multititre-peripherie`)** : **PMT-8** ✅ (gate `RequireActiveTitle` 503 + seam restauré + oracle synthetic_b). **PMT-12** 🟡 — MT-21 ✅ (validateur boot capability-driven + golden HI + oracle synthetic_b), **lint MT-12** ✅ (règle eslint `no-title-slug-literal`, warn) ; reste **MT-09** (cutoffs → factory player-scoped par titre — re-vérifié faisable maintenant, le resolver existe ; PAS bloqué par PMT-3). **PMT-14** ✅ — **volet A** ✅ (liste/détail/diagnostic+drift, toml-draft D10, CLI `levelup-titles diagnose`, admin-gating 401/403, page aide 2e titre) ; **volet C** ✅ (Lab monté + fail-closed + test anti-régression, livré 2026-06-14) ; **volet B** ✅ par construction (0 duplication ; atoms feature-local distincts — unifier violerait la modularité).
