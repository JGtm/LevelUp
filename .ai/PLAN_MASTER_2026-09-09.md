...
| 3.4 | **Nommer les rangs de palette relevés au Theater** (artefact 2.6) : rang 10 = écran occultant (`shroud_screen`), rang 19 = mur de protection à une utilisation (`wall`, variante), rang 22 = à recevoir (4 relevés restants + le rang 19 sur `4f77afc1`) → entrées `[ability_palettes.ranks]` de `config/titles/halo_infinite/mappings/replay_labels.toml` (icônes, FR/EN), garde-rail si le manifeste en a un, puis **recuisson du parc** (les libellés sont figés à la cuisson, `abilityLabelsUsed`) et `backfill-usage-summary --force` (le résumé joint sur la racine du libellé). Attend les 5 relevés de l'utilisateur pour ne recuire qu'une fois | Sonnet, effort bas ; recuisson superviseur | tests `analysis/replay` ; après recuisson : `prisesSansFamille` du backfill usage 269 → proche de 0 ; vue match : plus d'objets sans famille comptés | [ ] en attente des 5 relevés restants (utilisateur, « plus tard ») |
