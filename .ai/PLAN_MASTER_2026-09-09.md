...
| 4.2 | Console de la page de rejeu : neuf clés React dupliquées (identifiants de carte) + un 404 à identifier après redémarrage du serveur | Sonnet, effort bas | vitest + tsc + eslint sur le périmètre, cause corrigée à la source | [x] 10-09 : `0e8d3c725`, fusion `4423a6b93` — cause : le tiroir d'assets (`AssetGrid`, monté en permanence par `AppShell`) reçoit du catalogue de cartes deux entrées de même `map_asset_id` (`ListMapsByTitle` dédoublonne par `name_canonical`, `metadata_repo_assets_list.go:27`) ; dédoublonnage pur et testé côté web (`assetDrawerLogic.ts`, premier gagnant). Gates : vitest 6 + 171, tsc 0, eslint 0. **Reste** : le 404 (à identifier au redémarrage du serveur) ; la requête Go garde son `DISTINCT ON (name_canonical)` — deux libellés pour un même asset est un choix de données à trancher, consigné §6 |
