...
| 5.R | Revue unique (5.1 touche sync/persist : un relecteur L1 + L6), `make gate-push`, push = CI de vague | — | P0 = 0, P1 = 0 | [~] 10-09 : relecteur unique (L1 + L4 + L6, Opus) sur 5.1/5.5/5.6/5.7 : 0 P0, **2 P1, 1 P2**, 14 conditions tenues — l'agrégat de session lisait encore les poses (deux pages, deux vérités), le filtre des colonnes de la vue match ignorait `spent`, une fonction de famille sans appelant. **Corrigés `e9489efcd`** (accesseur `replay.UsageFamilySpawnsPiece` + garde-rail, fixtures distinguant spent/deployed, filtre en surensemble strict, un seul chemin de lecture de la famille ; 5 tests rouges → verts, mutations), fusion faite. Registre `.ai/V7.5/REVUE_VAGUE5_2026-09-10.md`. Ronde 2 (corrections seules) et `make gate-push` en cours ; push = CI de vague à suivre |
