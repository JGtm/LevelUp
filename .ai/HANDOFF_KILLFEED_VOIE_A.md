...
> `.ai/WALK_PORT_NOTES.md` (avancée walk + **PISTE POSITION-LINKING** en bas) → `.ai/DBG_CAPTURE_dmg_record.md`
...
- **CAPTURE LIVE DEBUGGER réussie** (Ghidra dbgeng via ghidra-mcp, fix `go_blocking`) : a donné le framing. Détail +
  runbook reproductible dans `.ai/DBG_CAPTURE_dmg_record.md`.
...
1. **Finir le walk biped** (le « long pole ») : porter les branches amont non portées qui désyncent le walk —
   **i54** (ctx+0x9d mid-mobility), **i59** (tag==3 = `FUN_142f25e90`), **i63** (count1>0). Réf décodeur :
   ...
   + `HANDOFF_FRAME_DECODER_L3.md` (caveats des branches non portées). **Besoin de Ghidra statique** pour décompiler
...
**AVANCÉE PORT WALK : voir `.ai/WALK_PORT_NOTES.md`** — i59 (`FUN_142f25e90`) décompilé + structuré (tags 0-6),
...
Serveur sur 8099 (peut être relancé). Runbook reproductible complet dans `.ai/DBG_CAPTURE_dmg_record.md` (attach par
...
- `.ai/DBG_CAPTURE_dmg_record.md` = capture live brute + runbook debugger.
