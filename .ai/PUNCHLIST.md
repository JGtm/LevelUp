...
| `[R]` | OP | **Squad/Sessions §3** — Q29 friends-only filter + AddFriendFlow modal/hook + tests (5 Go + 7 vitest). Reste : intégration UI dans SquadLayout (CTA "Ajouter X comme ami" sur saisie hors top). | lourd | [.ai/PLAN_SQUAD_STATS_SESSIONS_OVERHAUL.md §3+§3bis](PLAN_SQUAD_STATS_SESSIONS_OVERHAUL.md) — branche `feat/op-squad-friends-flow` |
| `[R]` | OP | **Squad/Sessions §4** — recompute `is_with_friends` (sync/friends_recompute.go + FriendsOrchestratorService + port + handler async PATCH /settings + RefreshAggregates exporté + tests purs diff). Reste : CLI bootstrap. | très lourd | [.ai/PLAN_SQUAD_STATS_SESSIONS_OVERHAUL.md §4](PLAN_SQUAD_STATS_SESSIONS_OVERHAUL.md) — branche `feat/op-squad-friends-flow` |
