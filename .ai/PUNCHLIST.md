...
| `[R]` | GS | **Squad/Sessions §6** — Notifications `friend_added` + `friend_sync_completed`. §6.A backend Go ✅ + §6.B Discord ✅ (NotifyConfig + NotifyFriendAdded + NotifyFriendSyncCompleted + 7 tests purs + i18n FR/EN). §6.C Frontend ✅ (NotificationCategory étendu, i18n labels/descriptions/templates FR+EN, IconUser, ToggleRow Settings). **Self-review impossible (livré par GS) — laissé en `[R]` pour audit ultérieur.** | moyen | [.ai/PLAN_SQUAD_STATS_SESSIONS_OVERHAUL.md §6](PLAN_SQUAD_STATS_SESSIONS_OVERHAUL.md) |
