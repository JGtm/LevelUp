...
| `[R]` | OP | **Squad/Sessions §3** — Q29 friends-only filter + AddFriendFlow modal/hook + UI CTA "Ajouter comme ami" dans SquadLayout combobox + tests (5 Go + 7 vitest). | lourd | [.ai/PLAN_SQUAD_STATS_SESSIONS_OVERHAUL.md §3+§3bis](PLAN_SQUAD_STATS_SESSIONS_OVERHAUL.md) — branche `feat/op-squad-friends-flow` |
| `[R]` | OP | **Squad/Sessions §4** — recompute `is_with_friends` (sync/friends_recompute.go + FriendsOrchestratorService + port + handler async PATCH /settings + RefreshAggregates exporté + CLI `levelup recompute-friends` + tests purs diff). | très lourd | [.ai/PLAN_SQUAD_STATS_SESSIONS_OVERHAUL.md §4](PLAN_SQUAD_STATS_SESSIONS_OVERHAUL.md) — branche `feat/op-squad-friends-flow` |
| `[R]` | OP | **Squad/Sessions §5** — SessionMultiSelect solo sur MatchHistoryPage. Backend: `BuildSessionLabelsList` helper partagé + `PickedSoloSessionLabels` filtrage. Front: localStorage `stats-sessions-${slug}` + reset pagination. | moyen | [.ai/PLAN_SQUAD_STATS_SESSIONS_OVERHAUL.md §5](PLAN_SQUAD_STATS_SESSIONS_OVERHAUL.md) — branche `feat/op-squad-friends-flow` |
| `[~]` | GS | **Squad/Sessions §6** — Notifications `friend_added` + `friend_sync_completed`. §6.A backend Go ✅ (catégories ajoutées + émetteurs SettingsHandler/FriendsOrchestrator + 11 tests purs + seed migration). §6.B Discord ❌. §6.C Frontend i18n + UI settings ❌. | moyen | [.ai/PLAN_SQUAD_STATS_SESSIONS_OVERHAUL.md §6](PLAN_SQUAD_STATS_SESSIONS_OVERHAUL.md) |
