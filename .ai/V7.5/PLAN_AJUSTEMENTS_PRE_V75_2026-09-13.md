...
- (MÉDAILLES 2) Sur une base `metadata.duckdb` héritée de l'ère Python, `medal_definitions` n'a PAS de clé primaire : tout `ON CONFLICT (medal_name_id)` lève une Binder Error, et le runner de migrations s'arrête à `seed_custom_vengeur_medal` (préexistante) → TOUTES les migrations metadata suivantes n'étaient jamais appliquées sur ce poste (et vraisemblablement en prod, même origine). Corrigé `3d83ca75e` (seeds en `INSERT … WHERE NOT EXISTS`, étape de réparation séparée, ratchet anti-`ON CONFLICT` dans `steps.go`). À AUDITER : le même piège ailleurs dans les migrations ; vérifier en prod `schema_migrations` metadata à la release.
...
- 2026-09-14 — Passe 5 : Firefight hors Tactique (103 cartes vs 105), donut, badges, pied CONFORMES ; médaille VIP : image OK, nom encore « Inconnue » → cause racine = chaîne de migrations metadata bloquée par une étape préexistante sur une table sans PK ; correctif fusionné `3d83ca75e` ; VÉRIFIÉ via l'API authentifiée sur le serveur reconstruit (11:19) : `pages/medals` sert « Clash of Kings » / « Kill a VIP while being a VIP yourself », image servie. CI verte.
