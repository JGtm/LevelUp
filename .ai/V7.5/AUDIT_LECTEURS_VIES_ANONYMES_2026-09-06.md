...
| **P2-1** `birthOfLives` (naissance du SLOT servie comme naissance de la VIE) | **CONFIRMÉ SUR PIÈCES, CORRECTIF HORS PÉRIMÈTRE (2026-09-07, lot R6)** — la racine est dans `filmdec` (décodeur gelé §0.6) : `bornAt(slot)` n'a que le slot en paramètre, `assembleEquipmentChanges` fusionne TOUTES les vies d'un slot recyclé en une seule chaîne (`mergeEquipEmissions`, groupement par slot) et n'interroge la naissance qu'UNE fois par slot (`hadPrevious=false` seulement au 1er élément de la chaîne) — aucun correctif côté `replay` (appelant) ne peut distinguer la vie concernée sans changer cette forme. Disposition doctrine §0.6 : diagnostic + registre, correctif reporté au plan décodeur post-v7.5.0 | `document_equipment_changes.go:169-183`, `filmdec/equipment_changes.go:150,162,421-424` |
| **P2-2** `coverage.equipmentChanges.lives` (compte des SLOTS publié comme des VIES) | **CONFIRMÉ SUR PIÈCES, CORRECTIF HORS PÉRIMÈTRE (2026-09-07, lot R6)** — même racine que P2-1 : `st.Lives++` s'exécute une fois par entrée de `mergeEquipEmissions` (`filmdec`, groupement par SLOT), qui mélange les vies d'un slot recyclé. Corriger le dénominateur exige de regrouper par (slot, vie) dans `filmdec` — décodeur gelé §0.6. Diagnostic + registre, correctif reporté | `document_equipment_changes.go:89-90,117`, `filmdec/equipment_changes.go:172-183,202-241` |
| **P2-3** borne de manche = MIN sur population non filtrée | **NON-LIEU — DÉJÀ CORRIGÉ** avant ce lot, par un commit distinct de la même campagne (`fix(manches/MANCHES-R1)`, 2026-09-07 00:05, `f1b4f4ee5`, déjà sur `feat/v75`) : `roundStartsOf` prend désormais `RoundStartsMS` (consensus par MÉDIANE sur majorité de slots, `chainedRounds`) comme source unique, avec repli au minimum brut UNIQUEMENT pour une manche sans consensus. Un enregistrement de bruit isolé (2,5 % mesurés) n'atteint jamais la majorité de slots requise (`2*len(debuts) <= slotsMax` exclut le round) : le scénario décrit par le constat ne peut plus se produire | `objectiveevents/slotidentity_rounds.go:140-190`, preuve `objectiveevents/round_bounds.go:267-300` (`chainedRounds`), commit `f1b4f4ee5` |
| **P2-4** `indexBySlot` web (geste d'un slot recyclé crédité au DERNIER occupant) | **CORRIGÉ (2026-09-07, lot R6)** — les 3 canaux consommateurs (`grappleLines`, `equipmentEpisodes`, `equipmentPlacements`) sont passés de l'agrégat match `indexBySlot` à `buildSlotOwnership(players).ownerAtFrame(slot, t0)`, résolu à l'INSTANT du geste ; mutation jouée (rouge sans le correctif) | `apps/web/src/features/match-replay/model/equipmentUsageLogic.ts` |
| **P2-5** jointures roster↔joueur perdant les BOTS (clés de deux espaces) | **CORRIGÉ (2026-09-07, lot R6)** — nouvel helper canonique `rosterEntryKey` (rosterLogic.ts, même dérivation que `seatLogic.filmIndexByIdentity`) substitué aux deux comparaisons directes `entry.xuid === player.xuid` ; le second site avait BOUGÉ depuis l'audit (`ReplayTeams.tsx:245` extrait vers `playerCardReadings.ts:92` le 2026-09-06, plan fiches compactes) — corrigé à son emplacement actuel ; mutations jouées (rouges sans le correctif) | `apps/web/src/lib/replay/rosterLogic.ts` (helper), `apps/web/src/features/match-replay/model/equipmentUsageLogic.ts`, `apps/web/src/features/match-replay/model/playerCardReadings.ts` |
