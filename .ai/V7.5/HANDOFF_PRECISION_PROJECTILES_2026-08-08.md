...
| « les projectiles portent des trajectoires dans le flux delta, la voie est vivante » | **VRAI, ET LE DÉCODEUR EST DÉJÀ LIVRÉ.** `filmdec/projectiles.go` décode ces trajectoires **en production**, câblé dans `replay.BuildFromFilm` avec goldens, et validé par **70 lancers de grenade sur 70** (0,77 u du bipède lanceur, 1,0° du cap de visée). ⚠ **Mais PAS la chaîne de §3** : la production part de l'événement de LANCER, qui porte déjà son auteur, et ne se sert du projectile que pour le PLACER. Le `0,77 u` est un contrôle, pas un mécanisme. Ce qui est acquis, c'est la **prémisse physique** — une naissance désigne son auteur sans ambiguïté ; ce qui ne l'est pas, c'est l'inférence en sens inverse pour les projectiles d'ARME, qui n'ont aucun événement de lancer |
