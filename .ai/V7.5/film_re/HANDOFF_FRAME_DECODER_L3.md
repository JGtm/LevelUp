...
- **Arme = id GLOBAL dans le film** (high-32 `WeaponIDToName`) : ~35 familles dans keyframe (type-2) + deltas (type-0). Tranche `../killweapon/REPRISE_KILLWEAPON_FILM.md` : **global stable, pas handle per-match**.
...
> Détails grammaire/architecture : `../killweapon/PLAN_FILM_KILLFEED_V3.md`. RE exe : `RE_EXE_GHIDRA_FINDINGS.md`.
> Reprise antérieure : `../killweapon/REPRISE_KILLWEAPON_FILM.md`.
...
**LE BUT (ancré)** : RE du **KILL FEED** = tracker d'événements — KILL (tueur · arme/**méthode** · victime), SUICIDE (joueur · méthode de mort), DÉPART/ARRIVÉE. PAS "l'arme au kill" isolée. Réf : `../killweapon/PLAN_FILM_KILLFEED_V3.md`. Arme **reconstruite** comme le moteur : hitscan → held du tueur **au tick** (état ECS reconstruit) ; projectile → entité projectile létale @position-de-mort. Events = chunk_27 (type-3, killer/victim/time/méthode).
