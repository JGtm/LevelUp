...
3. **Déplacer les données et réglages non suivis — y compris, et d'abord, les CACHES DE FILMS** : `data/cache/film_chunks` (1 351 films, 34 Gio au 2026-09-13), `data/cache/film_manifests` (1 351) et `data/cache/replays` (141 artefacts) sont irremplaçables (les films expirent côté Halo) ; compter les entrées AVANT et APRÈS le déplacement, et ne supprimer `LevelUp-go-migration` (étape 7) qu'après ce comptage. Le déplacement complet (renommage sur le même disque, instantané) :
...
- Elle ne supprime pas les worktrees `LevelUp-wt-*` non fusionnés (`film-residus`,
  `section3-chunk00`, `ti11-cadre`) ni les 2 branches WIP conservées du lot F
  (`wt/psa-index-cause`, `wt/lusr-h5-cause`).
