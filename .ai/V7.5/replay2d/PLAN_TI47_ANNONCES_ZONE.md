...
- [ ] 0.1 Porter la déser de `ti=47 i2` (patron des composants déjà portés, table de déser
      existante ; largeurs depuis le descriptor +0x28, jamais devinées).
- [ ] 0.2 Instruments de recherche gatés par env var `TI47_FILM` (patron exact de
      ...
      principal.
- [ ] 0.3 Recensement masque -> atteint sur >= 6 films couvrant >= 3 modes (2 Strongholds,
      2 KOTH, 1 CTF témoin négatif, 1 Slayer témoin négatif), AVEC filtre réel/fantôme
      (rattachement à une vie/entité confirmée, leçon i27).
