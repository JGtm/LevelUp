...
| **[2026-08-08] La ligne ci-dessus est PERIMEE sur son bloqueur** | Le « bloqueur » `i0` de `ti=41` ne bloque que **6,2 % des records** : `filmdec/projectiles.go` decode la branche dominante **en production** (rejeu 2D, 70/70 lancers de grenade). Ce qui reste ferme, c est la branche a 60 bits — et la voie qu elle ouvrirait vers `i18 projectile-at-rest-state` reste ouverte sur les 93,8 % restants. **Avant de rouvrir cette ligne : greper `filmdec/projectiles.go`.** | note i0 ti=41 §11.3-11.4 · MESURE |
...
| QUESTION | REPONSE | SECTION + STATUT |
|---|---|---|
| Un enregistrement de degat (code 36 / `0xD2`) est-il un TIR ou une TOUCHE ? | ⚠⚠ **CONTESTE PAR LA PISTE E (2026-08-08) : C EST UN TIR POUR TOUTES LES ARMES, projectiles compris.** Deux mesures par ARME, contre les mesures par FAMILLE DE MODE ci-dessous : **(a) taux de porteur** (drapeau bit 110 a 0) — Needler **0.0067** en Fiesta et 0.0080 en standard, Pulse Carbine 0.0096, Ravager 0.0066, Cindershot 0.0033, SPNKr 0.0053, quand le MA40 vaut 0.4405 et le BR75 0.4242 ; **si le record etait la touche, ce taux vaudrait ~1**. **(b) cadence inter-record** — Needler **mediane 83.4 ms, q90 100.1 ms**, c est-a-dire son TEMPS DE CYCLE : un flux de touches n est pas periodique. Instrument calibre sur des valeurs connues (MA40 83.4 ms, Sidekick 184.1 contre 183.8 publie). **L agregat Fiesta `records/touches = 1.0983` reste vrai mais s explique par une coincidence** : le film n y montre que **0.306** des tirs et la precision y vaut **0.283**. ⚠ Ceci **retablit 7ter.81 (5)** (<< NON — c est un enregistrement de TIR >>), que 7ter.101 avait surcharge. Statut du contestataire : `[MESURE]`, non reproduit par un tiers — **a verifier avant de retirer la ligne d origine**, qui suit. | **piste E** (`VERDICT_PRECISION_PROJECTILES.md` §3) vs 7ter.101 · `[MESURE]` contre `ETABLI` |
| *(ligne d origine, conservee)* Un enregistrement de degat est-il un TIR ou une TOUCHE ? | **CELA DEPEND DE L ARSENAL — c est la correction de 7ter.101.** Sur arme a trace instantanee : **un TIR** (Tactical `records/shots_fired` **0.9300**, `records/shots_hit` **3.4655** pour 1/p = 3.601 ; egalite `|d|<=5` **45** contre **14.3** au fond permute, **0/200** ; et `records/tirs` reste **PLAT** 0.9305 -> 0.9457 quand la precision DOUBLE). Sur arsenal a projectiles : **une TOUCHE** (Fiesta `records == shots_hit` **174** contre **134.7** permutees, max 163, **0/200** ; BTB 43 contre 29.1, 0/200). **LA TOUCHE EST DANS LES DEUX CAS LE PORTEUR** (tableau A non vide) : `porteurs/shots_hit` 0.8509, et le taux de remplissage `porteurs/records` **EST** la precision du joueur (0.4267 contre 0.4462, erreur mediane 0.027, r = +0.82 ; par quartile 0.1724 / 0.2222 / 0.2727 / 0.3220 contre 0.1819 / 0.2465 / 0.2906 / 0.3558). | 7ter.98 amende par **7ter.101** · ETABLI |
...
| **[2026-08-08] La CAUSE de ce « PAR MAP », et elle est banale** | La table dumpee est celle de **Cliffhanger** ; l appliquer a une autre carte est exactement le bug que `filmdec/map_bounds.go` decrit en tete (« toutes les cartes dequantifiees avec les bornes de Cliffhanger — facteur 0,38 sur Catalyst »). Il ne fallait pas une table portable : il fallait **une table PAR CARTE**, et `cmd/mapquant-build` en produit une depuis les `.module` pour **15 cartes**, offline-pur. **La reserve tient, la voie qu elle fermait est ouverte ailleurs depuis.** | note i0 ti=41 §11.2 · MESURE |
| **[2026-08-08] La table `DAT_1445cc9e0` dumpee apporte-t-elle une information neuve ?** | **NON — 32/32 niveaux en ACCORD avec la forme fermee** `min(26, ceilLog2(min(ceil(40000/(2*step(L))), 2^22)))`, `step(L)=2^(16-L)/120`, etendue de `DAT_143b8c6b8`. Ce n est **que la loi precalculee au chargement de carte** : elle confirme le desassemblage, elle ne le complete pas. Corollaire : « les largeurs ne sont pas derivables statiquement » (note §2.2) est **FAUX** — la globale est elle-meme derivee de constantes statiques. | note i0 ti=41 §11.1 · MESURE |
| **[2026-08-08] `FUN_1406d310c` = `bitLen` ou `ceilLog2` ?** | **`ceilLog2`.** Les deux coincident partout SAUF sur les puissances exactes de deux — et c est ce cas qui decide des niveaux 17 a 22 (saturation du compte a `2^22`). `filmdec/map_bounds.go` ecrivait deja `ceilLog2` : **c est la production qui avait raison contre le desassemblage du dossier.** | note i0 ti=41 §11.1 · MESURE |
| **[2026-08-08] L AABB `.module` du catalogue est-il le bon ?** | **OUI, confirme par une source INDEPENDANTE.** L entree 0 de `ce_prec_ranges_14462cbe0.bin` (dump memoire runtime) EGALE au flottant pres l entree `cliffhanger`/`ridgeline` de `data/titles/halo_infinite/reference/map_quant_bounds.json`. Jusqu ici le catalogue n etait controle que par `DetectI0Layout`, c est-a-dire par les largeurs mesurees dans le film — un controle indirect. **La chaine `.module` -> table runtime -> largeurs -> positions est validee de bout en bout.** | note i0 ti=41 §11.2 · MESURE |
...
| **PISTE E — PRECISION PAR ARME DES ARMES A PROJECTILE : VERDICT NEGATIF, TIMEBOX CLOS (2026-08-08)** | `piste E` · `tmp_pjcnt` · `taux de porteur` · `cadence inter-record` · `83.4 ms` · `0.0067` · `porteurs_tous` · `0.1729` · `0.8556` · `31 400 touches` · `deconvolution` · `grain joueur` · `resolvePIFast` · `576 accords` · `DAT_1451222c8` · `entry+0x10` · `objet + 0x114` · `ti=41` -> **`.ai/V7.5/VERDICT_PRECISION_PROJECTILES.md`** (§0bis = le verdict en une page). **Cinq resultats a ne pas re-chercher** : (1) le compteur de tir 7 bits ne porte PAS les tirs de projectile invisibles (pas moyen Needler 1.3383 contre BR75 1.3545 — il mesure la completude du scan) ; (2) les touches de projectile ne sont PAS dans les records de degat, filtre d arme leve (Fiesta 0.1729 des touches API contre 0.8556 en Tactical, 31 400 touches sans porteur) ; (3) le record est un TIR pour toutes les armes (cf. ligne dediee plus haut) ; (4) la porte `DAT_1451222c8` explique l absence des stats 0x0D/0x0E ; (5) la deconvolution au grain joueur (8 562 observations, coefficients bornes) rate son controle positif : **2 armes sur 4 a +-0.03** (BR75 +0.0043, Bandit Evo -0.0246 ; MA40 -0.0324, Sidekick +0.0461). **Ce qui RESTE ouvert et n appartient pas a cette piste** : rattacher l ENTITE projectile a un joueur par sa TRAJECTOIRE — bloque par `object-position-component` de `ti=41`, non bit-exact (45 bits contre 60). |
...
| Qu est-ce que `RoundsCorrected` ? | **Un terme de RECONCILIATION PAR LES MUNITIONS** : `FUN_1410af034` ajoute `(short)(dMag + dReserve)` a **`entry+0x10`**, appele par tick avec le delta de munitions mesure avant/apres. **Le jeu sait lui-meme que son compteur de tirs perd des coups, et il le rattrape.** ⚠⚠ **DEUX CORRECTIONS DU 2026-08-08 (piste E, re-decompilation)** : **(a) l offset est `+0x10`, PAS `+0x08`** — huit octets, et ils changent le sens : la correction **ne touche jamais** le compteur de tirs, qui vit a `entry+0x08` (`FUN_1408df45c`), et les touches a `entry+0x0c` (`FUN_1408df6a4`), meme enregistrement de pas **0xa8**, meme resolveur d indice `FUN_1408df3dc`. **(b) le << NON TESTE >> se leve dans le sens rassurant** : les trois compteurs sortent comme **trois champs nommes distincts** du meme evenement de telemetrie `FUN_140ad4e74` (chaines adjacentes espacees de 0x10 : `RoundsCorrected` 14369eec0, `ShotsLanded` 14369eed0, `ShotsFired` 14369eee0). Aucune sommation **dans le chemin lu** — le producteur des noms agreges `TotalShotsFired` / `TotalShotsLanded` n a PAS ete trace. | 7ter.81 (1f) **corrige par piste E** (`.ai/V7.5/VERDICT_PRECISION_PROJECTILES.md` §4bis) · `[MESURE]` |
...
| **POURQUOI le push de 0x0D / 0x0E n arrive-t-il pas dans le film ?** | **PARCE QU IL EST DERRIERE UNE PORTE.** `FUN_142bb2d80` (0x0D) et `FUN_142bb2e28` (0x0E) ne poussent que si `*(obj + 0x59d09) != 0` **ET** `DAT_1451222c8 != 0`. Or **`DAT_1451222c8` porte EXACTEMENT QUATRE references dans le binaire, et les quatre sont des LECTURES** — 0x0D, 0x0E, 0x13 (`FUN_142bb24c8`), 0x14 (`FUN_142bb2580`). **Aucune ecriture.** Les kills et le score passent par un AUTRE chemin, et ce sont eux qui arrivent. **Meme motif que la porte du tag des codes 6/7** : un chemin qui existe et dont la porte n est jamais ouverte. Le negatif de 7ter.83 (1) cesse d etre un resultat sans cause. ⚠ Portee : << aucune ecriture parmi les xrefs >> n est pas << aucune ecriture >> (un ecrivain par adresse calculee y echapperait) — ce qui rend l enonce exploitable est sa **convergence avec la mesure offline**, qui a son propre controle positif. | piste E (`VERDICT_PRECISION_PROJECTILES.md` §4bis.3) · `[MESURE]` |
...
| **[2026-08-08] Le « 45 / 60 » de `ti=41` : la part reelle de la branche a 60** | **6,2 %**, pas la moitie (mesure `tmp_i0w` REFUTEE). Grammaire de production sur 30 films : 9 382 records opaques sur 152 535. Deuxieme instrument, **deja dans le cache film** : `data/cache/film_chunks/<match>/calib.txt` porte `object-position-component:45` a une frequence modale de **0,98**. Enjeu reel : 12 % des VIES de projectile n ont que la branche opaque -> perdues par le rejeu 2D. **Gain de couverture sur une feature livree, pas un deblocage.** | note i0 ti=41 §11.4 · MESURE |
| **[2026-08-08] Les trajectoires de projectile sont-elles decodees ?** | **OUI, EN PRODUCTION, depuis le chantier rejeu 2D.** `filmdec/projectiles.go` (`ScanFilmProjectiles`, archetype `ti=41`), cable dans `replay.BuildFromFilm`, avec goldens. Verite terrain : **70 lancers de grenade sur 70** apparies a une naissance d entite (contre 5,3 % par decalage circulaire), distance au bipede lanceur **0,77 u** (6,4 u a instant permute, 33,9 u au hasard), direction initiale a **1,0°** du cap de visee. ⚠ **Ce n est PAS la chaine « naissance -> joueur le plus proche -> tireur »** : la production part de l evenement de LANCER (qui porte deja son auteur) et ne se sert du projectile que pour le PLACER. Le `0,77 u` est un CONTROLE, pas un mecanisme. **Acquis = la premisse physique** (une naissance designe son auteur) ; **non acquis = l inference inverse** pour les projectiles d ARME, qui n ont aucun evenement de lancer. **GREPER `filmdec/projectiles.go` AVANT de rouvrir quoi que ce soit sur `ti=41`.** | note i0 ti=41 §11.3 · MESURE |
