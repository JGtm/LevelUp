...
| `tmp_forgeshape` | `fields` `inventory` (champs du record .mvar) · `shapes` (extraction des formes) · `coverage` (couverture par carte) · `props` (champs jetes) · `upz` (orientation sol/mur) · `types` `zones` `obj` · **`cratevar`** (variantes de caisse par objet + divergences) · **`cratedump`** (arbre `#8/#1` d'un type) · **`crateobj`** (detail par objet : position, variante, style, reapparition) · **`repname`** (les 848 StringID de representation) · **`names`** (les chaines ecrites par les auteurs de cartes — vivier de vocabulaire) · **`spawners`** (les 3 predicats de reconnaissance d un emplacement) · **`observe`** (la table d observation : par Representation Name, ou et a quelle cadence) |
