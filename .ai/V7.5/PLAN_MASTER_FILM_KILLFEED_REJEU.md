...
| **D — icônes d'armes** | `PLAN_RECHERCHE_ASSETS_ICONES` → **phase 1 CLOSE 2026-08-08**, résultats dans `V7.5/icones/ETAT_DE_L_ART_ICONES.md` | `himodule/` lecture, `static/`, `TitleAssetURLAdapter` | rien ; peut échouer sans dette ; **GATE HUMAIN entre phase 1 et phase 2** (revue visuelle utilisateur AVANT toute intégration — décision #4) |
...
| `PLAN_RECHERCHE_ASSETS_ICONES` | **bon** | Bien isolé, échec sans dette, licence = question utilisateur AVANT intégration. La règle « vérifier visuellement, jamais le nom de fichier » est déjà payée | **Phase 1 close le 2026-08-08** : la voie A a rendu mieux que le gate demandé — le lien arme→icône est LU dans le jeu (29/29), et l atlas dit « sandbox » s est révélé être celui du kill feed, avec sa propre table de nommage.
...
| **Hygiène** — dette H1-H6, résiduels C-bis R1/R2/R3, gate des artefacts, lot E | **FAIT 2026-08-05** — statuts item par item ci-dessous (§ « STATUTS À LA CLÔTURE DU LOT D'HYGIÈNE »). Ouvert par un merge d'`origin/main` puis re-mesure du ratchet : **0**. Le gate des artefacts est **PASSÉ** (§ dédiée) : régénérés au commit pré-intégration, les 3 artefacts sont BIT-IDENTIQUES à ceux en place. `go.mod` monté 1.26.1 → 1.26.5 : c'était la **CI** qui compilait avec la version aux 11 CVE, pas le poste | trois défauts LATENTS trouvés en vérifiant plutôt qu'en croyant : un horodatage de préséance à +2 h (R1), un oracle qui skippait sur toutes les machines (R3), et une CI sur toolchain vulnérable (E6) | 1 |
...
### STATUTS À LA CLÔTURE DU LOT D'HYGIÈNE — 2026-08-05

Le lot a été ouvert par un merge d'`origin/main` (3 commits de retard) puis une re-mesure du
ratchet lint sur la nouvelle base : **0 issue** (F2 du plan de dette). CI verte au niveau job
sur le dernier commit porteur de code avant le lot.

| # | statut | ce qui a été livré | ce qui le garde |
|---|---|---|---|
| H1 | `[x]` | les deux commentaires corrigés, **et un troisième site trouvé en vérifiant** : `player_repos_test.go` CRÉAIT encore la vue dans son fixture — code mort testé, retiré | le test d'empty reste vert pour la bonne raison (la table canonique du fixture est vide) |
| H2 | `[x]` | compteur `convergence_events_fetch_failed_total` + `WarnContext` **avant** la dégradation, même traitement que J4R-5. WARN et non ERROR : un fetch réseau échoue légitimement, c'est la répétition qui fait le signal | `TestEventsConvergence_FetchError_Skipped` lit le compteur — **mutation vue rouge** (`want +1, got +0`) puis re-verte |
| H3 | `[~]` | **déjà refermée le 2026-08-03** avec l'inversion de préséance, avant l'ouverture de ce lot : les valeurs vivent dans `domain/killscope`, `persist.FilmReadPaths` les lit, la frontière est tranchée (le propriétaire typé reste `killsource.Path`, title-specific) | `archlint/no_raw_kill_scope_literal_test.go` police `marche`/`scan` depuis le 03/08, **allowlist VIDE** — vérifié rouge par mutation dans cette session ; `killcollector.TestFilmReadPathsEgalesAuDecodeur` verrouille l'égalité |
| H4 | `[x]` | l'outil ne refait plus le swap : il **délègue** à `migration.RebuildMatchParticipantsART`, le chemin de production testé. Plus de liste `dependentViews` en dur, plus de `DROP CASCADE` + `CREATE VIEW`. `//go:build ignore` → `//go:build cgo` : il est enfin compilé, vet et lint. Le diagnostic passe par la sonde existante `duckdb.ProbeARTDivergences` au lieu du match codé en dur de l'incident de mai | `TestRebuildMatchParticipants_AvecVuesDejaPresentes` — les cinq tests existants partaient tous d'une base SANS vues, donc le mode de panne réel n'était couvert par aucun. **Mutation vue rouge** : retirer un `OR REPLACE` reproduit au texte près l'erreur de J4R-7 |
| H5 | `[x]` | les 4 coquilles supprimées ; les **trois** assertions qu'aucun autre test ne couvrait (vue supprimée qui ne revient pas, repli du bot inconnu, idempotence 3 passes) réveillées dans `games/halo_infinite/migrations/shared_end_to_end_guards_test.go`, le seul paquet qui peut importer `migration` ET lui fournir ses steps. **Effet de bord rattrapé par la CI** : les 4 noms figuraient encore dans `.ai/baselines/tests_pre_migration.jsonl` — 20 lignes retirées (procédure du dépôt) | 3 tests dans la suite NORMALE (pas seulement `-tags=integration`) ; garde **anti-vacuité** sur l'idempotence ; **2 mutations vues rouges**. La baseline est un gate **Linux** : rejoué sous Windows il rougit pour 57 paquets `cmd/*` qui n'y compilent pas — seule la CI tranche |
| H6 | `[x]` | 3 documents déplacés, liens croisés mis à jour dans 8 fichiers. **3 candidats REFUSÉS après vérification, et le refus est le résultat** : HANDOFF_REPLAY_2D est la porte d'entrée déclarée d'un plan ACTIF ; HANDOFF_DUMPS et SESSION_CAPTURE portent 19 cases jamais statuées ; CONCEPTION_INVERSION_PRESEANCE et PLAN_BRANCHEMENT_KILLSOURCE sont cités PAR LEUR CHEMIN dans 6 fichiers Go | les journaux ne sont pas réécrits (règle posée par `V7.5/README.md`) |
| R1 | `[x]` | la reprise des couples écrivait `now()` NU dans `written_at`, donc l'heure de SESSION dans une colonne TIMESTAMP naïve — sur ce poste, **deux heures dans le futur**. C'est la colonne de tri de `_latest` : une passe de film écrite dans ces deux heures perdait l'arbitrage, et l'enrichissement disparaissait de la lecture sans erreur ni compteur. Le harnais de test mélangeait les mêmes deux horloges | `TestRepriseHorodateEnUTC` force `TimeZone = 'Pacific/Kiritimati'` pour que la mesure ne dépende pas de la machine — **mutation vue rouge, écart mesuré exactement 14 h** |
| R2 | `[x]` | l'entrée de baseline contrat retirée (47 → 46). La prémisse de son commentaire était FAUSSE (l'unique appelant envoie toujours les deux champs) ; la vraie cause, trouvée en tentant le shim puis en lisant `tsc` : `filters` pointait un schéma de RÉPONSE. Le fragment manuel décrit désormais la forme PRODUCTEUR | contrat et `generated.ts` **régénérés par les générateurs** ; `tsc -b --force` tombait avant le correctif, passe après ; ratchet contrat clean |
| R3 | `[x]` | **l'oracle est rejoué** : 696a9d7c 16 confrontations 8/8, 1bc77d2e 48 confrontations 8/8, plus le pont d'identité et la vérité terrain CTF/zones — 30 tests, aucun skip. Ce qui les taisait n'était pas « le cache n'est pas sur ce poste » mais un **chemin écrit en dur vers un poste disparu** : ils skippaient PARTOUT | `FILM_CACHE_ROOT`, même convention que `KILLSOURCE_FIXTURES` ; chaque message de skip nomme la variable et sa valeur ; skip propre vérifié dans les deux sens |
