...
- [x] 1.9.4 **La carte du film vient du nom de match, plus d'une signature de largeurs.**
      ...
      **LA SIGNATURE N'EST PAS DEVENUE UN REPLI, ET C'EST UN ÉCART ASSUMÉ AU BRIEF (D3 (1.9.4) en §4).**
      ...
      (`repli_distances_de_touche_desactivees`). **À confirmer par le pilote.**
      ...
      **RÉVISIONS.** `GrammarRev` **grammar-2026-09-15.7 -> .8** (golden régénéré, ligne d'historique
      ...
      pas cette fonction) — à confirmer par l'équivalence. Aucun contrat publié ne change (`git status`
      ...
      **STATUT DES ITEMS DU BRIEF.** (1) `[x]` mesure avant de coder — (a) grep des appelants, (b) 17
      ...
      repli déclaré »** — non traité DÉLIBÉRÉMENT, justification mesurée en D3 (1.9.4), à confirmer par le
      pilote ; **`[~]` pour « contrôle accord / contradiction compté »** — oracle de test, même motif que
      ...
      (5) `[x]` révisions statuées une par une, population chiffrée (0 match).
...
| 2026-09-15 | 1.9.4 (seuils du dépôt) | ce commit | `wc -l` des fichiers touchés | `positions.go` **508 -> 494** (la résolution de carte en sort), `hits.go` 246 -> 296, `map_identity.go` 100, `weapon_hit_distance_resolver.go` 234 -> 209. **Aucun fichier au-dessus de 500**, la dette baisse de 14 lignes |
