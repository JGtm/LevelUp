...
| 2026-09-14 | 1.2 revue R1 (C1) | **D6 (1.2) — `Registry.TruncatedBytes` est POSÉ mais n'a encore AUCUN LECTEUR.** Le champ compte les octets de queue qu'aucun bloc entier ne couvre quand le parse a épuisé le tampon (0 sur les 1 351 `chunk_00` du cache). Il rend la troncature NOMMÉE et comptable au sens de D14, mais aucun des deux appelants de production (`killcollector/hits.go`, `filmdec/film_context.go`) ne le journalise : un `chunk_00` tronqué ne fait plus tomber le processus, il rend un registre court SANS que rien ne le dise en exploitation. NON TRAITÉ dans ce lot (brancher un journal touche des appelants hors périmètre). | **à compter au registre des replis du lot 1.9.0** (nom, fait, condition de déclenchement, date de pose, critère de retrait) |
