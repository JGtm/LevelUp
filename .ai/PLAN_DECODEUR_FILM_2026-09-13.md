...
| 2026-09-16 | 1.9.9 | **D1 (1.9.9) — `0x038df01a` N'EST PAS LE SEUL CHÂSSIS INCONNU, ET L'UN DES AUTRES EST MOBILE.** Mesure AVANT de coder (`TestInventaireChassisDesArtefacts`, 76 artefacts du parc, schéma 54, lecture seule — aucun décodage) : **33 châssis distincts**, dont 25 hors table. Trois sortent du bruit. `0xae845375` : **18 vies sur QUATRE films** (`4f77afc1` 11, `5676a9ba` 4, `0a44c6cc` 2, `8a485699` 1), la plupart MOBILES (376 à 2 352 échantillons) — c'est un VÉHICULE CONDUIT dont la table ignore le châssis, pas un objet de carte, et `labels.tsv` ne le nomme pas. `0x20737e7f` : 6 vies IMMOBILES, fenêtre = le match entier, sur le seul `396cfc92` — même signature que la tourelle bannie, autre identifiant. `0xf6f54e56` : 3 vies mobiles sur trois films. Le reste est une QUEUE DE 21 SINGLETONS (1 vie chacun, 17 sans aucun échantillon), concentrée sur `4f77afc1` (9) et `5676a9ba` (4) : signature d'un bruit de lecture, et les artefacts du parc sont ANTÉRIEURS aux gardes `n1`/`n2` du lot 1.9.1 bis (qui déroulaient des records à `n2 == 0`) — la mesure définitive de cette queue demande une re-cuisson. NON TRAITÉ (règle 7) : le lot 1.9.9 ne nomme QUE la tourelle ; les autres restent comptés par `repli_chassis_vehicule_marqueur_neutre`, dont la cible de retrait passe donc de 1.9.9 à « le lot qui les nommera ». | lot 1.9.10 (véhicules) pour `0xae845375` et `0xf6f54e56` ; `0x20737e7f` à instruire comme la tourelle (banque de sons inconnue de `labels.tsv`) ; la queue de singletons à re-mesurer APRÈS la recuisson du parc de fin de M1 |
