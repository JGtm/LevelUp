...
| (phase 5b, `NOTE_RESIDUS_CHUNK00_2026-09-13.md`) Table des slots : les deux écarts aberrants sont des slots VACANTS (booléens 0/0/0, XUID nul, 13 619 bits chacun) ; la transposition par build est portée par le SEUL bloc de personnalisation (`sub+0xcc0` : 1 852 / 1 492 / 1 312 / 2 052 o), tout le reste vaut 270 bits sur les 7 builds ; le lecteur par grammaire calibré sur le film lit 32 slots exactement sur 1 351 films sur 1 351 | fermeture 2 121/2 121 écarts sur 96 films ; `HI_1_9_0` et `HI_1_12_0` ajoutés au tableau (186, 459/460 bits) | `profile` (`Slots` : largeur du bloc de personnalisation par build) + `source` | lecture de la table des slots exacte sur tous les builds, y compris 2025 ; le résidu `+200` de `HI_1_4_1` reste ouvert |
| Drapeau de contrôle par build : INACTIF partout, mesuré (le mot qu'il gate est la sentinelle `0x0FFDDCBA`, 0 occurrence sur 250 paquets, 7 builds) | témoin positif 4 368/4 368 lisent `n2` à la place | `profile` | pas de forme à 218 dans le cache ; à garder comme présumé pour un build futur |
| Désignateurs : `team_id 0` = désignateur 0 = `First` (empilement `team_0..team_7, team_neutral` prouvé) ; aucun film ne porte plus de 2 désignateurs (2 équipes sur 595 matchs de Grande bataille) | phase 5b | `facts` | l'hypothèse « jusqu'à 8 équipes en BTB » est réfutée ; le libellé affiché exige le corpus `gamefiles` |
| Pied de film : l'équipe d'un événement est à l'OCTET 37 (et 38), pas 55 : `b37` = équipe 665/665 sur 14 films, `b55` = 0 sur 665 | phase 5b, plancher 4 lectures parfaites sur 180 essayées | `facts` | correction d'UNE ligne en production (`objectiveevents/film.go` lit `b55`) qui rend l'équipe des événements d'objectif fiable sans ouvrir la base ; à faire sous corpus gate |
| (phase 5a, `NOTE_IMAGECLE_ETAT_COMPLET_2026-09-13.md`) Record d'image-clé : sur 62 686 records bornés (6 films, 3 builds), le modèle de la production ferme 0 record ; la forme d'état complet en ferme 8 796 (14,0 %, plancher de hasard 0,8 %), 8 archétypes gagnent, 0 régresse, ti=6/15/18/22 à 100 % ; cinq largeurs d'état par défaut manquantes désignées par `n2` et confirmées au désassemblage (ti=14 → 6, 17 → 8, 21 → 18, 29 → 1, 47 → 6), projection 30,8 % avec trois d'entre elles | mesuré + décompilé | `grammar` (cadre) + `profile` (états par défaut par archétype) | le correctif ne répare la sortie d'AUCUNE page aujourd'hui (seul `bomb_armings.go` lit un corps d'image-clé) : il ouvre une voie, il n'en répare pas une ; aucun compteur de production ne voit le déraillement par le cadre |
