...
## 4. Découvertes
- (passe 6) Sessions : la rangée des niveaux n'affiche pas sa propre couverture (seul « Matchs mesurés 9/11 » du résumé d'usage) — à ajouter si les périmètres divergent un jour.
- (passe 6) Une URL Sessions avec un libellé de session inconnu du contexte courant renvoie 404 « Session indisponible » (lien partagé dont le contexte a changé).
...
- 2026-09-14 — Fusionné dans feat/v75 (`21f42544b`) après les deux revues. Rattrapage LOCAL exécuté serveur arrêté : `--dry-run` puis réel — 76 matchs écrits, 1 200 lignes, 1 495 prises, 0 match sans carte à la référence, 0 échec, 3 s. Serveur relancé ; passe visuelle 6 sur données réelles CONFORME : match view base 17 / terrain 28 / puissance 22 ; Sessions (session escouade du 28/07) trois jauges avec leurs propres parités et « — » sans zéro ; Escouade base 24 / terrain 32 / puissance 40 / non identifié 2, pied « Mesuré sur 44 matchs sur 427 » distinct du voisin (54/427) ; Timeseries pied 18/529 vs 30/529 + deux notes explicatives ; EN vérifié (« Gravity Hammer »). Réserve P2 consignée : sur Sessions la rangée porte ses parités mais pas sa propre ligne de couverture (périmètres coïncidents sur la session testée). RESTE : rattrapage PROD à la release v7.5 (`backfill-pad-tiers` après `backfill-flag-grabs-net`).
