...
- **Table ECS universelle** : `DAT_144e61d88` (runtime) → descripteur par typeIndex → deser par composant (`vtable[0x28]`, ou `[0x30]` si thunk `FUN_14076ce9c`). Constante de build. Recette + procédure d'extraction CE : **`.ai/RECETTE_DECODAGE_FILM_CHUNKS.md`** + mémoire `reference_ecs_runtime_deser_table`. Table des 64 desers du biped dans la recette §6.
...
- Recette : `.ai/RECETTE_DECODAGE_FILM_CHUNKS.md`. Plan CE : `.ai/PLAN_CE_BREAKPOINT_VALIDATION.md`. Journal : `.ai/thought_log.md` (entrées 2026-07-01/02).
