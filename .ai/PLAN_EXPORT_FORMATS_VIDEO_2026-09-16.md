...
- [!] Gate visuel : la main a l'utilisateur (deux exports 1080p/720p ouverts dans un lecteur/monteur). — report valide (verification que seul l'utilisateur peut faire). A regarder : dimensions reelles des deux fichiers ; bandes autour d'une carte non 16:9 a la couleur du bloc (fond `--card`) et non noires ; nettete du texte HUD en 720p (choix du rendu direct NON MESURE) ; a l'ecran pendant l'export, image inscrite sans deformation ni saut de page, puis retour a la taille d'ecran.
