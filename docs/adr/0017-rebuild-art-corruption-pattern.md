...
**Date** : 2026-05-22
**Status** : ✅ **CLOSED (2026-05-24)** — superseded by ADR 0019 + Phase 4 (post-sync batch INSERT-only). L'auto-heal runtime au boot a été SUPPRIMÉ le 2026-05-24 (Phase 5 cleanup) suite à validation empirique de Phase 4 (16 syncs / 0 FATAL). Le rebuild ART reste disponible comme **outil ops one-shot** via `cmd/rebuild_mp/` (shared `match_participants`) et `cmd/rebuild_pme_art/` (player `player_match_enrichment`). Le CLI `cmd/force_rebuild_art/` et son étape `match_skill_rank` ont été SUPPRIMÉS le 2026-09-13 : cette table est append-only depuis la v5.3 (ADR 0026) et ne peut plus porter de `PRIMARY KEY (match_id)` — le rebuild échouait, et s'il avait passé il aurait détruit l'invariant append-only et la vue `_latest`. Requis avant tout déploiement Phase 4 sur DB héritée d'un mode legacy (corruption pré-existante).
