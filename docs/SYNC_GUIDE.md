...
The film decoder is five layers (ADR 0034, D-1), and its revised layers carry a revision of their own (D-6): `source.Rev`, `profile.Rev`, `grammar.Rev`, and — since 2026-09-26, one per consumer of facts — `killsource.Rev` and `objectives.Rev`, each in its layer's `rev.go`. It is `killsource.Rev` that commands the backlog (it took over the value of `facts.Rev`, `killsource-2026-09-24`, so no backlog was reopened by the split).

A revision is a string frozen in a golden beside the fingerprint of that layer's perimeter: a source that changes without its revision reddens the fingerprint test, and a revision that changes without its source reddens it too. The fingerprint hashes the **tokens** of the sources (comments and layout are ignored, `//go:` directives count) and the files they embed. The **perimeter** is the closure of the layer's production imports inside the module: another revised layer enters by its **value**, any other package by its sources; it is frozen in `testdata/<layer>_perimetre.golden`. The calculation, the closure, the chronicle, the regeneration door and the failure messages are shared (`internal/games/halo_infinite/film/revision`); a layer only declares its exclusions, the values of the layers it imports, its golden, its door and the question its failure asks.

| Revision | What it hashes | It rises when |
|---|---|---|
| `source.Rev` | the source layer (loading, decompression, chunk and packet cutting, the canonical bit reader) and `film/types` | the way the bytes are reached changes — everything is re-decoded |
| `profile.Rev` | the profile layer (the table per build and per map); it imports no other layer | one row of the profile table changes |
| `grammar.Rev` | the grammar tree and the packages it imports (`film/types`, `games/weapons/filmshell`...), plus the **values** of `source.Rev` and `profile.Rev` | a width, a frame, a component order, a new reader |
| `killsource.Rev` | `facts/killsource` and the packages it imports (`film/damagetag` with its embedded tables, `film/types`...), plus the **values** of `source.Rev`, `profile.Rev`, `grammar.Rev` | the kill-feed output can change — backlog candidate |
| `objectives.Rev` | `facts/objectives` and the packages it imports (`domain/objectiveevent`, `film/types`), plus the **value** of `source.Rev` | the objectives / statborg output can change — no backlog, the facts and the objective layers go stale |

Each layer hashes ITS sources and the VALUES of the layers it imports — never their bytes. That is the one link that does not go without saying, and the defect it closes is measured: a grammar fix can change the kill-source output without touching a byte of the facts layer, the revision then stood still, and the rows already written carried the running revision — excluded from the backlog for good. Chaining the values makes it mechanical: the lowest layer that rises raises every layer above it, up to the backlog. A false positive costs one re-decode; a false negative costs a fleet of wrong rows.

**What the backlog is** (in force, under `killsource.Rev`, formerly `facts.Rev`). Every row of `match_kill_events` carries in `decoder_rev` the revision that produced it. A match whose current pass — read through the `match_kill_events_latest` view, never the raw table (ADR 0026) — does not carry the running revision becomes a candidate again (`conditionBacklog`, `internal/sync/killcollector/postsync.go`). The post-sync step catches up at a bounded pace (8 films per cycle, a five-minute budget) and publishes what is left in the expvar `killsource_postsync_backlog_restant`; `levelup backfill-killsource --online` drains it deliberately. Those bounds hold per cycle and per title, not per player: the v2 cycle runs each player's post-sync in its own goroutine against the same global backlog, so only one pass runs per process and title at a time — a concurrent call steps aside, returns 0 and is counted in `killsource_postsync_passe_deja_en_cours` (normal whenever several players sync together, not a fault). A re-cook of the whole park stays a separate gesture, taken on user signal, never per lot. Since schema 62 that pass is no longer necessarily a re-decode: `replaybuild.Digest.Verdict` answers `a-jour`, `republier` — the decoding is intact and only the publication moved, so the document is replayed from the facts persisted per film under `data/cache/film_facts/`, in hundreds of milliseconds instead of tens of seconds — or `redecoder`, when a layer revision moved or the facts are missing; `levelup backfill-replay --only-existing` reports both counts.
...
**Reading the revisions off an artifact** (born at schema 61, extended at 62). A cooked replay document carries `coverage.decoder.{sourceRev, profileRev, grammarRev, killsourceRev, objectivesRev, build}` (`factsRev` up to schema 71) (`GET /players/{player_slug}/matches/{match_id}/replay`): the artifact says under which revisions it was cooked, instead of being guessed from its schema version. `build` is the profile key, read in clear text in `chunk_00` (D-3); it is the empty string — the block staying present — both when the film writes no build at all and when the profile table does not know the one it wrote (`ErrUnknownBuild`). The **absence** of the block means "artifact cooked before schema 61", never "unknown build". The sub-block `coverage.decoder.registry` classifies the film's ECS registry fingerprint (`fingerprint`, `status` = `connue` / `presumee` / `inconnue`, `blocks`, `namedSlots`); it is absent when the registry was not read. Since **schema 62** the document also carries `layers`, one revision per cooked layer: the presence of a layer is read in its revision, never in the absence of a field.

**A film whose key is unknown is set aside** (ADR 0034, D-4; in force since lot 3.1.1). The profile is a table indexed by the keys the film *writes* — its format version, its build, and its major version when it writes no build. When that key is absent from the table, neither `sync/killcollector` nor `replaybuild` decodes the film: no kill-source fact, no positions, no shots, no artefact — and never a decode under the neighbouring build's profile, which would produce a plausible and wrong document. Each producer emits one `WARN` naming the refused key, and the counters are the ones `grammar` names: `filmdec_unknown_build_<build>` (`sans_section` for a film with no identification section) and `filmdec_unknown_format_<n>`, plus `killsource_ecartes_cle_inconnue` for the passes stopped. The kill-source outcome is `ecarte-cle-inconnue` and the cooking child exits `filmproc.CodeSkipped`: an **aside**, never a failure. No registry marker is set — `MBitFilmAbsent` is terminal, whereas the film returns of its own accord the day the profile table carries the key. The procedure for adding it is [RUNBOOK_FILM_PROFILES.md](RUNBOOK_FILM_PROFILES.md).
...
### LUSR v2 post-sync: nothing new, no writer

The LUSR v2 step of the post-sync (`RunLUSRV2ShadowOwnerOnly`, `internal/sync/skill`) decides what is new on a **reader**, before it asks for the shared writer. Under the read segment it loads the player's candidate matches, the watermark of each mode group (`last_match_at` in `player_skill_state_v2_latest`) and, for the candidates above it, the shared eligibility predicate (two human teams, balance, scorable outcome). When nothing above the watermark can be scored, the cycle takes **no writer** and logs one INFO line per player, `lusr_v2: rien de nouveau`, with `candidates`, `new` = 0 and the counters of skipped candidates. Otherwise it takes the writer in **bounded bursts** for every candidate above the watermark: a burst releases the writer after 50 matches, or as soon as it has held it for 2 s (the shared provider's watchdog threshold), and the rest of the queue is resumed under a new burst in the same cycle. Under each burst every match goes through all its checks again on the read-write handle (held group, watermark, eligibility); a group held by a failed canonical write stays held across bursts until the end of the cycle. Each burst logs one INFO line, `lusr_v2: rafale bornée` (`candidates`, `new`, `bursts`, `matches`, `held_ms`, `remaining`), and the cycle logs `lusr_v2: rafale terminée` (`candidates`, `new`, `bursts`, `processed`, skip counters). What is written does not change: INSERT-only rows, reads through the `_latest` views, the same rows in the same order. Before this change (measured 2026-09-23), the step took the writer every three candidates and tested "already processed" inside the burst: 1,233 read-only to read-write swaps in under two minutes for zero rows written, each one draining the HTTP readers and dropping the DuckDB cache.
