...
State as of 2026-09-16: the repository carries TWO revisions — `GrammarRev` (the grammar layer, `internal/games/halo_infinite/film/filmdec/grammar_rev.go`) and `KillSourceDecoderRev` (the killsource facts, `internal/sync/killcollector/killsource_decoder_rev.go`) — and it is `KillSourceDecoderRev` that commands the backlog today; the cooked replay document is at schema 60. The table below describes the TARGET of lot 2.6 of the film decoder plan (four revisions, one per layer), not the present state: that lot will put this sub-section in the present tense.

The film decoder is five layers (ADR 0034, D-1), and four of them are to carry a revision of their own (D-6). A revision is a string frozen in a golden beside the fingerprint of that layer's non-test sources: a source that changes without its revision reddens the fingerprint test, and a revision that changes without its source reddens it too — the two revisions above already work that way.

| Revision (target, lot 2.6) | What it will hash | It will rise when |
|---|---|---|
