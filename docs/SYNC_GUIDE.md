...
The LUSR v2 step of the post-sync (`RunLUSRV2ShadowOwnerOnly`, `internal/sync/skill`) decides what is new on a **reader**, before it asks for the shared writer. Under the read segment it loads the player's candidate matches, the watermark of each mode group (`last_match_at` in `player_skill_state_v2_latest`) and, for the candidates above it, the shared eligibility predicate (two human teams, balance, scorable outcome). When nothing above the watermark can be scored, the cycle takes **no writer** and logs one INFO line per player, `lusr_v2: rien de nouveau`, with `candidates`, `new` = 0 and the counters of skipped candidates. Otherwise it takes **one writer burst per player and per cycle** for every candidate above the watermark; under that burst each match goes through all its checks again on the read-write handle (held group, watermark, eligibility), and the cycle logs `lusr_v2: rafale terminée` (`candidates`, `new`, `processed`, skip counters). What is written does not change: INSERT-only rows, reads through the `_latest` views, the same rows in the same order. Before this change (measured 2026-09-23), the step took the writer every three candidates and tested "already processed" inside the burst: 1,233 read-only to read-write swaps in under two minutes for zero rows written, each one draining the HTTP readers and dropping the DuckDB cache.
