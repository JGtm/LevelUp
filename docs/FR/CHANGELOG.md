...
- **(1) Re-projeter le niveau des armes des prises de socle** — les passes (1) à (4) se jouent dans cet ordre, **serveur arrêté** (un seul writer sur la base partagée). `levelup backfill-pad-tiers --force` : une arme de départ se lit désormais avant la première prise de la vie, et les lignes déjà écrites portent l'ancienne règle, que la reprise ne reviendrait jamais visiter.
- **(2) Recalculer la source des frags** — `levelup backfill-killsource` (100 % hors ligne, depuis le cache local des films) : la révision `killsource-2026-09-22.2` a changé les faits, donc chaque match décodé sous une révision antérieure est décodé de nouveau. 3 ouvriers par défaut, `--status` depuis un autre terminal, reprenable en relançant la même commande.
- **(3) Recuire les artefacts de rejeu** — `ReplayDocument.SchemaVersion` vaut 68 ; un artefact cuit à un schéma antérieur est servi avec un badge « à recuire » et certains calques restent vides. Lancer `backfill-replay --only-existing` sur le parc après déploiement : il republie depuis les faits de film persistés quand ils sont frais, redécode sinon et écrit les faits au passage (un film = un processus enfant ; **jamais** de cuisson en lot sans accord explicite).
- **(4) Re-projeter les usages d'équipement et de socle** — `levelup backfill-usage-summary`, après la recuisson : il re-résume quand la révision de projection ou le schéma de l'artefact a bougé.
