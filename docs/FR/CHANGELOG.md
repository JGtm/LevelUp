...
> La version du film Theater. 3 640 commits et 667 fusions depuis `v7.3.0` (tag `a2719a68c`, 2026-08-04), 7 867 fichiers touchés — de loin la plus grosse version du projet. Halo écrit un film de chaque match et l'application ne l'avait jamais ouvert ; elle le décode désormais de bout en bout et en tire un **rejeu 2D vu du dessus**, un **onglet Tactique**, et une série de mesures qu'aucun champ d'API ne porte : l'arme de chaque frag, la portée et la hauteur des engagements, le niveau des armes de socle, les prises nettes de drapeau, les statistiques d'Assaut, la riposte et l'appui entre coéquipiers, et l'équipement lu comme utilisé / gardé / lâché. Tout ce qui vient du film est gardé par des capabilities fines `film.*` et n'existe que sur Halo Infinite. Quatre ADR ont été écrits en chemin : **0032** (modes à manches), **0033** (population des sessions d'escouade), **0034** (décodeur de film — cinq couches, une porte unique aux octets, amendé à chaque jalon jusqu'aux faits de film persistés), **0035** (annuaire des joueurs — le xuid comme clé d'identité unique). **Des actions post-déploiement sont requises — voir *Ops*.**
...
- **Schéma de rejeu 54 → 68, une montée par changement publié** — 55 `coverage.tracks` (une vie refusée à la publication est comptée au lieu de disparaître) ; 56 le registre d'identité prend la table des 32 emplacements de joueurs du film (`chunk_00`) comme lien direct, et une vie d'un seul échantillon est publiée ; 57 l'équipe de chaque joueur est lue dans le film (`Track.Team`, `roster[].team`) au lieu d'être jointe depuis la base ; 58 `coverage.fallbacks` nomme et compte chaque repli déclenché pendant une cuisson ; 59 l'origine d'une pose d'équipement est lue (`deployed` = ce qu'un événement de type 103 désigne, `dropped` = ce qui quitte son porteur, le reste demeure `unknown`) ; 60 une vie finit à une mort écrite (un trou de réplication devient une lacune comptée), une vie de véhicule finit à son état inerte écrit (`vehicles[].end`), le Wraith, le Scorpion et la tourelle automatique bannie sont nommés, et le désignateur de manche est publié tel qu'écrit ; 61 `coverage.decoder` porte les révisions de couche sous lesquelles l'artefact a été cuit ; 62 `layers` (une révision par calque) et `coverage.deathsPaths` ; 63 la jauge de retour du drapeau (`flagCarries[].spans[].returnProgress`) et les cycles de réapparition des véhicules par emplacement (`vehicleCycles`) ; 64 le camp qui pousse une capture (`zoneStates[].gaugeRamps[].capturingTeam`) ; 65 `stances[]` (accroupi, glissade, action de mobilité) ; 66 le sprint lu dans le film, et `jumpDerived`, calculé sur la vitesse verticale et nommé comme tel ; 67 `rides[].src` = `film` ou `proximity`, avec `coverage.vehicles.ridesRead` / `ridesProximity` ; 68 `mobility` renommé `clamber` après un verdict Theater de 9 sur 9. Les raisons de chaque montée sont écrites dans `film/replay/document_chronicle.go`.
- **Le décodeur en cinq couches tenues par le compilateur, avec des faits de film persistés (ADR 0034, amendé aux jalons M2, M3 et M4)** — `source`, `profile`, `grammar` et `facts` vivent sous `film/internal/` derrière la façade `film/decfilm` (166 symboles, ratchet de surface `archlint/film_facade_surface_test.go`), `film/replay` est la couche de publication ; une porte unique aux octets (`film.Source`) ; plus aucune variable de paquet n'est écrite, si bien que deux films de builds différents se décodent en parallèle sous `-race` ; une révision par couche, chaînée par les valeurs et inscrite dans l'artefact. Le profil est une donnée (`film_profiles.json`, avec les empreintes de registre ; procédure dans `docs/RUNBOOK_FILM_PROFILES.md`), et un film à clé inconnue est mis de côté, compté et journalisé. Les faits d'un film sont persistés (`data/cache/film_facts/{slug}/<short8>.filmfacts.bin`, cinq sections, un en-tête de 110 octets portant les quatre révisions) et la publication rejoue depuis eux quand ils sont frais — identique à l'octet à un décodage sur 10 films, 95 à 442 fois plus rapide ; « à recuire » devient un verdict à trois sorties (à jour / republier / redécoder), et la page d'administration dit ce que les faits occupent.
- **La grammaire de trame lue plus loin (série 5, après le chantier)** — chaque rang de vue est décodé sous la grammaire de sa propre classe (trois classes de vue : les faux `DEL` du pied de trame ne délient plus d'entités vivantes), le rang de vue se lit dans les deux bits de tête d'un identifiant d'image-clé, le lecteur d'image-clé est lu en entier, le bloc de type 1 est la table de datums du chunk, le paquet de type 8 est la liste des joueurs (même corps que la table de `chunk_00`, factorisé), le contrôle de corruption par composant est lu dans le film, et une table anticipée des archétypes est un repli nommé et compté. Sur le film témoin le plus dense (`bfecd02b`), les paquets fermés passent de 888 à 3 919 et les intervalles d'état de mouvement de 616 à 841. Mesuré et écrit plutôt que caché (lot 5.25) : le calque des états de mouvement perd encore 35,4 % des ticks de bipède de joueur sur ce film. Révisions `grammar-2026-09-22.12` et `killsource-2026-09-22.2`.
- **Les états du joueur et des véhicules lus à l'instant** — accroupi, glissade, escalade et sprint sont lus dans le film, par vie et à la milliseconde (le saut n'a pas de champ répliqué : il reste dérivé et le dit) ; le siège d'un occupant de véhicule est lu (`i10 object-parent-state`), la proximité n'est plus qu'un repli qui cède à la lecture ; le cap d'un véhicule est lu dans le film (la vélocité est le repli), un tir de tourelle prend la visée de son tireur, apparié par emplacement ; le camp qui pousse une zone est lu sur un second canal `ti=13`, la déduction survit en repli nommé ; la jauge de retour du drapeau est `ti=13 i1` tag 3.
- **`levelup backfill-killsource` — parallèle, observable, reprenable** — `--workers N` (3 par défaut, 9 au plus face à un pic mesuré de 422 Mio par film ; ×2,7 avec des lignes écrites identiques), un fichier d'état réécrit après chaque film sous `data/global/admin_state/backfill_killsource_<slug>.json` et lu par `--status` sans ouvrir aucune base, une reprise décidée par `decoder_rev` en base, `Ctrl-C` cesse de distribuer, laisse finir les films en vol et sort avec le code 130. La passe de crédit n'est plus quadratique (la vue d'identité est lue une fois par passe, progression tous les 500 matchs). Documenté dans `docs/FR/COMMANDS.md`.
- **Les gates du décodeur** — `replay-corpus-gate` passe à 19 témoins (dont deux à zones) et nomme les changements à part des gains et des pertes ; `replay-equiv` nomme toutes les étapes divergentes et gagne un mode à deux passes (`-deux-passes`) qui prouve qu'un rejeu depuis les faits égale un décodage, sans pouvoir re-figer une référence ; les instruments de recherche lourds passent sous le tag de build `research` (passe unitaire de `filmdec` 89,9 s → 15,0 s), avec `go vet -tags=research` en CI.
...
- **Coordination : la Riposte et l'Appui** (`internal/analysis/coordination`) — une riposte est une mort reprise par son camp sur son tueur dans les **5 secondes**, une seule règle partout (`FenetreEchangeMs`) ; un appui est une assistance entre coéquipiers. Sur l'Escouade : une carte-récit, un anneau des morts ripostées et un histogramme du temps de riposte, un nuage par mort des frags non ripostés (ripostes tardives gardées visibles jusqu'à un plafond de 60 s sur un axe logarithmique, `PlafondRiposteTardiveMs`) et une frise sur tout l'historique des soirées ; sur les Sessions (des deux côtés du tiroir de comparaison, face au repère habituel), sur les Séries temporelles (par soirée, en écart au repère) et sur la fiche du match (par joueur, un graphe par camp).
- **La portée et la hauteur face au lobby** — la portée de frag de tous les joueurs du lobby, par match (`WeaponRangeFilters.AllPlayers`, borné par `MatchIDs`), pour qu'une médiane de joueur se lise face à celle du lobby plutôt que face à la playlist ; des cartes « Rôles de portée » sur l'Escouade et les Séries temporelles, un repère de portée de période sur les Sessions avec la session en surbrillance, des « Rôles de hauteur » sur l'Escouade, et la **hauteur d'engagement** (l'écart de hauteur signé au coup fatal) sur la fiche du match (`combat_tab.elevation`, clic vers l'instant du rejeu) et sur les Séries temporelles en nuage distance × hauteur.
- **Les records de distance par arme** sur la Synthèse — le frag mesuré le plus lointain de chaque arme, les armes écartées nommées dans le (i).
- **Le profil d'armes du Face-à-face** — part des frags par classe d'arme, portée par rôle (P10 / médiane / P90, min / max en infobulle, plancher d'échantillon à 8) et les trois armes de tête avec leurs icônes ; Halo 5 lit ses mécaniques natives.
- **Explorateur** — le briefing montre l'arme favorite de la sélection (une ou deux armes ; le film sur Halo Infinite, les données natives sur Halo 5), un bloc « Portée des frags » du joueur repéré, les assistances échangées avec la cible dans les stats de rencontre, la part des assistances et la répartition des résultats en pistes horizontales, et la section « matchs joués ensemble » sur trois rangées.
- **Les assistances échangées par relation** — une colonne dans les tableaux Relations et la fiche du match, une carte papillon « Binôme » et une carte « Noyau dur » ; les rôles « Larbin / Patron » sur les graphes d'assistances de l'Escouade et de la fiche du match.
- **De nouveaux badges** — « Voleur » dans la matrice d'impact de l'Escouade (le plus de frags volés à un ami : assisté par cet ami, 10 % des dégâts ou moins, ami vivant à ±500 ms), et « Sabordage » / « Abnégation » sur les modes à objectif (l'équipe qui perd au score a dominé aux frags, et son miroir), avec leur couleur propre dans les quatre palettes.
- **La tuile de match de l'accueil** — la part des frags assistés par un coéquipier, par tranche (bornes 25 / 50 %).
- **L'intensité de l'Escouade** — deux courbes de référence, la vraie équipe alliée et le lobby (l'ancienne courbe « Équipe » était le lobby).
- **La fiche du match réorganisée** — un onglet « Armes et terrain » (frags et armes, distance, hauteur d'engagement, usages d'équipement, contrôle des socles, positions du film), le contrôle des armes spéciales en colonnes empilées par socle, la part de chaque équipe en pistes horizontales par famille, l'occupation du terrain agrandie de 20 % et zoomable, les usages d'équipement scindés en deux cartes.
- **Les Séries temporelles et l'Escouade gagnent un onglet « Usages »** (portée, équipement, « les formes retenues ») ; la **balance des dégâts** cumulée arrive dans le Résumé des Séries temporelles ; Impact et Médailles rejoignent Contributions sur l'Escouade ; la page Sessions prend quatre sections (Bilan, Match par match, Frags et usages, Détail des matchs) et une comparaison côte à côte en rangées partagées, avec un espace réservé quand une section n'a pas d'équivalent.
- **Le rejeu** — les objectifs du mode portent leur étage comme les joueurs, avec une bascule « Objectifs du mode » et son aide ; la jauge de retour du drapeau (« Retour N % ») et le cycle de réapparition des véhicules montré sur la carte ; l'état de mouvement (accroupi, glissade, escalade, sprint, saut dérivé) sur la fiche du joueur ; les armes de véhicule ont leur style d'éclair et leur teinte, les véhicules sont dessinés 20 % plus grands avec des sprites redessinés, les marqueurs sont dimensionnés en espace écran (taille réelle quand l'échelle le permet, un minimum garanti sinon) ; le remplissage d'une capture prend la couleur du camp qui pousse ; l'export vidéo rend aux formats standard 1080p / 720p, avec le cadrage au choix quand la carte est zoomée.
- **Des amis par joueur et des invitations sans groupe** — la liste d'amis appartient à un joueur (points d'entrée `GET`/`PUT`, migration au démarrage), une page « Amis et groupes », une section d'invitation en administration et une page « Rejoindre » générique, avec un droit de provisionnement à usage unique épinglé à l'identité du porteur.
- **L'annuaire des joueurs (ADR 0035)** — le xuid comme clé d'identité unique entre profils, comptes, jetons et suivi en direct ; `GET /admin/identities` et une section « Identités » avec un interrupteur « Instance fermée » sur la page Gestion ; `Onboard` est le seul chemin qui crée un profil (ratchet) ; `levelup identity purge` retire une identité sans toucher la base partagée.
- **Un bouton de copie du code d'appareil Xbox**, confirmé au clic.
- **Des primitives d'interface partagées** — une famille de couleurs dédiée aux stats de combat (frags, morts, assistances, assistance donnée / reçue) dans les quatre palettes, des graphes sans cadre dans les cartes de section (plus de double cadre), un titre de section partagé (`DetailSection`) et un helper de titre à infobulle, et un état vide canonique (`EmptyStateNotice`) qui nomme sa cause sur les blocs d'usage, « les formes retenues » et toutes les cartes de graphe.
...
- **Deux citations d'objectif désactivées, deux visuels corrigés** — `flag_steals` (décision utilisateur du 2026-09-10) et `returner_takedown` (décision utilisateur du 2026-09-19 : citation maison — nom et définition inventés le 2026-07-25, seule la colonne `flag_returners_killed` est native — jugée source de confusion et sans intérêt à suivre) partent avec `Enabled: false`, sans `image_path`, toujours listées pour l'inventaire et la parité EN, ignorées par le moteur ; 8 des 10 citations d'objectif de la v7.2.1 restent actives. Le PNG de `flag_captures` portait un damier de transparence cuit dans ses pixels (rendu sur un carré blanc) : fond retiré, alpha réel. `untouchable_carrier` pointait par erreur un visuel d'**arme** Halo 5 (`Éradicateur`) ; Halo 5 n'a aucune citation Oddball, elle sert donc une silhouette de crâne fournie par l'utilisateur, recadrée 100x100 avec une bordure noire de 2,5 px (`HI_citation_Crane_intouchable.png`). Les trois tests `*_Disabled` se replient en une table `TestDisabledCitations`.
- **La synchronisation par le pool pour tout profil suivi** — un profil suivi sans jeton propre est synchronisé par le pool de jetons ; les points d'extension propres au titre sont câblés dans toutes les CLI ; `--token-pool-size` plafonne le nombre d'emplacements sains, pas les sources tentées ; le rejeu d'historique est borné à une page avec un verdict de passe fidèle ; le point d'entrée du rang de carrière est traité comme public (le client du pool n'est plus épinglé) ; les migrations partagées tournent avant chaque synchronisation de CLI.
- **Rien ne tourne pour un compte sans profil suivi (ADR 0035)** — `domain.ProfileGate` sur le coordinateur de synchronisation, le démon du watcher et le SSO ; le verrou d'instance se décide en un seul point (`authz.InstanceLocked`), avec des défauts sûrs en mode appliqué.
- **Les noms de mode des `pair_name` inversés** — « Slayer:Arena », « CTF:Arena Neutral Flag » et leurs semblables donnent un mode, pas un conteneur (une seule liste des jetons de conteneur dans `analysis/modelabel`) ; seeds français « Assassin en duo » et « Assassin tactique ».
- **Les libellés** — les noms d'armes suivent la locale de la requête (fiche du match, précision par arme, armes marquantes, arme favorite de l'accueil) ; la carte des détournements de la Synthèse porte un seul libellé pour tous les titres (« Dépositaire » était le nom de la médaille Reclaimer) ; vocabulaire français : « hauteur d'engagement » au lieu de « dénivelé », « appui » pour une assistance entre coéquipiers, « usages » au lieu de « gestes », avec des garde-rails contre les anciens mots.
- **Le niveau des armes à l'affichage** — les socles de bonus (camouflage, surbouclier) sont des équipements et quittent les sections d'armes, « non identifié » n'a plus de ligne, et l'ordre d'affichage des niveaux est découplé du départage.
- **Une seule grille de carte de chaleur** — Synthèse, Relations, Explorateur et Escouade partagent un rendu de grille catégorielle unique, sans barre de dégradé.
- **La barre de filtres de l'Escouade hors du layout** — son état transitoire vit dans la barre, si bien qu'un clic ne rejoue plus l'animation des graphes (9 → 0 rendus du contenu de la page, garde-rail de frontière).
- **Le Face-à-face nettoyé** — le champ `filters` de `CompareRequest`, jamais rempli, est retiré, ainsi que le repli « échantillon croisé », qui servait des stats de coopération de campagne Halo 5 comme échantillon de matchmaking.
- **Les budgets de CI** — le job Coverage passe de 45 à 60 minutes (annulé à 45 deux fois de suite sur `feat/v75`, 35 à 41 minutes quand il passe, critère de retour écrit dans le workflow) ; le job unitaire du décodeur reçoit 300 s sur le runner Windows.
...
- **`personal_score_awards` perd ses trois derniers index ART** — récidive de DuckDB #23645 sur des insertions ordinaires ; aucun lecteur ne les empruntait (la vue `_latest` balaie de toute façon) ; la sonde `data_health` PSA et `cmd/repair_psa_index` sont supprimées, le ratchet existant étendu.
- **Le provisionnement d'un titre additionnel traite toutes ses bases** et joint les erreurs — Halo 5 était resté sans ses bases partagée et sociale du 12 au 20 septembre.
- **Les anomalies de démarrage** — la purge des libellés `weapon_families` appartient désormais au jeu de migrations Halo 5 (une ERROR de provisionnement depuis le 12 septembre) ; le fournisseur partagé draine ses lecteurs par un compteur et un canal au lieu d'une WaitGroup (deux plantages le 16 septembre) ; la provenance du `RpsTicket` est mesurée au point de passage XBL et persistée (plus de 401 à chaque démarrage) ; l'avertissement de sécurité de configuration ne se déclenche que si l'instance est exposée.
- **Une arme de départ se lit avant la première prise de la vie** — le canal `loadouts` est publié sur une grille globale d'images-clés de 20 secondes, pas à l'apparition ; l'Empaleur quitte les armes de base de `b1ad85eb` (6,85 % → 4,25 %) et 34 matchs du corpus changent d'ensemble d'armes de base, dans le bon sens (Go et le jumeau TypeScript).
- **Le rejeu** — un bot remplaçant rejoint la bonne équipe (plus jamais deux équipes « Cobra »), un châssis inconnu n'est plus du décor (son occupant et ses tirs reviennent), les effets de tir des véhicules se voient, les sons des bases suivent le geste (rampe de capture, captures dédoublonnées, colline par déplacement, tic de score sur l'horloge du jeu), et les anneaux d'étage (joueurs et objectifs) ne se posent plus sur un terrain plat.
- **Les scores d'équipe de `match_registry` reviennent en `INTEGER`** — une dérive de schéma réparée.
- **La visionneuse de médias** — ouvrir le clip suivant n'avorte plus le manifeste de ses voisins (`stopLoad` HLS réservé aux instances démarrées, vignette en affiche).
- **Les sauvegardes** — `data/global` entre dans le périmètre restic : les amis par joueur et les alias Xbox n'étaient pas sauvegardés.
- **Les passes de citations lisent `metadata` par `OpenReadForQuery`** — le rattrapage d'administration tourne sous un handle en lecture-écriture.
- **Les Sessions** — la colonne de gauche rétrécit quand le tiroir de comparaison s'ouvre (une colonne tiroir ouvert, ratchet transverse).

### Ops

- **(1) Recuire les artefacts de rejeu** — les passes (1) à (4) se jouent dans cet ordre, **serveur arrêté** (un seul writer sur la base partagée). `ReplayDocument.SchemaVersion` vaut 68 ; un artefact cuit à un schéma antérieur est servi avec un badge « à recuire » et certains calques restent vides. Lancer `levelup backfill-replay --only-existing` : il ne recuit que les artefacts déjà sur disque, republie depuis les faits de film persistés quand ils sont frais, redécode sinon et écrit les faits au passage (un film = un processus enfant ; **jamais** de cuisson en lot sur des matchs sans artefact, jamais sans accord explicite).
- **(2) Re-projeter les usages d'équipement et de socle** — `levelup backfill-usage-summary`, après la recuisson : il re-résume quand la révision de projection ou le schéma de l'artefact a bougé.
- **(3) Re-projeter le niveau des armes des prises de socle** — `levelup backfill-pad-tiers --force`, après la recuisson lui aussi : il lit les artefacts de rejeu, donc le lancer plus tôt calculerait des niveaux sur des artefacts au schéma 62. `--force` parce qu'une arme de départ se lit désormais avant la première prise de la vie, et que les lignes déjà écrites portent l'ancienne règle, que la reprise ne reviendrait jamais visiter.
- **(4) Recalculer la source des frags** — `levelup backfill-killsource` (100 % hors ligne, depuis le cache local des films, indépendant des trois passes précédentes) : la révision `killsource-2026-09-22.2` a changé les faits, donc chaque match décodé sous une révision antérieure est décodé de nouveau. 3 ouvriers par défaut, `--status` depuis un autre terminal, reprenable en relançant la même commande.
...
- **Re-seeder les correspondances de citations** — `levelup seed citation-mappings` (serveur arrêté : un seul writer sur `metadata.duckdb`) pour que `flag_steals` et `returner_takedown` lisent `enabled = false` et que `untouchable_carrier` / `flag_captures` servent leurs nouveaux visuels. Aucun recalcul nécessaire : la progression stockée est ignorée pour une citation désactivée.
...
- **Indicateur de prolongation (`is_overtime` / `overtime_seconds`)** — aucun champ de l'API ne le porte (vérifié sur 56 payloads réels) : la règle est donc mesurée. `elapsed` = médiane du `time_played` des joueurs présents du début à la fin (repli `MAX`), prolongation si `elapsed > T_reg(variante) + 40 s`. Calibrée sur 1 793 matchs (démonstration sur `bc918a5a` : 990 s jouées pour 720 s de temps réglementaire), elle donne **0 faux positif sur 724 matchs Slayer de contrôle** et signale 761 matchs. Les temps réglementaires sont déclaratifs — `config/titles/{slug}/mappings/regulation.toml`, 9 variantes Arena à 720 s côté Halo Infinite, **vide côté Halo 5** — donc une variante non déclarée n'est jamais signalée (dégradation sûre, zéro comparaison de slug). `internal/analysis/overtime.go` porte le helper pur et la constante documentée `OvertimeMarginSeconds = 40`, avec un garde-rail contre toute redéfinition de la marge ailleurs ; `platform/duckdb/elapsed_seconds.go` est l'estimateur best-effort (WARN structuré avant toute dégradation). Calculé à la lecture : **aucune migration, aucun backfill, rétroactif sur tout l'historique**. Front : `lib/narrative/overtime.ts` comme source unique, `narrative.overtime.*` FR/EN, badge d'en-tête plus une pastille dans l'Explorateur via un `NarrativePill` extrait pour l'occasion (coexistence avec le repère de domination).
- **Graphe `FirstBloodLanes`** — une bande par joueur : nuage des premiers frags au-dessus, des premières morts en dessous, marqueurs de médiane, et une barre de fenêtre étirée entre les deux médianes (interpolation linéaire, valeurs nulles exclues), avec des libellés riches à gauche (joueur, `méd. X → Y`, avance signée) et des bandes triées par médiane de premier frag. Jetons d'issue uniquement, zéro hexadécimal ; une série personnalisée pour la barre de fenêtre (étirée entre deux X, hauteur en pixels) et des nuages de points pour les semis et les médianes. Constructeur pur testé sans locale, manifest partagé `first_blood.toml` (FR/EN), vitrine sur `/lab/charts`.
- **Rotation des journaux par taille** — il n'existait aucun mécanisme à étendre : les fichiers `app.log.1-3` de production sont les restes du `RotatingFileHandler` Python supprimé lors de la migration vers Go, et le README du paquet le supposait déjà. `rotatingWriter` maison (~200 lignes, zéro dépendance — `lumberjack` n'est pas sûr en multi-process) branché sur le writer des catégories du `MultiModuleHandler` : **100 Mo × 3 archives par catégorie** par défaut, pilotable sans redéploiement via `LEVELUP_LOGS_MAX_SIZE_MB` / `LEVELUP_LOGS_MAX_BACKUPS`. Durcissement multi-process : fermeture avant renommage (Windows), détection `SameFile` du perdant d'une course (réouverture au lieu d'un double décalage), temporisation d'une minute après un échec, et garde contre une rotation à chaque ligne. Les archives `.log.N` sont invisibles du visualiseur d'administration et de `logtail` par construction (filtre `.log` existant).
- **Les réponses 429 sont enfin observables** — compteur expvar `rate_limit_429_total` plus un WARN échantillonné toutes les 30 s via `WithLimitHandler` ; `httprate` ne journalisait strictement rien, ce qui est la classe de l'incident des images du 2026-05-06.
- **`/docs` (Stoplight) sur le routeur racine** — gardé par `IsProduction` **ou** `DemoMode` (la démo publique ne pose pas `LEVELUP_ENV`), aux côtés de `securitySchemes.sessionCookie` déclaré au contrat, son nom lu à sa source (`session_platform.CookieName`). Aucune exigence `security` n'est posée : le garde-rail `bare_routes` reste l'autorité sur les routes protégées.
- **Axe de participation Objectifs** *(déployé par anticipation)* — un index par opportunité calculé sur `match_objective_stats_latest`, en remplacement d'une somme PSA diluée par le nombre total de matchs face à des seuils (350×N) jamais calibrés. 7 familles (`ctf`, `zones_koth`, `zones_strongholds`, `oddball`, `stockpile`, `extraction`, `vip`), poids alignés 1:1 sur `awards.toml`, `r = min(1,25 ; 0,65 × actions/P80 + 0,35 × possession/P80)`, agrégation pondérée par `n_f`. Injecté dans la comparaison et le détail de Session, le radar de synergie d'Escouade, le radar de la vue de match et le profil Ascension ; l'axe est retiré quand aucun match à objectif n'est dans le périmètre ou quand la capability est absente (Halo 5), et les coéquipiers non suivis sont couverts (source partagée). Le lecteur `ObjectiveScores` (PSA), devenu mort, est supprimé.
- **Deux lots d'assets rangés par titre** — capacités spartan (11 × 2 variantes plus un index FR/EN, `static/abilities-assets/halo_infinite/`) et grenades (4 types × 2 variantes plus un index, `static/grenades-assets/halo_infinite/`), même convention que `weapons-assets`. Aucune page ne les consomme encore.

### Modifié

- **Premier frag / première mort — un seul noyau, plus de découpage serveur** — le délai était calculé **deux fois** (escouade en tranches de 15 s, séries temporelles en tranches de 10 s) sur la même chaîne `highlight_events` → correction T0 → minimum par (joueur, match, type), et les deux graphes racontaient deux histoires différentes. Noyau unique (`firstEventsAcc` + `ComputeFirstEventsByActor`), valeurs **par match** exposées dans les réponses de page (`first_blood` et `compare_first_blood`), et plus aucun découpage côté serveur. Escouade : le graphe quitte les Contributions pour l'onglet **Dynamique** (après l'Intensité). Séries temporelles : il remplace l'histogramme de 10 s au même emplacement (onglet **Progression**). Sessions : nouveau bloc sous l'Intensité, présent aussi dans le volet de comparaison. Adaptateur partagé `_shared/firstBlood.ts` ; `maxSec` = p99 arrondi à la minute supérieure avec un plancher de 300 s, si bien qu'un match interminable n'écrase plus l'échelle. Ancien code **supprimé** (2 constructeurs Go, 4 types, 2 composants web, i18n orpheline) ; les tests de décalage T0 et de décompte ignoré sont réécrits sur le nouveau constructeur.
- **Les fichiers statiques ne sont plus affamés par la limitation de débit** — la liste des extensions est centralisée dans `middleware.IsStaticAssetPath` (source unique, garde-rail archlint contre une seconde liste, détecteur auto-testé) ; les chemins statiques sont exemptés de la limitation de débit (jamais `/api/*`, sauf `/api/v1/assets`) ; `Cache-Control: public, max-age=3600` sur les fichiers non hachés du dist, `index.html` exclu — un nouveau build est toujours servi.
- **Écriture atomique des réglages d'exécution** — nouveau paquet `atomicfile` (fichier temporaire + renommage, repli sur place borné à `EBUSY` et au fichier temporaire impossible — un seul `Write` puis `fsync`, limite documentée) appliqué aux 3 écritures d'exécution d'`app_settings.json` (`Save`, `SaveTitleOverlay`, `last_notified_version`). Seule la dernière était réellement cassée en conteneur ; les bascules d'administration gagnent l'atomicité qu'elles n'ont jamais eue. Garde-rail archlint contre une écriture nue.
- **La télémétrie fine du writer n'est plus écrasée** — la ventilation `sync_v2_postsync/{weapons,events,psa_aliases}` existait, mais `acquireSharedRW` (`sync_v2_wiring`) écrasait l'étiquette fine juste avant `AcquireWriter` : **100 % des WARN de production tombaient sous l'étiquette grossière**. `ctxkeys.WithDBWriterLabelIfAbsent` posé au seul site fautif, avec des tests de la propriété. Taille de lot, chien de garde et seuil sont délibérément intacts — ils seront resserrés après lecture de la télémétrie en production.
- **Notification de version — démarrage moins bruyant** — `app_release: emit` filtre les comptes `auth_only` de façon déclarative **avant** résolution (5 WARN par redémarrage → 1 `DebugContext` groupé) et déduplique par joueur ; chaque joueur était auparavant résolu une fois par titre, un bug latent.
- **`ApiError.details`** — `oneOf[object, array]` rétabli via le `SchemaTransformer` avec un `additionalProperties` explicite : un `{type: object}` nu génère `Record<string, never>` côté `openapi-typescript`.
- **Mise en page de la vue de match** *(déployée par anticipation)* — le bloc Médias passe en dernier, seul et en pleine largeur ; la grille des frags est portée par `MatchFragCard` avec un prédicat miroir de `FragSunburst` (plus d'« Outils de destruction » orphelin sur un tiers de largeur) ; la rangée de graphes et la rangée médailles/citations deviennent des grilles fluides auto-ajustées sans orphelin ; les vignettes de médias passent en `auto-fill minmax(200px)` au lieu de 5 colonnes de fenêtre (~65 px chacune).
- **`echarts` 5.6.0 → 6.1.0 reporté de nouveau** — l'entrée v7.2.1 annonçait le saut de version pour la v7.3 ; la décision du 2026-07-26 le maintient reporté et `.ai/BACKLOG.md` le consigne comme tel. Écrit ici pour que l'entrée précédente ne reste pas une promesse non tenue.

### Corrigé

- **Prestige — les objectifs cumulatifs ne pouvaient jamais être complétés** — la jauge (`computeCurrentValue`) et la persistance (`evaluateOne`) mesuraient chacune la progression à leur façon : un objectif cumulatif affichait une moyenne et n'atteignait jamais sa cible. Point de mesure unique `evaluateChallengeNow` pour les deux, avec un test de non-divergence qui rend l'écart inexprimable. `CumulativeSince` est une somme SQL non plafonnée bornée à `created_at` : le plafond `WindowMatches = 20` aurait tronqué un objectif « 220 au total ». `MedalEvent` supprimé — aucun écrivain, code mort.
- **Démo — campagnes d'amélioration invisibles** — l'insertion dans `improvement_campaign` utilisait le slug du joueur alors que la lecture filtre sur le XUID : le profil de démonstration n'affichait aucune campagne ; `plan.XUID` est désormais écrit. Le statut `'closed'`, hors énumération, devient `StatusCompleted`, et `CampaignStatus.Valid()` garde l'écriture du seed : la colonne est un `VARCHAR` libre, une valeur invalide produisait donc une invisibilité silencieuse au lieu d'une erreur SQL.
- **Démo — statistiques d'objectif absentes et XUID publiés en clair** — `match_objective_stats` est désormais extraite vers la démo **sans** le marqueur `appendOnly` (la table naît en forme append-only ; basculer ce drapeau produit une Binder Error prouvée). La colonne `xuid` est ajoutée à l'anonymisation universelle du corpus de démonstration — sans quoi l'extraction publiait les vrais XUID sur la démo publique.
- **Tableau des scores cassé par une vue objectifs absente** — le `LEFT JOIN match_objective_stats_latest` quitte Q12 pour une requête isolée (Q12b) chargée en best-effort avec un `slog.WarnContext` avant dégradation : une vue absente donne désormais un tableau des scores servi et une section Objectifs vide, au lieu d'un échec total. Aucune duplication (le SQL de 41 colonnes et le scan Go ont été déplacés, pas copiés). Diagnostic en local : l'hypothèse d'un décalage A/B est **infirmée** (le B-swap opère sur un seul fichier) ; la vraie cause est un snapshot v65 sans le parquet des objectifs, dont le DDL manuscrit n'a jamais été migré. Le repli live est correct et se réarme à la prochaine coupe de snapshot.
- **Halo 5 — la page Réalisations renvoyait 500** — `milestone_catalog` pour `halo_5` n'avait ni `condition_fr` ni `condition_en`, et `CREATE TABLE IF NOT EXISTS` n'ajoute jamais de colonne à une table existante. Colonnes ajoutées au `CREATE` plus un step `ALTER` idempotent pour les bases existantes (un step tracé n'est jamais rejoué), seed aligné sur Halo Infinite, et un commentaire trompeur corrigé. Front : les 3 états de la grille des Réalisations conservent leur titre de section (FR/EN).
- **Les libellés de citations ne suivaient pas la bascule de langue** — `queryKeys.citations` et `commendationTotals` portent désormais la locale (même motif que les médailles) : les libellés préparés par le serveur suivent un changement de langue effectué en cours de session.
- **`headshot_kills` jamais traduit** — la forme snake_case du contrat retombait sur `humanizeMetricKey` ; la clé canonique est ajoutée FR/EN dans `metricLabel` et l'alias `FieldHeadshotKills` est réaligné.
- **Référentiel des champs récupéré deux fois au démarrage** — `useFieldMappings` est gardé par `isBootstrapped`, ce qui ferme la fenêtre d'hydratation pendant laquelle le slug et la locale valent encore leurs valeurs par défaut.
- **Fuite de référence DuckDB à l'arrêt** — `MonitoringStore` est fermé par `ServiceRegistry.Close` : fin du `shutdown_db_leak rw:monitoring.duckdb refCount 1` signalé à chaque arrêt, et le WAL est enfin checkpointé. `SetCronRunSink(nil)` en ceinture, test `DumpCachedLeaks` ajouté.
- **Un panic pouvait retenir le writer RW jusqu'au redémarrage suivant** — les 4 libérations non `defer` du chemin d'écriture partagé (rafales d'événements, d'armes et de `psa_aliases`, plus `combined_persister`) passent sous une fermeture `defer`. Un panic d'analyse de film rattrapé par le recover global laissait auparavant le writer retenu. Le moment nominal de libération est strictement inchangé.
- **Images de production dépouillées des assets `public/`** *(déployé par anticipation)* — `.dockerignore` ré-inclut `apps/web/public/` ; tous les PNG/WebP étaient retirés de l'image de production (logo Waypoint des tableaux de matchs, `logo.png`, `og-default.png`, emblèmes Halo 5). Un asset à extension connue renvoie désormais un 404 franc au lieu du 200 silencieux de repli SPA sur `index.html`, un garde-rail `verify-public-in-dist.mjs` s'exécute au stage `web-builder` du Dockerfile, et les dépendances `useMemo` sur `currentTitleSlug` sont corrigées dans `ExplorerMatchesTable` et `SquadSynergyHistoryTable`.
- **Bandeau « match incomplet » sur tous les matchs du snapshot** *(déployé par anticipation)* — régression de la v7.2 : Q12 joignait `match_objective_stats_latest`, absente du snapshot partagé → Catalog Error → `scoreboard_empty` → bandeau affiché sur des données parfaitement saines. La table est désormais exportée dans la coupe et la vue `_latest` recréée à la lecture, son DDL centralisé (`internal/migration/objective_stats_view.go`, 5 sites migrés) avec un garde-rail archlint contre les copies et un test de contrat du schéma de snapshot (16 relations) ; un snapshot plus ancien se soigne tout seul via `ErrSnapshotIncomplete` et un repli live propre.

### Ops

**Actions post-déploiement (requises)**

- **Backfill des objectifs à rejouer** — le backfill côté serveur a été interrompu avant la fusion de cette branche, et les références P80 de l'axe de participation Objectifs sont mesurées pour `ctf`, `strongholds` et `koth` mais **provisoires** pour `oddball`, `stockpile`, `extraction` et `vip`. Séquence : réinitialisation du bit `MBitObjectiveStats` (SQL exact dans le commentaire de tête de `cmd/backfill_objective_stats/main.go`), backfill, puis re-mesure des P80 provisoires.
- **Régénération de la démo requise** — les statistiques d'objectif et l'anonymisation des XUID n'atteignent la démo publique que par une nouvelle coupe, et les campagnes d'amélioration ont besoin de la même régénération pour apparaître.
- **Stock de journaux de production à purger** — la rotation ne s'applique qu'à partir de ses propres écritures. Les fichiers historiques `app.log.1-3` (restes du gestionnaire Python) se tronquent **sur place**, jamais par suppression : un descripteur de fichier ouvert survit à un `rm`.
- **L'`ALTER` de `milestone_catalog` s'exécute au démarrage** — un step DDL idempotent sur les bases `halo_5` existantes au premier démarrage suivant le déploiement. Aucune donnée n'est perdue.

**Divers**

- **Aucune migration ni backfill pour l'indicateur de prolongation** — il est calculé à la lecture et s'applique donc rétroactivement à tout l'historique dès le déploiement de la version.
- **La rotation des journaux se pilote sans redéploiement** — `LEVELUP_LOGS_MAX_SIZE_MB` (défaut 100) et `LEVELUP_LOGS_MAX_BACKUPS` (défaut 3), par catégorie.
- **`/docs` n'est exposé ni en production ni sur la démo publique** — la garde couvre les deux, démo comprise, qui ne pose pas `LEVELUP_ENV`.

## [7.2.5] - 2026-07-26

> Version corrective fermant la traîne opérationnelle de la v7.2.1 : le pipeline de déploiement de la démo, le logo Waypoint qui s'affichait sous Chrome mais pas sous Firefox, et deux commentaires de documentation inversés. Aucun changement de schéma, aucune migration, aucune action post-déploiement.

### Corrigé

- **Logo Waypoint invisible dans les tableaux de matchs** — trois causes distinctes, dont une seule propre à un navigateur. (1) Le lien était un conteneur flex sans dimension propre, dans une cellule de tableau en largeur automatique ; Chrome et Firefox ne résolvent pas la taille intrinsèque de la même façon dans ce cas, et Firefox écrasait l'image à zéro. Le lien porte désormais une taille fixe : plus rien ne se négocie. (2) Les attributs HTML `width`/`height` manquaient, laissant le moteur sans taille avant la résolution du CSS — c'est le point qui comptait pour Firefox. (3) L'asset est un logotype 360×160 forcé dans un carré de 16×16 sans `object-fit` : le `fill` par défaut l'écrasait en traits sub-pixel sur **tous** les navigateurs, y compris ceux où il apparaissait, ce qui explique que ce volet n'ait jamais été signalé. `object-contain` rétabli, boîte à 20×16, et largeur réservée par colonne d'icône (`w-9`) plutôt qu'un élargissement du padding commun, qui aurait décollé tout le tableau de ses bordures. Correctif identique sur `ExplorerMatchesTable.tsx` et son jumeau `SquadSynergyHistoryTable.tsx` — les deux alimentent aussi les pages Session, Timeseries et Carrière.
- **Jalons de la démo bloqués à 0 (pipeline de déploiement)** — `insertDemoMilestonesEarned` attache la metadata démo en lecture seule pour dériver les jalons du vrai catalogue, mais le job de régénération n'arrêtait que le service de prod ; le conteneur de la démo gardait le fichier verrouillé et l'étape échouait en best-effort (`Conflicting lock is held in PID 0`). Le job stoppe désormais `levelup-demo` aussi, avec un piège `EXIT/INT/TERM/HUP` qui remet les deux services en service quoi qu'il arrive — une régénération qui échoue en cours de route ne peut plus laisser la démo à terre. Les jalons passent de 0 à 6 sur les deux titres. La publication des DuckDB de la démo est maintenant atomique (échange d'inode) avec gestion du WAL, de sorte qu'un lecteur n'observe jamais une base à moitié écrite.
- **Documentation inversée** — `discovery_client.go` affirmait que l'API de découverte UGC ne nécessite pas d'authentification Spartan ; elle renvoie 401 aujourd'hui (mesuré en anonyme le 2026-07-25). Le commentaire décrivait un état révolu de l'API et induisait en erreur quiconque écrivait un nouvel appel. À côté, la voie anonyme qui fonctionne réellement est désormais documentée : le chemin blob des variantes de carte, qui a permis de récupérer 120 cartes sur 123 sans authentification. Le commentaire de la capability Waypoint prétendant que Halo 5 ne déclare pas `waypoint_match_url` est corrigé lui aussi — c'est le cas depuis le 2026-07-24, et c'est ce qui a produit un faux diagnostic — et la mention désormais périmée de Halo 5 est retirée du libellé d'aide de la colonne, dans les deux langues.

### Ajouté

- **Garde-rail de parité des capabilities** — `capabilities_parity_test.go` et `capabilities_parity_scan_test.go` : la liste des capabilities existait en trois endroits (le descripteur Go, une liste de validation, une liste côté front) sans rien qui les relie, ce qui est précisément ce qui a laissé les commentaires diverger. Mordant prouvé par mutation : retirer `CapWaypointMatchURL` de `knownCapabilities` faisait disparaître le titre Halo 5 entier du registre. Le vrai trou était `knownCapabilities` — le miroir Go↔TS existait déjà depuis la v7.2.
- **Tests d'intégration du seed de la démo** — publication atomique, récupération du WAL et échange d'inode couverts (`seed_demo_integration_test.go`, `seed_demo_wal_test.go`).

## [7.2.1] - 2026-07-25

> Version corrective consolidant le « backlog Notion v7.2.1 » (items V721-01 à V721-15, branche `feat/v7.2.1-notion-batch`) : trois modes à objectif de plus collectés de bout en bout, dix nouvelles citations d'objectif, ventilation par saison de l'Explorer complétée, vivier de défis d'escouade réparé, 35 routes ajoutées au contrat publié, retrait de la structure `arc_titles`, et sortie de la version applicative du build. Résumé par domaine. **Des actions post-déploiement sont requises — voir *Ops*.**

### Ajouté (API Go)

- **Stats objectifs — Stockpile, Extraction et VIP (V721-02)** — mapping figé sur 10 payloads réels `GetMatchStats` : `StockpileStats` (6 champs, 2 durées ISO 8601), `ExtractionStats` (5 champs, aucune durée) et `VipStats` (7 champs, 2 durées) — ce dernier était **réel mais non déclaré dans `StatsBundle`** (`internal/openspartan/halo_api_payload.go`) et y a été ajouté. Step de migration SÉPARÉ `shared_objective_stats_add_stockpile_extraction` (`TargetShared`) : 18 `ALTER TABLE match_objective_stats ADD COLUMN IF NOT EXISTS` nullables **plus `CREATE OR REPLACE VIEW match_objective_stats_latest` dans le MÊME step** — DuckDB fige la liste de colonnes d'une vue à sa création, sans quoi les 18 colonnes seraient invisibles pour TOUS les lecteurs. Le `CREATE` existant n'est pas édité (migrations keyées par nom : les bases déjà migrées ne le rejoueraient jamais). `ObjectiveStatsInsert` gagne 18 champs pointeurs, `buildObjectiveRow` les 3 blocs, et `persistObjectiveStats` passe à 43 colonnes / 43 emplacements / 43 arguments — toujours un **INSERT pur**, sans `ON CONFLICT` ni `INSERT OR REPLACE` ; les 4 durées passent par `objectiveDurationSeconds` (fractions préservées). Chaîne de lecture étendue en parallèle (`ObjectiveRaw`, Q12, scan du scoreboard, `buildScoreboardObjective`, `MatchScoreboardObjective`). Fixtures anonymisées sous `internal/sync/objective/testdata/objective_stats/`. `ALTER ADD COLUMN` est du DDL pur, hors périmètre ART #23645 ; aucun garde-rail ART touché, aucune nouvelle capability (`CapMatchObjectiveStats` / `CapObjectiveStats` sont déjà génériques à la table ; Halo 5 reste `not_exposed`, la table reste simplement vide pour ce titre).
- **Elimination et Infection délibérément NON spécifiés (V721-02)** — aucun payload du corpus ne porte ces blocs : les 2 matchs Arena:Attrition (`GameVariantCategory` 7) n'exposent que `CoreStats`, au niveau joueur comme au niveau équipe, et le seul candidat Infection est un Firefight UGC (`GameVariantCategory` 41). Aucun schéma n'a été déduit par analogie. Condition de reprise : un match matchmaking d'un de ces modes joué puis synchronisé, puis réplication du même patron (1 migration ALTER, 1 bloc dans `buildObjectiveRow`, 1 jeu de colonnes UI).
- **Dix citations d'objectif (V721-03)** — `flag_captures`, `flag_secures`, `flag_steals`, `returner_takedown`, `unstoppable_carrier`, `aggressive_return`, `zone_defense`, `untouchable_carrier`, `skull_carrier_takedown` et `skull_grabs` amorcées dans `defaultCitationMappings()` (`seed_citation_data.go`) avec 8 constantes de paliers calibrées sur `match_objective_stats_latest` (joueur de référence : 304 matchs à objectifs) plutôt que sur l'échelle générique, qui en aurait rendu quatre littéralement inatteignables. Parité EN complète (`citationDisplayEN` **et** `citationDescriptionEN`, 98/98, zéro orphelin) et quatre garde-rails : unicité des norms, existence sur disque de tout `image_path`, couverture EN du nom affiché, et une fixture figeant les 10 colonnes sources et leurs paliers. Le registre porte désormais **98** citations.
- **Explorer — ventilation par saison complète (V721-06)** — `deterministicSeasonPath` (id de catalogue `^season(\d+)$` → `Seasons/Season{N}.json`, surcharge `extra.matchmade_path` pour l'opération intra-saison) et `seasonCMSPaths` (union dédupliquée, chemin déterministe en tête) dans `explorer_target_seasons.go`. `computeSeasonBreakdown` ne tente plus uniquement les saisons présentes dans `rec.SeasonIDs` — la cause du « 5 sur 14 » diagnostiqué. `domain.SeasonMatchCount.Unresolved` (optionnel, `omitempty`) plus un statut de section (`ok` / `local_partial` en résolution partielle / `failed` si aucune saison ne résout) distinguent « saison non jouée » de « saison non remontée ». Le plafond de concurrence (`seasonBreakdownConcurrency = 6`) est inchangé et le fan-out CSR — le plus coûteux, un appel par playlist classée engagée — n'est désormais demandé que pour les saisons à `matches > 0` : budget CSR identique à avant.
- **Garde-rails** — `no_cross_title_aggregation_test.go` (scan AST, 4 détecteurs, liste d'autorisation datée, morsure prouvée sur le code exact retiré ci-dessous) ; `bare_db_recovery_routing_test.go` (interdit tout accès plat sur un champ NU `db *duckdb.DB` dans `platform/duckdb/prestige/` — le trou par lequel les deux incidents de reprise sont passés, invisible à `player_db_recovery_routing_test.go` qui ne voit que les formes `pdb.Player.Query(`) ; `default_status_ratchet_test.go` ; `TestSharedObjectiveStatsStockpileExtractionColumns` (18 colonnes, nullabilité, famille de type, aller-retour **par la vue `_latest`**).

### Ajouté (React / TypeScript)

- **Trois jeux de colonnes d'objectif** — `ObjectiveMode` gagne `stockpile` | `extraction` | `vip` ; `detectObjectiveMode` et `objectiveColsFor` étendus dans `MatchScoreboard.logic.ts` : 4 colonnes Stockpile, 4 Extraction, 5 VIP sur les 18 champs exposés (les 5 restants demeurent dans le DTO sans colonne dédiée, même proportion que la CTF, qui en expose 11 et en affiche 4). `longest_time_as_vip_seconds` s'agrège en `max`, jamais en somme (même règle qu'Oddball). 13 clés de libellé/infobulle ajoutées dans les **deux** locales dans `match-view/i18n.ts`.
- **`lib/charts/avgLife.ts`** — l'unique helper de durée de vie moyenne côté front (valeur réelle, repli sur le proxy), et `lib/narrative/dominance.ts` comme source unique des jetons de domination.
- **Outillage de revue** — `ReviewBadge` + `lib/review`, inerte hors tournée de revue, câblé par une propriété `reviewKey` sur les graphes en revue.

### Modifié

- **Contrat publié complété (V721-04)** — les quatre portes du harnais de rendu (`BuildDemoRouter`) ont été assouplies pour monter Prestige, catalogue, diagnostics de synchronisation automatique et métadonnées d'assets : **+38 opérations, −3, soit 35 routes nouvellement publiées** dans `api/openapi.yaml`. `EmptyCatalogRepo` (repli démo, même parti pris qu'`EmptyAssetMetadataHandler`) est injecté dans le **même** `CatalogHandler` : les 3 routes catalogue ne sont déclarées qu'une fois, il n'y a pas de handler jumeau à re-synchroniser, et il ne sert jamais hors mode démo. Un **seul harnais** a été conservé délibérément — un second rouvrirait le canal de dérive fermé en v7.2.
- **Statuts de réponse nominaux (V721-04)** — `humacore.DefaultStatus` posé sur **23 routes** (11 × 201 Created, 12 × 202 Accepted). Le statut est déclaré une fois au montage et fait source unique pour le runtime ET le document ; `default_status_ratchet_test.go` le verrouille. `api/openapi_manual_fragment.yaml` diminue d'autant.
- **La version applicative sort du build (V721-15)** — `ARG VERSION` et `-X main.version` retirés du `Dockerfile`, `build.args` retirés des deux services de `docker-compose.yml`. La version transite désormais **uniquement** par l'environnement d'exécution (`LEVELUP_APP_VERSION`), que `scripts/deploy.sh` persiste dans `.env` — docker compose le lit donc pour TOUS les services et à TOUTE commande (`up`, `--force-recreate`, redémarrage, session ssh manuelle), ce qui couvre le cas ayant motivé le bakage à l'origine (environnement perdu au recreate de la régénération démo). Deux conséquences : taguer une version n'invalide plus la couche de compilation CGO, et les images production et démo ne peuvent plus diverger — cause des deux compilations CGO parallèles qui ont épuisé la mémoire du VPS au déploiement de la v7.2.0. La divergence devient impossible par construction au lieu d'être évitée en alignant deux valeurs à la main. Le service démo affiche la vraie version au lieu de `dev`. `main.version` reste un repli utile pour un `go build` local avec ldflags.
- **Dependabot (V721-01)** — surcharge `js-yaml` relevée à `^4.3.0`, surcharges `brace-expansion` ajoutées (`^5.0.7`, et `^2.1.2` cantonnée sous `@redocly/openapi-core` → `minimatch`) : les trois alertes `high` sont fermées. Toutes trois sont des dénis de service dans de l'outillage de build jamais chargé par le navigateur, et les trois correctifs sont des correctifs de patch. **`echarts` 5.6.0 → 6.1.0 délibérément reporté à la v7.3** (entrée de backlog datée, critère : suite de graphes verte + tournée visuelle) — un saut de version majeure sur le moteur de TOUS les graphes de l'app n'a pas sa place dans une version corrective.
- **Lectures Prestige routées vers la reprise** — `prestige_metadata_repo.go` et les cinq lectures mono-ligne plates de `prestige_social_repo.go` (`GetUserPrestige`, `GetUserPrestigeCrossTitle`, `SquadRepo.Get`, `SquadChallengeRepo.Get`, `CountActiveParticipants`) passent aux variantes `*Recovered` ; la contention fichier est traduite en `dblease.ErrDBLocked` par `translateSocialLockErr` (`prestige_social_recovery.go`, 2e copie du patron — centralisation et garde-rail à la 3e).
- **Vivier de défis d'escouade** — `squadEligibleTemplates` écarte les templates `eval_type=threshold`, dont la règle affichée ne correspond pas à l'évaluation réellement pratiquée (cumulative pour tous en V1). Un ensemble éligible vide dégrade en `200 []` avec un log structuré distinguant un catalogue vide d'un catalogue entièrement filtré ; aucune comparaison de slug introduite.
- **Durée de vie moyenne** — `avg_life_seconds` (valeur native de l'API) exposée et préférée au proxy `temps joué / (morts + 1)`. `buildCorrelationPoints` en était un **3e** consommateur, avec son propre proxy calculé en dur — l'histogramme et le nuage racontaient deux histoires de la même métrique — et est rebasé sur le helper partagé `matchAvgLifeSeconds`, avec la même télémétrie Debug (replis / utilisés / total). Il est extrait vers `timeseries_service_correlation.go` (l'ajout de la télémétrie portait `timeseries_service_tabs.go` à 517 lignes, au-delà du seuil de 500) ; le commentaire du champ `LifeBuckets` (`domain/timeseries.go`) est corrigé en cohérence.
- **Taux de victoire et MMR sur des axes séparés** — Y2 devient le taux de victoire borné [0,100] avec un formateur `%`, Y3 porte le MMR (`offset: 48`, ajouté seulement quand le titre le fournit, donc aucun axe fantôme sur Halo 5) ; la marge droite de la grille ne s'élargit que quand l'axe MMR est présent. Les deux partageaient un Y2 auto-échelonné sur lequel le MMR écrasait la courbe de victoire sur la ligne du bas. Le graphe d'escouade était déjà correct et n'est pas touché.
- **Radar de synergie** — la valeur brute est affichée à côté du score normalisé 0-100 dans l'infobulle (`SquadSynergyRadarAxis.raw` était déjà porté par le DTO), avec une précision adaptée à l'ordre de grandeur de chaque axe plutôt qu'un arrondi unique qui écraserait les axes ratio.
- **Domination sur la bande de résultats** — `dominance_flag` ajouté aux deux DTOs concernés (`omitempty`, le 0 disparaît du JSON) et rendu en repère sur `OutcomeSequenceTape`.
- **Documentation** — ADR 0014 §6.1 amendé : le signal coach « soft-négatif » était livré alors que l'ADR annonçait encore « alertes positives uniquement » (doc inversée), ses garde-fous sont désormais écrits (seuil −0,10 sur ≥ 14 jours, catégorie neutre, jamais `outcome-loss`). `docs/COMMENDATIONS_REFERENCE.md` et son renvoi FR mis à jour (88 → 98 citations, table « PvP — Game mode » de 11 à 21 lignes, dont 4 corrigées d'`award` en `objective_stat`). `internal/openspartan/doc.go` pointe désormais vers `docs/ARCHITECTURE_V6.md`, où la fonctionnalité d'import OpenSpartan est documentée pour la première fois hors du code — la vraie cause de la question récurrente « pourquoi un dossier openspartan ? » (743 occurrences, zéro code mort, protégé par `RequireAuth`, 73 % de couverture). L'objet de chaînes de `CoachFocusCard.tsx` est migré vers le manifest i18n `profile.coach.*` (ADR 0003), dernier composant coach avec un objet `STR` en dur.
- **Plans clos et archivés** — `PLAN_COACH_V3_GENERATION.md` (livré à ~95 % le 2026-06-09, encore affiché « Proposé », avec des références de lignes périmées) et `PLAN_CROSS_TITLE_ARCS_2026-07.md` déplacés dans `.ai/archive/` avec un journal d'exécution corrigé ; `.ai/BACKLOG.md` trié (4 entrées livrées retirées, l'entrée `seedWeapons` corrigée — `weapon_labels.name_fr` est un **3e maillon VIVANT** du `COALESCE` du résolveur, pas de la plomberie vestigiale — et une entrée `echarts` 5→6 datée ajoutée).

### Corrigé

- **Sécurité — trois routes catalogue montées sans garde d'authentification** — `/titles/{slug}/catalog/{playlists,pairs,maps}` étaient hors de toute garde tout en acceptant `?xuid=` et `only_played`, ce qui laissait fuiter l'activité d'un joueur sans authentification. `RequireAuth` appliqué sur les **deux** branches (nominale et repli démo) ; aucun appelant front n'était concerné, vérifié. Le garde-rail ne les avait jamais vues parce que le harnais de démo ne les montait pas — c'est leur montage pour V721-04 qui a révélé le trou.
- **`GET /prestige/me` sans `title_slug` renvoyait 500** — `GetUserPrestigeCrossTitle` scannait `MAX(updated_at)` (un agrégat, donc NULL sur un ensemble vide) directement dans `prestige.UserPrestige.UpdatedAt` (`time.Time`) ; aucune sentinelle n'était reconnue par `serviceError`, la branche par défaut répondait donc 500. Scanné en `sql.NullTime` puis converti : un joueur sans PP obtient un prestige vide, même contrat que `GetUserPrestige`. L'audit des autres agrégats du fichier (`COALESCE(SUM(...), 0)`, `COUNT(*)`) n'a révélé aucun second site à risque.
- **Le vivier de défis d'escouade renvoyait un 500 permanent** — `prestige_metadata_repo.go` utilisait un `r.db.Query` plat : `dblease.ErrDBLocked` n'était jamais émis et le mapping 503 du handler ne se déclenchait donc jamais ; une seule invalidation concurrente laissait `POST /squads/{id}/challenges/pool/refresh` en erreur jusqu'au redémarrage suivant.
- **Rejoindre ou abandonner un défi d'escouade répondait 400 au lieu de 503** — la chaîne d'erreur était ré-emballée en `ErrInvalidInput` ; la cause est désormais conservée par un 2e `%w`.
- **Commentaire de `stats_canonical.go`** — il affirmait qu'`OffensiveConversion` / `DefensiveResistance` « restent nil » alors qu'ils sont calculés par `ComputeCombatYield` ; seul `MedalExploitScore` reste effectivement nil dans ce converter.
- **Bloc de doc mal placé dans `seed.go`** — il documentait `defaultCitationMappings` tout en surmontant `SeedMedalDefinitions`, dont il couvrait le `//nolint` par accident ; déplacé au-dessus de la fonction qu'il décrit, dans `seed_citation_data.go`.

### Ops

**Actions post-déploiement (requises)**

- **Backfill des objectifs, précédé d'une réinitialisation ciblée** — `markObjectiveDone` pose le bit `MBitObjectiveStats` pour TOUT match récupéré, y compris ceux n'ayant produit aucune ligne — le cas de tous les matchs Stockpile / Extraction / VIP sous la v7.2 — et `listCandidateMatchIDs` les exclut donc. Le SQL exact de réinitialisation figure dans le commentaire de tête de `cmd/backfill_objective_stats/main.go`. Les matchs synchronisés après le déploiement n'en ont pas besoin : la synchronisation native ne pose jamais ce bit.
- **Re-seed et recalcul complet des citations** — `levelup seed citation-mappings` (attendu : 10 insérées, 88 mises à jour) puis `levelup backfill --all --citations-recompute-all`. Le chemin incrémental `--citations` ne suffit **pas** : les matchs déjà traités portent une ligne sentinelle et ne seraient pas rejoués, ce qui laisserait les dix nouvelles citations à zéro sur tout l'historique. Les deux écrivent dans DuckDB — arrêter le serveur avant (un seul process writer par base, ADR 0013/0016).
- **`drop_arc_titles` s'exécute au démarrage sur toutes les bases joueur** — aucune donnée n'est perdue (la table était un miroir 1:1 d'`arc.title_slug`), mais le DDL s'exécute au premier démarrage suivant le déploiement.

**Divers**

- **Deux des dix nouvelles citations partent avec un visuel provisoire fait en interne** — `flag_captures` et `flag_steals` pointent des SVG bouche-trous. Le remplacement consiste à déposer les PNG définitifs dans `static/commendations/halo_infinite/`, à basculer `.svg` → `.png` dans les deux `ImagePath` et à re-seeder ; `TestCitationImagePaths_ExistOnDisk` échoue si l'extension bascule avant l'arrivée des fichiers.
- **La version applicative vient désormais du seul environnement d'exécution** — voir *Modifié* ; `scripts/deploy.sh` l'écrit dans `.env`.

### Supprimé

- **`arc_titles` (V721-09)** — la capacité d'arc multi-titres est retirée : step de migration `drop_arc_titles` (l'entrée historique `create_arc_titles_join` est conservée — c'est un enregistrement de `schema_migrations`, pas du code mort exécutable — avec une dépendance déclarée créateur → suppresseur et son `ApplyBackfill` retiré), `ArcTitlesRepo` et son implémentation, l'invariant `Create`, `creditTitlesFor` (les appelants retombent sur `[challenge.TitleSlug]`), `internal/prestige/cross_title.go` et les trois fichiers de tests dédiés. `order_audit_test.go` affirme désormais que la table est **absente**, et un garde-rail de Phase 2 interdit de réintroduire une agrégation entre titres. La règle et sa justification sont consignées dans le skill `arch-rules`.
- **`PrestigeRepo.GetLeaderboard(titleSlug=nil)`** — la branche entre titres n'avait aucun appelant de production (la page « Leaderboard PP » du front est un emplacement réservé qui ne récupère rien) et était de toute façon interdite par le garde-rail. Le paramètre passe de `*string` à `string` : la voie entre titres n'est plus exprimable à la compilation, et un slug vide est refusé par `prestige.ErrInvalidInput` au lieu d'être dégradé en « tous titres ».
- **`CountActiveParticipants`** — aucun appelant de production : une déclaration d'interface, une implémentation, deux bouchons de test et une étape du test de reprise retirés.
- **Colonne `weapons.name_fr` (V721-05)** — retirée du `CREATE` du registre dès la V72-06 et lue par plus personne depuis, la colonne subsistait pourtant sur les `metadata.duckdb` déjà migrées : DuckDB refuse `ALTER … DROP COLUMN` tant que la clé primaire ART (`title_slug` + `weapon_key`) existe. Nouveau step `purge_weapons_name_fr_column` (reconstruction CTAS-swap, `TargetMetadata`), ordonné après `add_weapon_registry` avec la dépendance déclarée dans `order_dependency_test.go`. Idempotent (sans effet quand la colonne a déjà disparu, quand la table est absente, et à la seconde exécution), protégé contre la perte (nombres de lignes comparés dans la transaction avant tout `DROP`), clé primaire composite recréée après le renommage. Le commentaire de non-DROP de `weapon_registry.go` pointe désormais vers la migration au lieu du backlog. **À ne pas confondre avec `weapon_labels.name_fr`**, bien vivante — c'est le 3e maillon du `COALESCE` du nom d'affichage, le filet de sécurité des identifiants d'armes qui n'ont pas encore de `weapon_key` dans le registre.

## [7.2.0] - 2026-07-25

> Version d'ajustement consolidant le « backlog Notion v7.2 » (items V72-01 à V72-34, branche `feat/v7.2-notion-batch`) : statistiques des modes à objectif de bout en bout, `api/openapi.yaml` désormais **généré** par Huma, éradication des fuites entre titres, lectures live Explorer via le vivier de jetons, fiabilisation des alertes Discord, et une vingtaine de correctifs UI/i18n. Résumé par domaine. **Des actions post-déploiement sont requises — voir *Ops*.**

### Ajouté (React / TypeScript)

- **Stats objectifs — 4 surfaces** — `MatchObjectivesSection` (section de scoreboard par équipe, une colonne par statistique du mode joué, ligne « Total équipe » agrégée à la lecture), bloc `objective_stats` de la Synthèse, `SquadObjectiveStatsPanel` (cumul escouade) et `TimeseriesObjectiveCard` (KPI de scope, délibérément préféré à des champs par match `omitempty`). Toutes doublement gatées (capability de titre `objective_stats` + data-driven), zéro code mort ; Halo 5 ne déclare pas la capability, les sections restent donc masquées.
- **Explorer — badges d'état live** — `ExplorerLiveStatus` (`ok` / `failed` / `no_auth` / `local_partial`) surfacé sur les 5 fetchs live, avec un badge discret par section (fin de la dégradation muette, plan A3).
- **Recherche gamertag — interrogation Xbox explicite** — `useGamertagSuggestions` gagne `liveResults` / `isLiveLoading` / `liveAttempted` / `triggerLiveSearch` ; bouton « Rechercher sur Xbox » dans `GamertagSearchInput` (Explorer) et `GamertagCombobox` (générique), i18n dans `common.toml` ; plus de résultats live périmés et état explicite « aucun résultat Xbox ».
- **Admin — synchronisation initiale d'un joueur** — carte sur `AdminSyncPage` (`GamertagCombobox` max=1, plafond optionnel, `AdminActionButton` suivi inline, `useRunInitialSync`, pastille du titre actif) plus une légende `SyncActionsHelp` documentant la portée des quatre actions de synchronisation (cycle forcé / manuelle globale / convergence / initiale).
- **Infobulles d'en-têtes de tableaux** — `SortableTh.tooltip` + `ColumnMeta.headerTooltip`, les deux familles de tableaux couvertes, contenu FR/EN aligné sur l'ADR 0006.
- **Légendes en pied de bloc** — `ChartCard.legend` + nouveau composant `ChartLegend` ; blocs informatifs migrés (les légendes interactives sont documentées comme restant sur le canvas).
- **Badge « En placement »** — variante `perf` de `PlacementPendingCell` sur Perf / ΔPerf dans les tableaux et sur les tuiles d'accueil, alimentée par les nouveaux champs `perf_placement_done/total` ; la colonne Note garde `placement_*`, et aucun zéro n'est jamais fabriqué.
- **Sessions** — graphe `SessionCareerXP` (XP par match + XP cumulée, miroir des Séries temporelles, gate data-driven) et bouton « Voir les synergies » dans l'en-tête L3 d'une session d'escouade.
- **Menu Escouade** — l'entrée L1 « Dynamique » manquante (`navL1Sections.tsx` + `common.nav.tab_dynamique`) ; la page, la route et l'i18n existaient déjà.
- **Match non synchronisé** — écran `PageUnavailable` dédié sur `match_not_found` (FR/EN), en remplacement de l'écran d'erreur générique.
- **Garde-rails de contrat** — `contract-surface.guard` (snapshot statique de `generated.ts` : 176 chemins / 522 schémas / 30 enums, disparition = rouge, morsure prouvée par mutation), `response-types.guard` durci (274 citations résolues, 20 réponses critiques ré-exportées), et un garde vitest verrouillant `generated.ts` sur `openapi.yaml` en CI.

### Ajouté (API Go)

- **Stats objectifs (V72-03)** — mapping figé sur 8 payloads réels `GetMatchStats` (CTF 11 champs, Zones 6 partagés Strongholds/KOTH, Oddball 6 ; KOTH = `ZonesStats` confirmé, colline discriminée par `StrongholdScoringTicks > 0`) ; table `match_objective_stats` créée **directement append-only** (séquence + `written_at` + vue `_latest` + index `match_id`, garde-rails ART étendus) ; `ExtractObjectiveStats` pur + `persistObjectiveStats` INSERT-only dans la transaction partagée, peuplé par `engine_fetch` et `engine_v2bridge` ; extracteur isolé dans le sous-package `sync/objective` (ratchet god-package). Fixtures anonymisées sous `internal/sync/testdata/objective_stats/`.
- **Capabilities objectifs — deux axes** — `CapMatchObjectiveStats = "match.objective.stats"` (axe serveur, gouverne le chemin de données ; `halo_infinite = supported`, `halo_5 = not_exposed`) et `CapObjectiveStats = "objective_stats"` (axe titre, gouverne l'affichage, miroité en TS `TITLE_CAPABILITIES` avec son garde-rail de parité).
- **CLI de backfill objectifs** — `cmd/backfill_objective_stats`, append-only via `persist.InsertObjectiveStats` ; candidats = matchs sans le bit `MBitObjectiveStats` (1<<23) et sans ligne `_latest`, bit posé systématiquement. Passage local : 5 656 lignes.
- **Citations d'objectif** — nouveau type de mapping `objective_stat` + loader `loadObjectiveStats` lisant `match_objective_stats_latest` (23 colonnes mappées, best-effort intégral) ; quatre citations basculées des awards vers les compteurs : `charge`→`zone_captures`, `got_you`→`flag_returns`, `stakeholder`→`zone_secures`, `flag_carrier_hunter`→`flag_carriers_killed`.
- **Live Explorer via le vivier de jetons** — ordre de résolution session fraîche stricte → profil → **vivier** (`PolicyAnyPublic`), WARN structuré + compteurs expvar ; validé de bout en bout sur une cible dont le jeton de rafraîchissement est mort (carrière, identité, médailles et saisons servies à l'identique d'un jeton sain).
- **Synchronisation initiale admin** — `POST /admin/actions/initial-sync/run {player_slug, title_slug?, max_matches?}` → `RunPlayerInitialSync` (miroir RunFull de `RunPlayerConvergence`, type de job distinct `JobTypeAdminInitialSync` passant par le vivier de jetons ADR 0023, `resolveInitialMaxMatches` borné 1..2000), mêmes 400/409/503 que la convergence.
- **Notification de médaille inédite** — catégorie `medal_first_earned` : diff des totaux de médailles après synchronisation, amorçage silencieux au démarrage à froid, récapitulatif au-delà de 3 médailles, noms FR/EN, déduplication Discord par agrégation.
- **Catégories de médailles Halo 5** — résolveur à la lecture, miroir de Halo Infinite : 215 médailles en 11 catégories sous les 4 super-sections (0 non classée), sans re-seed ni backfill ; garde-rail de complétude et liste d'autorisation catégorie→super-section.
- **Noms d'armes par titre** — `weapon_names.toml` par titre (29 Infinite / 55 Halo 5), loader validé, amorçage au démarrage de `weapon_name_labels`, résolveur `weapon_id` → `weapon_key` → `{en, fr}` ; deux garde-rails (complétude registre↔TOML, anciennes sources interdites dans le résolveur).
- **Signal de placement de la performance** — `perf_placement_done/total` (nullable, `omitempty`) calculés par l'unique `computePerfPlacements` partagé (chaîne via `GetPerformanceChain`, éligibilité miroir du batch perf, renseignés uniquement si `1 ≤ taille de chaîne ≤ seuil` et si le match n'a pas de `perf_score`), indépendants de l'état LUSR/CSR.
- **XP de carrière sur les Sessions** — `CareerXPEstimated` sur `SessionDetailMatchRow`, miroir exact du calcul des Séries temporelles, gaté par `games.ProvidesCareerXPEstimate` / `CareerXPErasFor`.
- **Notification Discord de fin de cycle** — `scheduler/auto_sync_notify.go` : `notifyDiscordSyncCycle` câblé sur le tick périodique du planificateur (bascule existante `discord_notify_sync`, sans effet sans webhook ou sans nouveau match), `mergeCycleNewMatchPlayers` fusionnant les joueurs du moteur V2 et ceux du live/filet, dédupliqués par gamertag ; garde-rail de relais coach intact.

### Modifié

- **`api/openapi.yaml` est désormais GÉNÉRÉ (V72-01)** — `make openapi-gen` rend le document Huma partagé en trois passes (chemins serveur, retrait des wrappers opaques, fusion JSON Merge Patch du fragment manuel) avec une sérialisation déterministe et un vrai OpenAPI 3.1. `api/openapi_manual_fragment.yaml` est versionné (9 routes chi brut, 15 `RawBody`, erreurs et descriptions d'opération, 55 schémas non dérivables ; le fragment est prioritaire). Les golden `TestOpenAPIYAMLIsUpToDate` et `TestManualFragmentPathsSurviveGeneration` remplacent le test de dérive supprimé. Diff sémantique vs la baseline H0 : 597 ajouts, 16 pertes toutes inventoriées (yaml périmé), 530 noms de schémas inchangés. **Ne jamais éditer `openapi.yaml` à la main** — éditer le handler/DTO Go, ou le fragment.
- **Document OpenAPI partagé sur les 74 points de montage Huma** — `NewAPI` variadique + `WithSharedDoc(cfg, docPrefix)` (le legacy est inchangé sans l'option), préfixes absolus au point de composition, `MarkRequestBodyOptional` résolvant local→absolu, hook anti-collision `OnOperationRegistered`, test de fidélité `TestSharedOpenAPIDocCoversAllHumaRoutes`.
- **Métadonnées d'opération sur les 204 routes Huma** — helper `humacore.Op` (operationID / summary / tags), parité verbatim avec l'ancien yaml (159 exactes, 45 supplémentées documentées), gate `openapi_operation_metadata_test`.
- **Sémantique portable des DTOs en tags Huma** — 19 descriptions, 11 enums et 5 valeurs par défaut portées par 10 types domaine ; gate `openapi_schema_semantics_test` (35 sémantiques vérifiées, 0 perte hors inventaire fermé).
- **Modèle d'erreur unifié** — un seul type Go porté par `humacore.apiError`, enrichi de `Details any` + `FieldErrors []FieldError` (les deux `omitempty`) → contrat runtime `{code, message, retryable}` inchangé sur 458 points d'appel ; `ApiErrorSchema` / `FieldErrorSchema` abandonnés au profit de `ApiError` / `FieldError`.
- **Routes `groups` migrées en Huma typé** — les 7 routes quittent le `writeJSON` manuel / chi brut pour le document partagé (`RawBody` + décodage maison préservant le 400 `invalid_body`), contrat JSON runtime identique, zéro assertion de test modifiée ; `Group` / `GroupMember` dérivés fidèlement (enum `Role`, `members` jamais nil à la source).
- **Étanchéité entre titres (V72-29)** — `useCapabilityStrict` (fail-closed) sur les portes de synthèse des bandeaux ; `titleSlug` ajouté en 2e segment de toutes les fabriques de clés de requête par joueur (48 clés, 27 fichiers de points d'appel, invalidations par préfixe préservées) avec un garde-rail de complétude des clés et un garde-rail anti fail-open `spartan_customizer` ; caches serveur keyés par titre (`HomeMatchesCache`, `CachedRecentMatchesProvider`, `seasonEntries`) ; `CareerLiveCache` keyé `(xuid, titleSlug)` ; store d'apparence re-keyé `titre::joueur` (migration v3 avec réinitialisation), couleurs d'emblème et de bannière stockées séparément.
- **Course de bascule de titre (V72-29bis)** — le middleware Go fait l'écho de `X-LevelUp-Title-Resolved` sur chaque réponse (exposé en CORS) ; le client rejette toute réponse dont l'écho diverge du titre actif (`title_mismatch`, limité aux GET/HEAD — les mutations se contentent d'un avertissement, en anti double-soumission) ; `applyActiveTitle` annule **avant** le `POST /session/context`, valide le slug après confirmation, puis vide et ré-amorce, le refetch de focus du bootstrap étant neutralisé pendant la bascule.
- **Recherche gamertag** — nouveau paramètre `live` sur l'endpoint (`ctxkeys.WithGamertagLiveSearch` gatant `LiveFallbackGamertagSearch`) ; **tous les appelants sont en `live=0` par défaut** (local seul, ~200 ms), l'interrogation Xbox devenant une action explicite — aucune latence surprise nulle part, un seul point d'armement.
- **Sunburst des frags** — grenades ventilées par famille (fragmentation / plasma / dynamo / éclats) réconciliées avec le total natif ; `mechanic_kills` retranché des classes d'armes, ce qui met fin au double comptage Halo 5 (un frag au corps-à-corps en tenant une arme comptait deux fois). La somme des arcs égale désormais le total sur Halo 5 ; la liste « Frags par arme » est inchangée.
- **Vue de match** — le repli LIVE disparaît : `GetMatchView` renvoie un 404 typé `match_not_found` dès que le match est absent du substrat local (voir *Supprimé*) ; le contrat openapi préexistant est inchangé.
- **`mode_name_tr` centralisé** — 6 copies convergées vers `duckdb/mode_name_tr.go` avec un garde-rail à liste d'autorisation vide ; deux erreurs auparavant avalées sont désormais loguées. Les sous-titres de composition d'escouade reçoivent leurs modes en FR depuis l'API (`ModeTranslatorFR` injecté dans le provider, `analysis.NormalizeModeLabel` pour les libellés composés) et leurs playlists en FR résolues par `playlist_id` (id > `name_fr` > EN).
- **Formulation française** — « Lobby » → « Partie », les axes « OC / DR » → « Rendement / Résistance » (EN « Yield / Resistance »), « Net score cumulé » → « Solde frags − morts cumulé », et la description FR de la médaille Méganaute corrigée par une migration idempotente (la localisation officielle Waypoint est défectueuse).
- **Tri de LeaderboardBlock** — migré vers le `SortableTh` partagé (13 en-têtes, `aria-sort` gagné), exemption datée retirée ; garde-rail AST bidirectionnel de parité entre `registry.go` (Go) et `TITLE_CAPABILITIES` (TS), avec liste d'autorisation datée pour `weapon_kills`.

### Corrigé

- **Stats objectifs sur les matchs live** — le watcher dérive ses options de `DefaultSyncOptions` et un garde-rail par réflexion interdit d'oublier un drapeau d'extraction ; sans cela, la fonctionnalité serait restée vide sur les matchs fraîchement synchronisés.
- **Invalidation du cache Accueil** — keyée sur les vrais `(xuid, titre)` de la base joueur ; elle était sans effet, keyée sur le slug du joueur.
- **XUID de bots** — `cleanXUID` traite la forme canonique `bid(x.y)` sur le chemin objectifs **et** PvE (le contrat des chiffres nus est préservé).
- **Instantané des médailles** — `rows.Err` vérifié ; un ensemble invalide amorce désormais en silence au lieu d'émettre de fausses notifications.
- **Explorer** — `navigate(..., { replace: true })` à la sélection de mode/cible (fini l'empilement d'historique à chaque clic) ; axe horizontal rétabli sur le graphe « Écart de frags cumulé » (Relations inchangé) plus un état explicite « aucun match en commun » ; le bouton de repli du bandeau de briefing ne casse plus la mise en page avec le libellé long, et son aria reprend le vocabulaire du libellé visible.
- **Notifications** — noms de rang de carrière traduits par id (`RankLabelResolver`), paramètres `gap` / `current_mu` / `next_tier_mu` arrondis par défense.
- **Page Carrière** — blocs Classements et Évolution LUSR/CSR alignés en hauteur (`h-full` + fluid).
- **Menu déroulant d'escouade** — largeur minimale du popover et sous-titre sur deux lignes ; clés React composites sur les listes de joueurs (XUID Halo 5 vides et dupliqués).
- **Médailles fantômes Halo 5** — 3 `medal_name_id` gagnés en partie mais absents du catalogue officiel comme de halopedia (contenu coupé/bêta) exclus de la page Médailles par une liste datée ; les données sous-jacentes sont conservées.
- **Éligibilité au placement de la performance** — alignée sur le batch (`start_time`) ; top 2 des playlists résolu par id.
- **Lecture des erreurs côté front** — le front lisait un `status.error.message` inexistant côté serveur ; bascule vers `error_detail` / `error_code`, avec gardes de nullabilité. Les corps sont désormais déclarés sur les 7 réponses 201/202 et 4 `requestBody` manquants ont été ajoutés.
- **Tri des escouades** — `scoreSquad` compare les deux côtés normalisés ; les libellés composés résiduels ne cassent plus le tri en silence.

### Ops

**Actions post-déploiement (requises)**

- **Backfill des objectifs** — exécuter `cmd/backfill_objective_stats` en production, serveur arrêté. Sans lui, les statistiques d'objectif ne couvrent que les matchs synchronisés après le déploiement.
- **Re-seed et recalcul des citations** — `levelup seed citation-mappings` puis `levelup backfill --all --citations-recompute-all`. Les quatre citations d'objectif sont recalculées depuis `match_objective_stats` : rien n'est perdu, mais leurs valeurs sont révisées (source : awards → compteurs).
- **Contributeurs** — `api/openapi.yaml` est généré : lancer `make openapi-gen` puis `make generate-types` ; `make openapi-check` échoue en cas de dérive.

**Divers**

- **Alertes disque — anti-rafale persistant** — `DiskWatchState` persisté hors DuckDB (JSON atomique, `PathResolver.DiskWatchStatePath()` = `data/global/admin_state/disk_watch_state.json`, survit au recréation du conteneur grâce au montage lié de `data/`). L'entrée en alerte, l'aggravation et l'unique notification de rétablissement survivent désormais aux redémarrages ; le rappel périodique en dépassement stable a été retiré.
- **Version applicative bakée dans le binaire** — `build.args.VERSION` dans `docker-compose.yml` + `scripts/deploy.sh` calculant et exportant `LEVELUP_APP_VERSION` **avant** le build. Les releases ne dépendent plus de l'environnement d'exécution — cause racine des notifications de version manquées en v7.0.0 / v7.1.0, qui sortaient tôt sur `AppVersion == "dev"`.
- **Dependabot** — #65 (postcss 8.5.16 → 8.5.23, dépendance de développement) mergée, #51 (actions/checkout) fermée.

### Supprimé

- **Repli LIVE de la vue de match** — toute la chaîne supprimée avec ses tests et ses imports : `tryCanonicalMatchView`, les champs `dataAdapter` / `viewerGamertag` et leurs setters `With*`, `LoadMatchDetail` sur `games.TitleDataAdapter` et ses 3 implémentations, le fichier Halo 5 `mapping_carnage_detail.go`, `enrichCommendations` (les totaux à vie sont conservés), `ctxkeys.WithViewerGamertag`, les types canoniques `MatchDetail` / `Commendation`, ainsi que le `WithRankedClassifier` orphelin et le chargement de `ranked_hoppers.toml` au démarrage.
- **Code mort** — `queryKeys.player()` (0 appelant), package `weaponregistry` (0 import), `CareerPage` et `CareerTopMatchesTable` avec leurs tests (non routés), le repli `name_fr` inventé du registre d'armes, et le test de dérive OpenAPI avec ses helpers (supplanté par les golden).
...
- **Graphes FDA attendu** — `TimeseriesFdaGapTrend`, `SessionFdaGapCumulative`, `SquadFdaGapCumulativeCard` par membre ; une fine courbe de FDA attendu par match ajoutée sur le graphe d'écart (axe partagé) ; helper canonique `divergentZeroGradient` (deux copies inline migrées + garde-rail).
...
- **Explorer — briefing V3 (compaction)** — le bandeau de briefing (mode Matchs) est compacté pour faire remonter le tableau de résultats au-dessus de la ligne de flottaison : **Classement** et **Séries** deviennent des tuiles KPI du socle, la tendance du taux de victoire devient une micro-courbe nue dans la tuile **Taux de victoire**, le contexte solo/escouade est fusionné dans la carte `Par contexte` de la grille `Par…`, les **Moments forts** deviennent une bande de pastilles nue, et la frise des résultats est retirée ; tooltips de légende `(i)` sur les tuiles, les cartes `Par…` et la bande ; le FDA net est coloré sur toutes les surfaces du bandeau.
- **Explorer — briefing V4** — refonte post-revue du bandeau (mode Matchs) : le module **Classement** est recalculé **par chaîne de playlist** (une ligne par chaîne `(type, playlist_group)`, jamais de flèche inter-chaînes) et quitte le socle de tuiles pour une 4ᵉ colonne scindée sous `Par contexte` ; les tuiles du socle sont refondues autour de la tuile hero **Taux de victoire** (`OutcomeBar` + V-D-N + tooltip des 4 issues) avec **Perf colorée**, **Durée totale**, **Pic FDA**, et une **cascade** de tuiles conditionnelles par priorité (Meilleure série > Pic rang > Pic MMR, au plus deux sur trois, jamais plus de huit tuiles) ; la sparkline de tendance est retirée ; les tooltips `(i)` sont désormais rendus via un **portal** et ne sont plus rognés par l'`overflow-hidden` des tuiles ; le tableau de résultats surligne le **meilleur/pire par colonne clé** (MVP/LVP, style du scoreboard réutilisé) et resserre le padding des cellules.
- **Explorer — briefing V5** — finitions post-revue du bandeau (mode Matchs) : les tuiles **FDA** et **Perf** deviennent des triptyques **min · moyenne · max** (bornes calculées côté serveur, la moyenne mise en avant colorée par le FDA net / le palier de perf), absorbant et supprimant la tuile autonome **Pic FDA** ; la cascade conditionnelle affiche désormais les **trois** tuiles présentes (Séries marquantes, Pic rang, Pic MMR — Pic MMR de nouveau visible, plafond à huit) ; le **Classement** rejoint `Par…` / `Par contexte` en sibling sur une seule rangée auto-fit (plus de 4ᵉ colonne empilée) ; le tableau de résultats surligne désormais le **décile haut / bas** (p90/p10 sur tout le scope chargé) par colonne clé en teinte plus douce au lieu d'un meilleur/pire unique, et les **colonnes numériques sont alignées à droite** (en-tête et cellules ; le texte reste à gauche) ; chaque tuile KPI porte un **accent** (jeton neutre sans sentiment), les valeurs de première ligne sont **centrées**, le compteur redondant « N matchs » est retiré et l'**export CSV déplacé sous le tableau** ; **Séries** est renommé **Séries marquantes**, et les compteurs de dimension/contexte perdent le mot « matchs » (conservé en survol).
- **Explorer — briefing V6** — finitions post-revue du bandeau (mode Matchs) : le surlignage par décile du tableau (MVP/LVP) porte désormais sur le **MMR d'équipe** (plus haut = meilleur) au lieu de la colonne Score (seul son surlignage part ; la colonne Score reste affichée) ; les bornes min/max des triptyques **FDA** et **Perf** deviennent plus lisibles (contraste plein `text-foreground`, un peu plus grandes) tandis que la moyenne colorée reste dominante ; tous les tooltips du bandeau sont réécrits en registre **factuel et impersonnel** (sans vouvoiement ; le triptyque se lit « plus bas · moyen · plus haut ») ; la tuile **Séries marquantes** est rendue ~10 % plus large et **Pic MMR** ~10 % plus étroite (socle flex, sans trou quand des tuiles conditionnelles sont absentes).
...
- **DTO briefing Explorer — classement par type & nouvelles sections** — `ExplorerBriefingRanked` refondu pour émettre un `ExplorerBriefingRankedKind` par type de rating (paliers début/fin, flags de placement, points par match) au lieu d'un cumul brut ; champ additif `KPIStats.RankDeltas []RankDelta` exposant les buckets par `RatingType` existants ; nouveaux blocs `ExplorerBriefingContextSplit` (solo/escouade), `ExplorerBriefingStreaks` et `ExplorerBriefingDominance` ; re-tri des dimensions par taux de victoire en plein historique. Le calcul `expected_win_prob` et ses champs DTO ont été retirés. Le champ `outcome_sequence` et son type `ExplorerBriefingOutcome` ont été retirés du DTO de briefing (la frise front a été retirée en V3).
- **DTO briefing Explorer — classement par chaîne & agrégats du socle (V4)** — `ExplorerBriefingRankedKind` gagne `playlist_group` ; le classement est recalculé par chaîne `(rating_type, playlist_group)` via l'algo pur `analysis.ComputeRankProgressionByChain` (variation nette du rating par chaîne, co-signée avec la progression de paliers), en remplacement du bucket cross-chaînes `KPIStats.RankDeltas` (désormais retiré). Le sous-DTO `trend` (`ExplorerBriefingTrend`/`...TrendPoint`) est retiré ; `ExplorerBriefingScope` gagne `total_duration_seconds`, `peak_kda`, `peak_team_mmr` et `peak_ranks` (meilleur palier atteint par système de rating, via `analysis.CSRTierOrdinal`).
- **DTO briefing Explorer — agrégats Min/Max du scope (V5)** — `ExplorerBriefingScope` gagne `min_kda`, `min_perf` et `max_perf` (`*float64`, `omitempty`), calculés en mémoire par les helpers de service symétriques `minScopeFloat`/`maxScopeFloat` sur les `KDA`/`PerformanceScore` par match DÉJÀ scannés (aucun SQL neuf, aucune migration) ; ils alimentent les triptyques FDA et Perf min·moy·max du front, aux côtés des `kda`/`avg_perf`/`peak_kda` existants.
- **Briefing Explorer — chaînes de classement pertinentes (V6)** — le module Classement ne liste plus toutes les chaînes : le service ne garde que les chaînes d'au moins `MinRankedChainMatches` (= `MinDimensionGroupMatches`, 10) matchs, plafonnées aux `RankedChainMaxCount` (3) les plus jouées (`selectTopByMatches`, restituées en ordre canonique), avec un **fallback** conservant la chaîne principale (type de rating majoritaire) quand aucune n'atteint le seuil — en miroir de la politique top/flop des dimensions. Filtrage en mémoire sur l'algo pur inchangé `analysis.ComputeRankProgressionByChain` ; la forme du DTO `ExplorerBriefingRanked` est inchangée (il émet simplement moins d'entrées).
