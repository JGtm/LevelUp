...
- **v7.0 — Média V2, likes persistants & grille allégée** — la page Médias V2 permet désormais de liker directement captures et vidéos, de grouper par aimés, session ou contexte solo/escouade, et de conserver ces likes au reload via une persistance robuste dans `ui_prefs.json`. La grille utilise aussi des thumbnails natifs plus légers, une lightbox partagée avec avance auto vidéo optionnelle, et les vrais assets coeur pour les états liké / non liké.

- **v7.0 — Page Synthèse** — une nouvelle entrée de navigation après Escouade regroupe les graphes de vue d'ensemble déjà présents dans l'app, mais cette fois sur tout l'historique par défaut avec un sélecteur de période local. La page ajoute aussi un duel chart Solo vs Escouade lisible sans radar, et le graphe top-vs-total adapte mieux son bucketting sur les profils multi-années très clairsemés.
...
- **v5.5 — Wizard & Xbox OAuth & macOS/Linux** — Wizard de configuration initiale avec deux parcours (Xbox Express / Azure manuel). Connexion Xbox en 1 clic. Workflow local documenté pour macOS/Linux et Windows. **75 nouveaux tests**. Page **Comparaison de sessions** entièrement revue : donuts résultats, courbe F/D + précision, highlights match, répartition modes/cartes, overlay LUSR/CSR sur le score cumulé. **Comparaison XP & rang Héros multi-joueurs** sur la page Carrière — précision variable selon l’historique de sync.
