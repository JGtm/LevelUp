...
État au 2026-09-16 : `source.Rev` et `facts.Rev` EXISTENT, et la sous-section ci-dessous les décrit au présent. Le dépôt porte TROIS révisions — `source.Rev` (la couche source, `internal/games/halo_infinite/film/internal/source/rev.go`), `GrammarRev` (la couche grammaire, `internal/games/halo_infinite/film/internal/grammar/grammar_rev.go`) et `facts.Rev` (la couche des faits, `internal/games/halo_infinite/film/internal/facts/rev.go`), et c'est `facts.Rev` qui commande le backlog. `profile.Rev` et `grammar.Rev` restent des CIBLES du lot 2.6 (ses volets grammaire et profil, après l'extraction des couches), et les lignes qui les nomment ci-dessous sont écrites au futur ; le document de rejeu cuit est au schéma 60.

Le décodeur de film est fait de cinq couches (ADR 0034, D-1), et quatre d'entre elles porteront leur propre révision (D-6). Une révision est une chaîne figée dans un golden à côté de l'empreinte des sources non-test de sa couche : une source qui change sans sa révision fait rougir le test d'empreinte, et une révision qui change sans sa source le fait rougir aussi. Le calcul, la chronique, la porte de régénération et les messages d'échec sont partagés (`internal/games/halo_infinite/film/revision`) ; une couche ne déclare que ses racines, son golden, sa porte et la question que son échec pose.

| Révision | Ce qu'elle hache | Elle monte quand |
|---|---|---|
| `source.Rev` (existe) | la couche source : chargement, décompression, découpage en chunks et paquets, lecteur de bits canonique | la façon d'atteindre les octets change — tout est redécodé |
| `profile.Rev` (cible, lot 2.6) | la couche profil : la table par build et par carte | une ligne de la table du profil change |
| `grammar.Rev` (cible, lot 2.6) | la couche grammaire, plus les deux révisions ci-dessus | une largeur, un cadre, un ordre de composants, un lecteur neuf |
| `facts.Rev` (existe) | tout l'arbre des faits (killsource, objectives, fallback), plus les **valeurs** de `source.Rev` et de `GrammarRev` | la sortie des faits peut changer |

`facts.Rev` hache les VALEURS des révisions du dessous à dessein, et c'est le seul lien qui ne va pas de soi. Le défaut qu'il ferme est mesuré : une correction de grammaire peut changer la sortie de la source de kill sans toucher un octet de la couche des faits, la révision restait alors immobile, et les lignes déjà écrites portaient la révision courante — exclues du backlog à vie. Un faux positif coûte un redécodage ; un faux négatif coûte un parc de lignes fausses. `GrammarRev` tient la place de `grammar.Rev` tant que celle-ci n'existe pas.

**Ce qu'est le backlog** (en vigueur, sous `facts.Rev`). Chaque ligne de `match_kill_events` porte dans `decoder_rev` la révision qui l'a produite. Un match dont la passe courante — lue par la vue `match_kill_events_latest`, jamais la table brute (ADR 0026) — ne porte pas la révision courante redevient candidat (`conditionBacklog`, `internal/sync/killcollector/postsync.go`). L'étape de post-sync rattrape à cadence bornée (8 films par cycle, budget de cinq minutes) et publie le reste dans l'expvar `killsource_postsync_backlog_restant` ; `levelup backfill-killsource --online` le vide délibérément. La recuisson du parc entier reste un geste à part, pris sur signal de l'utilisateur, jamais par lot.
...
**Lire les révisions sur un artefact** (cible : schéma 61, lot 2.6.3 — le dépôt est au schéma 60 et le bloc n'existe pas encore). À partir du schéma 61, un document de rejeu cuit portera `coverage.decoder.{grammarRev, factsRev, build}` (`GET /players/{player_slug}/matches/{match_id}/replay`) : l'artefact dira sous quelles révisions il a été cuit, au lieu de se deviner à sa version de schéma. `build` est la clé du profil, lue en clair dans `chunk_00` (D-3) ; sur un build inconnu (`ErrUnknownBuild`) elle vaudra la chaîne vide et le bloc restera présent. L'**absence** du bloc signifiera « artefact cuit avant le schéma 61 », jamais « build inconnu ».
