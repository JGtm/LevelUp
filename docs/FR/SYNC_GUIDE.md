...
État au 2026-09-16 : le dépôt porte DEUX révisions — `GrammarRev` (la couche grammaire, `internal/games/halo_infinite/film/filmdec/grammar_rev.go`) et `KillSourceDecoderRev` (les faits killsource, `internal/sync/killcollector/killsource_decoder_rev.go`) — et c'est `KillSourceDecoderRev` qui commande le backlog aujourd'hui ; le document de rejeu cuit est au schéma 60. Le tableau ci-dessous décrit la CIBLE du lot 2.6 du plan du décodeur de film (quatre révisions, une par couche), pas l'état présent : ce lot mettra cette sous-section au présent.

Le décodeur de film est fait de cinq couches (ADR 0034, D-1), et quatre d'entre elles porteront leur propre révision (D-6). Une révision est une chaîne figée dans un golden à côté de l'empreinte des sources non-test de sa couche : une source qui change sans sa révision fait rougir le test d'empreinte, et une révision qui change sans sa source le fait rougir aussi — les deux révisions ci-dessus fonctionnent déjà ainsi.

| Révision (cible, lot 2.6) | Ce qu'elle hachera | Elle montera quand |
|---|---|---|
