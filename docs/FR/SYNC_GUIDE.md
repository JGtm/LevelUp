...
| `objectives.Rev` | `facts/objectives` et les paquets qu'il importe (`domain/objectiveevent`, `film/types`), plus la **valeur** de `source.Rev` | la sortie des objectifs / du statborg peut changer — pas de backlog, les faits et les calques d'objectifs deviennent périmés |
