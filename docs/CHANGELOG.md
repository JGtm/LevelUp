...
> The release of the Theater film. 3 640 commits and 667 merges since `v7.3.0` (tag `a2719a68c`, 2026-08-04), 7 867 files touched — by far the largest release of the project. Halo writes a film of every match and the app had never opened it; it now decodes it end to end and turns it into a **2D top-down replay**, a **Tactics tab**, and a set of measurements no API field carries: the weapon behind every kill, engagement range and height, weapon levels on the pads, net flag grabs, Assault statistics, payback and support between teammates, and equipment read as used / kept / dropped. Everything film-derived is gated behind fine-grained `film.*` capabilities and exists on Halo Infinite only. Four ADRs were written along the way: **0032** (round-decided modes), **0033** (squad session population), **0034** (film decoder — five layers, a single gate to the bytes, amended at each milestone up to persisted film facts), **0035** (player directory — the xuid as the single identity key). **Post-deploy actions are required — see *Ops*.**
...
- **Replay schema 54 → 68, one climb per published change** — 55 `coverage.tracks` (a life refused for publication is counted instead of vanishing); 56 the identity registry takes the film's own 32-slot player table (`chunk_00`) as its direct link, and a one-sample life is published; 57 each player's team is read from the film (`Track.Team`, `roster[].team`) instead of being joined from the database; 58 `coverage.fallbacks` names and counts every fallback fired during a bake; 59 the origin of an equipment placement is read (`deployed` = what a type-103 event designates, `dropped` = what leaves its carrier, the rest stays `unknown`); 60 a life ends at a written death (a replication gap becomes a counted gap), a vehicle life ends at its written dead-state (`vehicles[].end`), Wraith, Scorpion and the banned auto-turret are named, and the round designator is published as written; 61 `coverage.decoder` carries the layer revisions the artifact was baked under; 62 `layers` (one revision per layer) and `coverage.deathsPaths`; 63 the flag return gauge (`flagCarries[].spans[].returnProgress`) and per-spot vehicle respawn cycles (`vehicleCycles`); 64 the side pushing a capture (`zoneStates[].gaugeRamps[].capturingTeam`); 65 `stances[]` (crouch, slide, mobility action); 66 sprint read from the film, and `jumpDerived`, computed from vertical speed and named as such; 67 `rides[].src` = `film` or `proximity`, with `coverage.vehicles.ridesRead` / `ridesProximity`; 68 `mobility` renamed `clamber` after a 9/9 Theater verdict. The reasons for each climb are written in `film/replay/document_chronicle.go`.
- **The decoder in five compiler-enforced layers, with persisted film facts (ADR 0034, amended at M2, M3 and M4)** — `source`, `profile`, `grammar` and `facts` live under `film/internal/` behind the `film/decfilm` facade (166 symbols, surface ratchet `archlint/film_facade_surface_test.go`), `film/replay` is the publication layer; a single gate to the bytes (`film.Source`); no package-level variable is written any more, so two films of different builds decode in parallel under `-race`; one revision per layer, chained by value and recorded in the artifact. The profile is data (`film_profiles.json`, with register fingerprints; procedure in `docs/RUNBOOK_FILM_PROFILES.md`), and a film with an unknown key is set aside, counted and logged. A film's facts are persisted (`data/cache/film_facts/{slug}/<short8>.filmfacts.bin`, five sections, a 110-byte header carrying the four revisions) and publication replays from them when they are fresh — byte-identical to a decode on 10 films, 95× to 442× faster; "needs re-cooking" becomes a three-way verdict (up to date / republish / re-decode), and the admin page shows what the facts occupy.
- **The frame grammar read further (post-milestone series 5)** — each view rank is decoded under the grammar of its own class (three view classes: the false `DEL`s of the frame footer no longer unbind live entities), the view rank is read from the two top bits of a keyframe id, the keyframe reader is read in full, the type-1 block is the chunk's datum table, the type-8 packet is the player list (same body as the `chunk_00` table, factored), the per-component corruption check is read from the film, and an anticipated archetype table is a named, counted fallback. On the densest witness film (`bfecd02b`), closed packets go from 888 to 3 919 and movement-state intervals from 616 to 841. Measured and written down rather than hidden (lot 5.25): the movement-state layer still loses 35.4 % of player biped ticks on that film. Revisions `grammar-2026-09-22.12` and `killsource-2026-09-22.2`.
- **Player and vehicle states read at the instant** — crouch, slide, clamber and sprint are read from the film per life and to the millisecond (the jump has no replicated field: it stays derived and says so); a vehicle occupant's seat is read (`i10 object-parent-state`), proximity is only a fallback that yields to the reading; a vehicle's heading is read from the film (velocity is the fallback), a turret shot takes the aim of its gunner, matched by slot; the side pushing a zone is read from a second `ti=13` channel, the deduction survives as a named fallback; the flag return gauge is `ti=13 i1` tag 3.
- **`levelup backfill-killsource` — parallel, observable, resumable** — `--workers N` (default 3, at most 9 against a measured 422 MiB peak per film; ×2.7 with identical rows written), a state file rewritten after every film at `data/global/admin_state/backfill_killsource_<slug>.json` and read by `--status` without opening any database, resume keyed on `decoder_rev` in the database, `Ctrl-C` stops dispatching, lets the films in flight finish and exits with code 130. The credit pass is no longer quadratic (the identity view is read once per pass, progress every 500 matches). Documented in `docs/COMMANDS.md`.
- **Decoder gates** — `replay-corpus-gate` grows to 19 witnesses (two of them with zones) and names changes apart from gains and losses; `replay-equiv` names every divergent stage and gains a two-pass mode (`-deux-passes`) proving that a replay from facts equals a decode, and it cannot re-freeze a reference; the heavy research instruments move behind the `research` build tag (`filmdec` unit run 89.9 s → 15.0 s), with `go vet -tags=research` in CI.
...
- **Coordination: Payback and Support** (`internal/analysis/coordination`) — a payback is a death avenged by its side on its killer within **5 seconds**, one rule everywhere (`FenetreEchangeMs`); a support is an assist between teammates. On the Squad: a narrative card, a donut of deaths paid back and a payback-time histogram, a per-death cloud of kills left unanswered (late paybacks kept visible up to a 60 s cap on a log axis, `PlafondRiposteTardiveMs`) and a timeline over the whole history of sessions; on the Sessions (both sides of the comparison drawer, against the usual reference), on Timeseries (per evening, as a gap to the reference) and on the match view (per player, one chart per side).
- **Range and height against the lobby** — the frag range of every player of the lobby, per match (`WeaponRangeFilters.AllPlayers`, bounded by `MatchIDs`), so a player's median is read against the lobby's instead of against the playlist; "Range roles" cards on the Squad and Timeseries, a period range reference on the Sessions with the session highlighted, "Elevation roles" on the Squad, and **engagement height** (the signed height gap at the killing blow) on the match view (`combat_tab.elevation`, click through to the replay instant) and on Timeseries as a distance × height cloud.
- **Distance records by weapon** on the Synthesis — the longest measured kill of each weapon, with the excluded weapons named in the (i).
- **Weapon profile in the Head-to-head** — frag share by weapon class, range by role (P10 / median / P90, min / max in the tooltip, sample floor of 8) and the top 3 weapons with icons; Halo 5 reads its native mechanics.
- **Explorer** — the briefing shows the favourite weapon of the selection (one or two weapons; film on Halo Infinite, native data on Halo 5), a "Frag range" block for the scouted player, assists exchanged with the target in the encounter stats, the assist share and the outcome split as horizontal tracks, and the "matches played together" section on three rows.
- **Assists exchanged per relation** — a column in the Relations tables and the match view, a "Duo partner" butterfly card and an "Inner circle" card; "Lackey / Boss" roles on the assist charts of the Squad and the match view.
- **New badges** — "Thief" in the Squad impact matrix (most kills stolen from a friend: assisted by that friend, 10 % of the damage or less, friend alive within ±500 ms), and "Scuttled" / "Selfless" on objective modes (the team that loses on score dominated the kills, and its mirror), with their own colour in the four palettes.
- **Home match tile** — the share of kills assisted by a teammate, by tier (bounds 25 / 50 %).
- **Squad intensity** — two reference curves, the real allied team and the lobby (the former "Team" curve was the lobby).
- **Match view reorganised** — a "Weapons and terrain" tab (kills and weapons, distance, engagement height, equipment usage, pad control, film positions), special-weapon control as stacked columns per pad, each team's share as horizontal tracks per family, the field occupation map 20 % larger and zoomable, equipment usage split into two cards.
- **Timeseries and Squad get a "Usage" tab** (range, equipment, "the shapes you keep"); the cumulative **damage balance** lands on the Timeseries Summary; Impact and Medals join Contributions on the Squad; the Sessions page gets four sections (Overview, Match by match, Kills and usage, Match details) and a side-by-side comparison in shared rows, with a placeholder when a section has no equivalent.
- **Replay** — mode objectives carry their floor like players, with a "Mode objectives" toggle and its help; the flag return gauge ("Return N %") and the vehicle respawn cycle shown on the map; the movement state (crouch, slide, clamber, sprint, derived jump) on the player card; vehicle weapons get their own flash style and tint, vehicles are drawn 20 % larger with redrawn sprites, markers are sized in screen space (real size when the scale allows, a guaranteed minimum otherwise); the capture fill takes the colour of the side pushing; the video export renders to standard 1080p / 720p formats, with the framing chosen when the map is zoomed.
- **Friends per player and invitations without a group** — the friend list belongs to a player (`GET`/`PUT` endpoints, boot migration), a "Friends and groups" page, an admin invitation section and a generic "Join" page, with a single-use provisioning right pinned to the bearer's identity.
- **Player directory (ADR 0035)** — the xuid as the single identity key across profiles, accounts, tokens and live tracking; `GET /admin/identities` and an "Identities" section with an "Instance closed" switch on the Management page; `Onboard` is the only path that creates a profile (ratchet); `levelup identity purge` removes an identity without touching the shared database.
- **A copy button for the Xbox device code**, confirmed on click.
- **Shared UI primitives** — a dedicated colour family for combat stats (kills, deaths, assists, assist given / received) in the four palettes, frameless charts inside section cards (no more double frames), a shared section title (`DetailSection`) and title-with-info helper, and a canonical empty state (`EmptyStateNotice`) naming its cause on the usage blocks, "the shapes you keep" and every chart card.
...
- **Sync through the pool for any followed profile** — a followed profile without a token of its own is synced by the token pool; the title-owned seams are wired into every CLI; `--token-pool-size` caps the number of healthy slots, not the sources tried; the history retry is bounded to one page with a faithful pass verdict; the career rank endpoint is treated as public (the pooled client is no longer pinned); shared migrations run before every CLI sync.
- **Nothing runs for an account without a followed profile (ADR 0035)** — `domain.ProfileGate` on the sync coordinator, the watcher daemon and the SSO; the instance lock is decided in a single place (`authz.InstanceLocked`) with safe defaults in enforced mode.
- **Mode names from inverted `pair_name`s** — "Slayer:Arena", "CTF:Arena Neutral Flag" and their kin give a mode, not a container (a single list of container tokens in `analysis/modelabel`); French seeds "Assassin en duo" and "Assassin tactique".
- **Labels** — weapon names follow the request locale (match view, per-weapon accuracy, highlight weapons, Home favourite weapon); the Synthesis hijack card carries one label for every title ("Dépositaire" was the name of the Reclaimer medal); French vocabulary: "hauteur d'engagement" instead of "dénivelé", "appui" for an assist between teammates, "usages" instead of "gestes", with guard-rails against the old words.
- **Weapon levels on display** — bonus pads (camouflage, overshield) are equipment and leave the weapon sections, "unclassified" no longer has a row, and the display order of the tiers is decoupled from the tie-break.
- **One heat-map grid** — Synthesis, Relations, Explorer and Squad share a single categorical grid renderer, without a gradient bar.
- **Squad filter bar outside the layout** — its transient state lives in the bar, so a click no longer replays the chart animations (9 → 0 renders of the page content, boundary guard-rail).
- **Head-to-head clean-up** — the never-filled `filters` field of `CompareRequest` is removed, and so is the "cross sample" fallback, which was serving Halo 5 campaign co-op stats as a matchmade sample.
- **CI budgets** — the Coverage job goes from 45 to 60 minutes (cancelled at 45 twice in a row on `feat/v75`, 35 to 41 minutes when it passes, return criterion written in the workflow); the decoder unit job gets 300 s on the Windows runner.
...
- **`personal_score_awards` loses its last three ART indexes** — a recurrence of DuckDB #23645 on ordinary inserts; no reader used them (the `_latest` view scans anyway); the `data_health` PSA probe and `cmd/repair_psa_index` are removed, the existing ratchet extended.
- **Provisioning an additional title processes all of its databases** and joins the errors — Halo 5 had been left without its shared and social databases from 12 to 20 September.
- **Boot anomalies** — the `weapon_families` label purge now belongs to the Halo 5 migration set (a provisioning ERROR since 12 September); the shared provider drains its readers with a counter and a channel instead of a WaitGroup (two crashes on 16 September); the `RpsTicket` provenance is measured at the XBL chokepoint and persisted (no more 401 at every boot); the configuration security WARN only fires when the instance is exposed.
- **A starting weapon is read before the first pickup of the life** — the `loadouts` channel is published on a global 20-second keyframe grid, not at spawn; the Impaler left the starting weapons of `b1ad85eb` (6.85 % → 4.25 %) and 34 matches of the corpus changed their set of starting weapons, in the right direction (Go and the TypeScript twin).
- **Replay** — a substitute bot joins the right team (never two "Cobra" teams again), an unknown chassis is no longer scenery (its occupant and shots come back), vehicle shot effects are visible, the base sounds follow the gesture (capture ramp, deduplicated captures, hill by movement, score tick on the game clock), and floor rings (players and objectives) are no longer drawn on flat terrain.
- **`match_registry` team scores back to `INTEGER`** — a schema drift repaired.
- **Media lightbox** — opening the next clip no longer aborts the manifest of its neighbours (HLS `stopLoad` reserved to started instances, thumbnail as poster).
- **Backups** — `data/global` enters the restic scope: per-player friends and Xbox aliases were not backed up.
- **Citation passes read `metadata` through `OpenReadForQuery`** — the admin backfill runs under a read-write handle.
- **Sessions** — the left column shrinks when the comparison drawer opens (one column when the drawer is open, transverse ratchet).

### Ops

- **(1) Re-cook the replay artifacts** — passes (1) to (4) run in this order, **server stopped** (single writer on the shared database). `ReplayDocument.SchemaVersion` is 68; artifacts cooked at an older schema are served with a "needs re-cooking" badge and some layers stay empty. Run `levelup backfill-replay --only-existing`: it re-cooks only the artifacts already on disk, republishing from the persisted film facts when they are fresh, re-decoding otherwise and writing the facts on the way (one film = one child process; **never** cook in batch on matches without an artifact, never without an explicit go).
- **(2) Re-project equipment and pad usage** — `levelup backfill-usage-summary`, after the re-cook: it re-summarises when the projection revision or the artifact schema has moved.
- **(3) Re-project the weapon tiers of pad pickups** — `levelup backfill-pad-tiers --force`, after the re-cook as well: it reads the replay artifacts, so running it earlier would compute tiers on schema-62 artifacts. `--force` because a starting weapon is now read before the first pickup of the life, and rows already written carry the previous rule, which resume would never revisit.
- **(4) Recompute the kill source** — `levelup backfill-killsource` (100 % offline, from the local film cache, independent of the three passes above): revision `killsource-2026-09-22.2` changed the facts, so every match decoded under an older revision is decoded again. 3 workers by default, `--status` from another terminal, resumable by re-running the same command.
