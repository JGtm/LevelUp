...
- **(1) Re-project the weapon tiers of pad pickups** — passes (1) to (4) run in this order, **server stopped** (single writer on the shared database). `levelup backfill-pad-tiers --force`: a starting weapon is now read before the first pickup of the life, and rows already written carry the previous rule, which resume would never revisit.
- **(2) Recompute the kill source** — `levelup backfill-killsource` (100 % offline, from the local film cache): revision `killsource-2026-09-22.2` changed the facts, so every match decoded under an older revision is decoded again. 3 workers by default, `--status` from another terminal, resumable by re-running the same command.
- **(3) Re-cook the replay artifacts** — `ReplayDocument.SchemaVersion` is 68; artifacts cooked at an older schema are served with a "needs re-cooking" badge and some layers stay empty. Run `backfill-replay --only-existing` on the park after deployment: it republishes from the persisted film facts when they are fresh, re-decodes otherwise and writes the facts on the way (one film = one child process; **never** cook in batch without an explicit go).
- **(4) Re-project equipment and pad usage** — `levelup backfill-usage-summary`, after the re-cook: it re-summarises when the projection revision or the artifact schema has moved.
