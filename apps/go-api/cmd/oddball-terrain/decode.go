...
func emitRounds(id string, recs []objectives.StatRecord) {
...
func joinPoints(pts []objectives.ScorePoint) string {
...
func sortedKeysInt2(m map[int][]objectives.ScorePoint) []int {
	out := make([]int, 0, len(m))
	for k := range m {
		out = append(out, k)
	}
...
func sortedSeriesSlots(m map[int]map[int][]objectives.ScorePoint) []int {
	out := make([]int, 0, len(m))
	for k := range m {
		out = append(out, k)
	}
