...
func chargeCartes(racine, slug string) ([]carte, []string, error) {
	...
	bornes, err := grammar.LoadMapQuantCatalog(res.MapQuantBoundsPath(slug))
	if err != nil {
		return nil, nil, err
	}
...
func ancresParModule(cat *replay.MapObjectivesCatalog, bornes *grammar.MapQuantCatalog) map[string][][3]float64 {
...
func moduleDeLEntree(e replay.MapObjectivesEntry, bornes *grammar.MapQuantCatalog) string {
