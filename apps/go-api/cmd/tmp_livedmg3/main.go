...
func main() {
	...
	// coup fatal par biped = dernier dégât (max tsc)
	type fin struct {
		atk int
		fam uint32
		tsc uint64
	...
	last := map[uint32]fin{}
	for _, r := range recs {
		if e, ok := last[r.vic]; !ok || r.tsc > e.tsc {
			last[r.vic] = fin{r.atkPi, r.fam, r.tsc}
		}
	}
	// death-events groupés par tueur
	deBy := map[int][]fin{}
	for _, f := range last {
		deBy[f.atk] = append(deBy[f.atk], f)
	}
	for k := range deBy {
		sort.Slice(deBy[k], func(i, j int) bool { return deBy[k][i].tsc < deBy[k][j].tsc })
	}
	// kill feed
	...
	// kills groupés par tueur (ordre temporel)
	kBy := map[int][]kf{}
	for _, k := range feed {
		kBy[k.killer] = append(kBy[k.killer], k)
	}
	// comptes
	fmt.Println("=== coups fatals (death-events) vs kills, par tueur ===")
	for p := 0; p < 8; p++ {
		fmt.Printf("  pi%d %-16s : %d coups fatals capturés / %d kills feed\n", p, roster[p], len(deBy[p]), len(kBy[p]))
	}
	// attribution : match dans l'ordre par tueur
	idx := map[int]int{}
	att := 0
	...
	fmt.Printf("\n=== ARME PAR KILL (live, biped-death + ordre par tueur) — 30 premiers ===\n")
	for i, l := range lines {
		if i < 30 {
			fmt.Println(l)
		}
