...
func runBackfill(cfg *config.AppConfig, args []string) error {
	...
	if !*engagementScores {
		return fmt.Errorf("aucun backfill selectionne (utiliser --engagement-scores)")
	}

	ctx := context.Background()
	if *allPlayers {
		return runBackfillAllEngagement(ctx, cfg, *force)
	}
	player, err := loadPlayerSummary(cfg, *gamertag)
	if err != nil {
		return err
	}
	return runBackfillEngagementForPlayer(ctx, cfg, player.Gamertag, player.XUID, *force)
}
