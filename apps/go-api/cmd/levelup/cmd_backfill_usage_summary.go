...
func runBackfillUsageSummary(cfg *config.AppConfig, args []string) error {
	...
	journaliserReplisUsageCorpus(ctx, b)
	return nil
}

// journaliserReplisUsageCorpus dit QUELS REPLIS la projection a declenches sur le corpus, et
// combien de fois (D14 c ; cablage du 2026-09-16, revue de jalon M1 ronde 2, constat F2).
//
// POURQUOI CE JOURNAL EST L INSTRUMENT, ET PAS LE CORPUS GATE. Les replis de
// `BuildUsageSummary` se declenchent APRES la cuisson, sur un document deja cuit : ils
// n entrent jamais dans `coverage.fallbacks[]`, et `replay-corpus-gate`, qui lit les artefacts,
...
func resumerCorpus(
	...
	for _, id := range candidats {
		...
		projetes++
		b.projetes = projetes
		// LES REPLIS DE LA PROJECTION, PAR MATCH PUIS CUMULES. La ligne par match n est ecrite
		// que s il s est declenche quelque chose : c est ce qui permet de RETROUVER l artefact,
		// et un corpus entierement lu n en produit aucune. Le cumul, lui, se dit toujours.
		if r := s.Match.Fallbacks; len(r) > 0 {
			b.replisConcerne++
			b.replis.Cumuler(r)
