...
// ScanFilmBipedAimOnly decode, sur tous les chunks du film, les lectures de visee des records
// bipedes qui NE PORTENT PAS de position. La bande de slots est celle des images-cles `ti=35`,
// exactement comme `ScanFilmBipedPositions`.
//
...
func ScanBipedAimRecords(payload []byte, slots map[uint32]bool) []BipedAim {
	total := len(payload) * 8
	var out []BipedAim
	for p := 0; p+bipedHeaderBits <= total; {
		at, slot, ok := matchAimOnlyRecord(payload, p, total, slots)
		if !ok {
			p++
			continue
		}
...
func matchAimOnlyRecord(pay []byte, p, total int, slots map[uint32]bool) (int, uint32, bool) {
	...
	at, ok := aimOnlyCursorToI21(pay, p+bipedHeaderBits+bipedIndexBits*mc, total, idx)
	if !ok {
		return 0, 0, false
	}
	return at, slot, true
...
func aimOnlyCursorToI21(pay []byte, at, total int, idx []int) (int, bool) {
	var d componentDirs
	...
	for _, id := range idx {
		...
		case 4:
			at, ok = readBodyVitalityComponent(pay, at, total, &v)
		case 5:
			at, ok = readShieldVitalityComponent(pay, at, total, &v)
		default:
			return at, false
		}
		if !ok {
