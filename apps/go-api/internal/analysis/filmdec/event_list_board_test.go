...
func evbUnFilm(t *testing.T, dir, id string, tot *evbTotaux) {
	...
	t.Logf("V3b %s — %d trous >= 3 s · %d embarquements · %d sorties (base bande %d, %d slots)",
		id, len(trous), nb, ne, base, len(band))
}
...
// evbBande relève la bande de slots bipèdes et sa base (min), comme ScanFilmVehicleEvents.
func evbBande(dir string) (uint32, map[uint32]bool) {
