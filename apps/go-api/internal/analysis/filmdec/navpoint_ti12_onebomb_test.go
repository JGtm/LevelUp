...
// obSeg est un segment CONTIGU (trou <= tpTrouMaxMS) d'une serie d'un slot, sans exigence de
...
// obTousSegments segmente toutes les series par contiguite (trou <= tpTrouMaxMS).
...
func obSegmenter(slot uint32, s []ti12Ech) []obSeg {
	...
	for i := 0; i < len(s); {
		j := i
		for j+1 < len(s) && s[j+1].tMS-s[j].tMS <= tpTrouMaxMS {
			j++
		}
...
func obVueOrphelines(t *testing.T, series map[uint32][]ti12Ech, exps []int32) {
	...
	var fins []fin
	for slot, s := range series {
		for _, f := range tpMonteesContigues(slot, s) {
