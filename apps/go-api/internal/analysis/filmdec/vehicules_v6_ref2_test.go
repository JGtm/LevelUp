...
func (s *v6Ref2Stats) scanFilm(dir string) bool {
	...
	vehBand := worldObjectSlotBand(dir, n, VehicleTypeIndex)
	wpnBand := worldObjectSlotBand(dir, n, GroundWeaponTypeIndex)
	found := false
	for c := 1; c <= n; c++ {
...
func (s *v6Ref2Stats) sample(pay []byte, base uint32, band, vehBand, wpnBand map[uint32]bool) {
	r0, r1, r2, seatBit := v6BoardRefs(pay)
	s.boards++
	if r0.Present {
		s.r0Present++
		if band[base+r0.Index] {
			s.occInBand++
		}
	}
	if r1.Present {
