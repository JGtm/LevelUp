...
func v11ViseeUnFilm(t *testing.T, dir string) {
	...
	bande := bipedSlotBand(dir, chunks)
	if len(bande) == 0 {
		t.Logf("V11 %s : bande bipede vide", dir)
		return
	}
	fantome := map[uint32]bool{}
	for _, s := range v11BandeFantome(dir, len(bande)) {
		fantome[s] = true
	}
	...
	t.Logf("V11 VISEE %s — lectures i21 SANS i0 : bande bipede %d (%.1f par slot, %d slots) · "+
		...
		dir, len(vues), float64(len(vues))/float64(len(bande)), len(bande),
		len(bruit), float64(len(bruit))/float64(len(fantome)), len(fantome))
...
func v11LitViseesSansI0(dir string, bande map[uint32]bool) []v11Visee {
	var out []v11Visee
	for c := 1; c <= CountFilmChunks(dir); c++ {
...
func v11OccupationUnFilm(t *testing.T, dir string) {
	...
	vues := v11LitViseesSansI0(dir, bipedSlotBand(dir, chunks))
