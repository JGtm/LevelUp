...
// TestInferHomeSkillHistoryFromCanonical_Fallbacks verifie les deux fallbacks
// ajoutés pour les joueurs dont match_registry.is_ranked = false (données
// anciennement migrées) :
// - Fallback 1 : SkillSnapshot.RatingType == CSR -> match classé
// - Fallback 2 : playlist_name contient "ranked" -> match classé
func TestInferHomeSkillHistoryFromCanonical_Fallbacks(t *testing.T) {
	csrType := canonical.RatingTypeCSR
	ratingVal := 1200.0

	rows := []canonical.PlayerMatchRow{
		// Match 1 : is_ranked=false mais SkillSnapshot CSR present
		{
			Summary: canonical.MatchSummary{
				MatchID:  "m1",
				IsRanked: boolPtr(false),
				IsPvE:    boolPtr(false),
			},
			Enrichment: canonical.PlayerMatchEnrichment{
	...
	gotR, gotU := InferHomeSkillHistoryFromCanonical(rows)
	if !gotR {
		t.Error("Fallback 1: expected hasRanked=true when SkillSnapshot.RatingType=CSR")
	}
	if gotU {
		t.Error("Fallback 1: expected hasUnranked=false")
	}

	rows2 := []canonical.PlayerMatchRow{
		// Match 2 : is_ranked=false, pas de SkillSnapshot, mais playlist "Ranked Slayer"
		{
			Summary: canonical.MatchSummary{
				MatchID:  "m2",
				IsRanked: boolPtr(false),
				...
				Playlist: &canonical.AssetReference{
					Kind:         "playlist",
					DefaultLabel: "Ranked Slayer",
				},
			},
		},
	}
	gotR2, gotU2 := InferHomeSkillHistoryFromCanonical(rows2)
	if !gotR2 {
		t.Error("Fallback 2: expected hasRanked=true when playlist_name contains 'ranked'")
	}
	if gotU2 {
		t.Error("Fallback 2: expected hasUnranked=false")
	}
}
