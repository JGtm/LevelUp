...
type VipCrownScan struct {
	...
	// Identity est le pont slot statborg -> xuid (par les instants de mort).
...
func vipReconstructPeriods(events []objectiveevents.NamedEvent, identity map[int]string,
...
func vipSelectionOpenings(events []objectiveevents.NamedEvent, identity map[int]string) []vipRawPeriod {
	out := make([]vipRawPeriod, 0, len(events))
	for _, e := range events {
		if e.Stat != objectiveevents.StatVipSelected {
			continue
		}
		out = append(out, vipRawPeriod{slot: e.Slot, xuid: identity[e.Slot], t0MS: int64(e.TimeMS)})
	}
...
// INSTANTS DE MORT — aucune base. `own.DeathOffsetMS` cale l'horloge du fil des morts (et des
// evenements nommes, meme horloge) sur l'axe des frames.
