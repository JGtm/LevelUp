...
func TestBuildPickupsResolvesFamilyFromTitleCatalogs(t *testing.T) {
	...
	got, cov := buildPickups(in, pkClock(equipement), pickupInputs{occupant: nil, st: filmdec.BipedPickupStats{}, weaponKeys: armes, judge: nil})
	if len(got) != 3 {
...
func TestBuildPickupsKindPerClassLiteral(t *testing.T) {
	...
	for _, c := range cas {
		...
		got, _ := buildPickups(in, pkClock(nil), pickupInputs{occupant: nil, st: filmdec.BipedPickupStats{}, weaponKeys: nil, judge: nil})
		if len(got) != 1 {
...
func TestBuildPickupsUnknownFamilyIsCountedNotInvented(t *testing.T) {
	...
	got, cov := buildPickups(in, pkClock(map[uint32]string{0xbcabbe43: "grenade_frag"}), pickupInputs{occupant: nil, st: filmdec.BipedPickupStats{}, weaponKeys: map[uint32]string{}, judge: nil})
	if len(got) != 3 {
		t.Fatalf("publiés = %d, attendu 3", len(got))
	}
...
func TestPickupFamilyNeverCrossesCatalogs(t *testing.T) {
	...
	got, _ := buildPickups(in, pkClock(equipement), pickupInputs{occupant: nil, st: filmdec.BipedPickupStats{}, weaponKeys: armes, judge: nil})
	...
	gotSeul, cov := buildPickups(seul, pkClock(map[uint32]string{}), pickupInputs{occupant: nil, st: filmdec.BipedPickupStats{}, weaponKeys: map[uint32]string{orphelin: "hinf_ma40_ar"}, judge: nil})
	...
	gotArme, _ := buildPickups(arme, pkClock(map[uint32]string{orphelin: "grenade_frag"}), pickupInputs{occupant: nil, st: filmdec.BipedPickupStats{}, weaponKeys: map[uint32]string{}, judge: nil})
	if gotArme[0].Family != "" {
