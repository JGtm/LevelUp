...
func TestBuildWeaponPadsPublieUnSocleEtSesOccupations(t *testing.T) {
	...
	pads, picks, cov, _ := buildWeaponPads(PadScans{Weapons: scan}, pos, gwTestClock(), padCatalogs{})
	if len(pads) != 1 {
...
func TestBuildWeaponPadsPublieLeCycleQuandIlEstEtabli(t *testing.T) {
	scan, pos := gwTestPadScan(t)
	pads, _, cov, _ := buildWeaponPads(PadScans{Weapons: scan}, pos, gwTestClock(), padCatalogs{})
	c := pads[0].Cycle
	if c == nil {
...
func TestBuildWeaponPadsPublieLesEcartsQuIlNAPasPuPrendre(t *testing.T) {
	...
	pads, _, _, _ := buildWeaponPads(PadScans{Weapons: scan}, pos, gwTestClock(), padCatalogs{})
...
func TestGroundWeaponObjectsEcarteLesCreationsSansIdentite(t *testing.T) {
	...
	_, _, cov, _ := buildWeaponPads(PadScans{Weapons: scan}, nil, gwTestClock(), padCatalogs{})
...
func TestBuildWeaponPadsSansBalayageNePublieRien(t *testing.T) {
	pads, picks, cov, _ := buildWeaponPads(PadScans{Weapons: WorldObjectScan{}}, nil, gwTestClock(), padCatalogs{})
