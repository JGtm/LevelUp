...
func TestAttachEpisodeKills_FragDansLaFenetre(t *testing.T) {
	...
	attachEpisodeKills(eps, []EquipmentKillRef{{XUID: 111, TimeMS: 1500}}, occupantFige(slotXUID), 0, 100)
	if eps[0].K != 1 {
...
func TestAttachEpisodeKills_FragHorsFenetre(t *testing.T) {
	...
	slotXUID := map[uint32]uint64{512: 111}
	// frame 25 : après T1.
	attachEpisodeKills(eps, []EquipmentKillRef{{XUID: 111, TimeMS: 2500}}, occupantFige(slotXUID), 0, 100)
	if eps[0].K != 0 {
...
func TestAttachEpisodeKills_BornesExactesT0EtT1(t *testing.T) {
	...
	attachEpisodeKills(epsT0, []EquipmentKillRef{{XUID: 111, TimeMS: 1000}}, occupantFige(slotXUID), 0, 100) // frame 10
	if epsT0[0].K != 1 {
		t.Errorf("borne T0 : K = %d, attendu 1 (bornes incluses)", epsT0[0].K)
	}

	epsT1 := []EquipmentEpisode{ekEp(10, 20)}
	attachEpisodeKills(epsT1, []EquipmentKillRef{{XUID: 111, TimeMS: 2000}}, occupantFige(slotXUID), 0, 100) // frame 20
	if epsT1[0].K != 1 {
		t.Errorf("borne T1 : K = %d, attendu 1 (bornes incluses)", epsT1[0].K)
	}

	epsApresT1 := []EquipmentEpisode{ekEp(10, 20)}
	attachEpisodeKills(epsApresT1, []EquipmentKillRef{{XUID: 111, TimeMS: 2100}}, occupantFige(slotXUID), 0, 100) // frame 21
	if epsApresT1[0].K != 0 {
...
func TestAttachEpisodeKills_PorteurDifferentDuTueur(t *testing.T) {
	...
	eps := []EquipmentEpisode{ekEp(10, 20)}
	slotXUID := map[uint32]uint64{512: 111}
	attachEpisodeKills(eps, []EquipmentKillRef{{XUID: 222, TimeMS: 1500}}, occupantFige(slotXUID), 0, 100)
	if eps[0].K != 0 {
...
func TestAttachEpisodeKills_EpisodeFermeParLaMortNeCompterPasApres(t *testing.T) {
	...
	slotXUID := map[uint32]uint64{512: 111}
	attachEpisodeKills(eps, []EquipmentKillRef{{XUID: 111, TimeMS: 5000}}, occupantFige(slotXUID), 0, 100) // frame 50 > 42
	if eps[0].K != 0 {
...
func TestAttachEpisodeKills_OrigineNonNulleEtConversionHorloge(t *testing.T) {
	...
	attachEpisodeKills(eps, []EquipmentKillRef{{XUID: 111, TimeMS: 4500}}, occupantFige(slotXUID), 3000, 100)
	if eps[0].K != 1 {
...
func TestAttachEpisodeKills_ListesVidesNePaniquentPas(t *testing.T) {
	...
	attachEpisodeKills([]EquipmentEpisode{ekEp(0, 10)}, nil, occupantFige(map[uint32]uint64{512: 111}), 0, 100)
	attachEpisodeKills(nil, []EquipmentKillRef{{XUID: 111, TimeMS: 100}}, occupantFige(map[uint32]uint64{512: 111}), 0, 100)
}
...
func TestAttachAllEquipmentKills_OrigineNonEtablie(t *testing.T) {
	...
	read := attachAllEquipmentKills(eps, kills, occupantFige(slotXUID), nil, 100)
	if read {
