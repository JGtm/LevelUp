...
func TestCycleBatchPersister_NilQueueReturnsError(t *testing.T) {
	persister := NewCycleBatchPersister("halo_infinite", nil, nil, 0)
...
func TestCycleBatchPersister_EmptyBatchDrainOnly(t *testing.T) {
	...
	persister := NewCycleBatchPersister("halo_infinite", playerBy, q, 100*time.Millisecond)
...
func TestCycleBatchPersister_ParsingErrorIsNonFatal(t *testing.T) {
	q := setupTestBatchQueue(t)
	playerBy := mkPlayerMap("alice")
	persister := NewCycleBatchPersister("halo_infinite", playerBy, q, 200*time.Millisecond)
