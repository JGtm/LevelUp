...
func BatchComputeLUSR(ctx context.Context, playerDB, sharedDB *sql.DB, xuid string, force bool) (int, error) {
	return BatchComputeLUSRWithMedals(ctx, playerDB, sharedDB, xuid, nil, force)
}

// BatchComputeLUSRWithMedals calcule le LUSR pour tous les matchs non classés.
...
func BatchComputeLUSRWithMedals(ctx context.Context, playerDB, sharedDB *sql.DB, xuid string, medalExploitByMatch map[string]float64, force bool) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("BatchComputeLUSRWithMedals: %w", err)
	}
	if len(excluded) > 0 {
		before := len(matches)
		filtered := matches[:0]
		...
		matches = filtered
		slog.DebugContext(ctx, "BatchComputeLUSRWithMedals: matchs exclus filtrés",
