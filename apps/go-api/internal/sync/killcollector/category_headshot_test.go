...
//
//	games/halo_infinite/film/internal/facts/killsource   `Category` — l'énumération, chez le décodeur qui la
...
// `killscope.CategoryHeadshot` divergeait d'un caractère de `decfilm.CategoryHeadshot.Name()`,
...
func TestCategoryHeadshotEgaleAuDecodeur(t *testing.T) {
	if got, want := killscope.CategoryHeadshot, decfilm.CategoryHeadshot.Name(); got != want {
	...
	if killscope.CategoryHeadshot == decfilm.CategoryHeadshotMultiplier.Name() {
		t.Fatalf("killscope.CategoryHeadshot (%q) egale desormais decfilm.CategoryHeadshotMultiplier.Name() "+
