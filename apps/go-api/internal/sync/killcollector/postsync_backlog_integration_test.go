...
func TestBacklogAJour_SelectionOrdreEtJauge(t *testing.T) {
	...
	inscrirePasseFilm(t, db, "deja-decode", facts.Rev, killscope.ReadPathFilmWalk, "p1", t0)
	inscrireMatch(t, db, "via-credit", t0.AddDate(0, 2, 0), 0) // (3)
	inscrirePasseFilm(t, db, "via-credit", facts.Rev, killscope.ReadPathCreditBackfill, "p1", t0)
...
func TestBacklogAJour_PasseCouranteSupplanteLaPerimee(t *testing.T) {
	...
	inscrirePasseFilm(t, db, "redecode", facts.Rev, killscope.ReadPathFilmWalk, "p2", quand.Add(time.Hour))
