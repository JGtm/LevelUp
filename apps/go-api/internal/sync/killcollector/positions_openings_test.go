...
func registreSynthetique(positions []grammar.BipedPosition,
	...
	creations := make([]grammar.BipedCreation, 0, len(sieges))
	for i, s := range sieges {
		idx.ByXUID[slotXUID[s]] = i
		creations = append(creations, grammar.BipedCreation{
...
func bipedAt(slot uint32, tMS int64, x, y, z float32) grammar.BipedPosition {
...
func trajectoiresSynthetiques(finMS int64) []grammar.BipedPosition {
