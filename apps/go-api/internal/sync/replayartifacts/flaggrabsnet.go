...
// du jonglage (`objectives.NetFlagGrabs`) avec la fenetre declaree par le titre, et
...
func ProjeterPrisesNettes(matchID string, doc *replay.ReplayDocument, window time.Duration) persist.FlagGrabsNetBatch {
	...
	res := objectives.NetFlagGrabs(tracks, window)
...
func completerRosterAZero(doc *replay.ReplayDocument, mesures []objectives.FlagGrabsNetPlayer) []persist.FlagGrabsNetRow {
