...
// un reader snapshot-préféré (lecture découplée du B-swap, Phase 3 pilote) avec
// fallback live, SINGLETON par titre (cache de queriers :memory: partagé entre
// requêtes). Dégrade sur pdb.SharedReadDB() si cfg indisponible (tests). Le snapshot
// reconstruit l'intégralité des relations shared lues par MatchView (validé par le test
// de fidélité) ; les lectures média (SharedSocial) et player (ReadDB) restent live.
