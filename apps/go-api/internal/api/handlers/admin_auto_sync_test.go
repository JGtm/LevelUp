...
// buildProbeHandler crée un handler de test avec un cfg minimal et un
// scheduler vide. dbProfilesJSON contient le JSON de db_profiles.json (peut
// être vide → aucun joueur configuré).
func buildProbeHandler(t *testing.T, dbProfilesJSON string) (*handlers.AdminAutoSyncHandler, string) {
	t.Helper()
	repoRoot := t.TempDir()
...
func TestProbeTokens_NotDiscovered_DiscoveredFalse(t *testing.T) {
	...
	h, _ := buildProbeHandler(t, `{"version":"3.0","profiles":{"halo_infinite":{}}}`)
...
func TestProbeTokens_GamertagInConfigButNoPlayerDB_NotDiscovered(t *testing.T) {
	...
	h, _ := buildProbeHandler(t, profiles)
