...
func (m *mockDaemon) AddUserClient(_ context.Context, _ *auth_platform.UserTokens) error {
	return nil
}
...
func newWatcherRouter(t *testing.T, daemon watcher.DaemonController, provider auth_platform.TokenProvider) *chi.Mux {
	...
	r.Route("/watcher", func(r chi.Router) {
		// 4 routes migrées vers Huma (status, auth/{attempt_id}, auth/start, subscriptions).
		h.Mount(r)
	})
	return r
}
