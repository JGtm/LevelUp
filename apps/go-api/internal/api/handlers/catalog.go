...
func (h *CatalogHandler) PlaylistsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(r.Context(), w, http.StatusInternalServerError, "catalog_query_failed", err.Error())
		return
	}
	writeJSONCatalog(r.Context(), w, map[string]any{
		"title_slug": slug,
		"playlists":  playlists,
	})
}
...
func writeJSONCatalog(ctx context.Context, w http.ResponseWriter, body any) {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Cache-Control", "public, max-age=300")
	if err := json.NewEncoder(w).Encode(body); err != nil {
		writeError(ctx, w, http.StatusInternalServerError, "marshal_failed", err.Error())
	}
}
