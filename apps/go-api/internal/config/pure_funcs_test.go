...
func TestApplyFlagsMap_SetPython(t *testing.T) {
	ff := defaultFeatureFlags()
	applyFlagsMap(&ff, map[string]string{
		"career":  "python",
		"history": "PYTHON",
	})
	if ff.Career != BackendPython {
		t.Fatalf("expected python, got %s", ff.Career)
	}
	if ff.History != BackendPython {
		t.Fatalf("expected python, got %s", ff.History)
	}
	if ff.Explorer != BackendGo {
		t.Fatalf("expected go (unchanged), got %s", ff.Explorer)
	}
}

func TestApplyFlagsMap_UnknownSurface(t *testing.T) {
	ff := defaultFeatureFlags()
	applyFlagsMap(&ff, map[string]string{
		"nonexistent": "python",
	})
	...
	if !ff.AllOnGo() {
		t.Fatal("expected all go after unknown surface")
	}
}

func TestApplyFlagsMap_EmptyMap(t *testing.T) {
	ff := defaultFeatureFlags()
	applyFlagsMap(&ff, map[string]string{})
	if !ff.AllOnGo() {
		t.Fatal("expected all go after empty map")
	}
}

// ── surfaceFields ────────────────────────────────────────────────────────────

func TestSurfaceFields_AllSurfacesCovered(t *testing.T) {
	ff := defaultFeatureFlags()
	fields := ff.surfaceFields()
	for _, s := range AllSurfaces {
		if _, ok := fields[s]; !ok {
			t.Fatalf("surface %q missing from surfaceFields()", s)
		}
	}
}

func TestSurfaceFields_SameCountAsAllSurfaces(t *testing.T) {
	ff := defaultFeatureFlags()
	fields := ff.surfaceFields()
	if len(fields) != len(AllSurfaces) {
		t.Fatalf("expected %d fields, got %d", len(AllSurfaces), len(fields))
	}
}

func TestSurfaceFields_Mutability(t *testing.T) {
	ff := defaultFeatureFlags()
	fields := ff.surfaceFields()
	*fields[SurfaceCareer] = BackendPython
	if ff.Career != BackendPython {
		t.Fatal("expected mutation via pointer")
	}
}
