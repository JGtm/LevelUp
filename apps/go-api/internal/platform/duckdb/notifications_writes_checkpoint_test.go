...
func seedFlushedNotif(t *testing.T, db *duckdb.DB, id int64) {
	...
	if _, err := db.SQLDb().Exec(`
		...
		VALUES ('x', ?, 'c', 'info', 'k', 's', NOW())`, id); err != nil {
...
func TestSetNotificationRead_PersistsAfterReopen(t *testing.T) {
	...
	got := reopenNotifCount(t, socialDB,
		`SELECT COUNT(*) FROM player_notifications WHERE xuid='x' AND id=1 AND read_at IS NOT NULL`)
	if got != 1 {
...
func TestSetNotificationDelete_PersistsAfterReopen(t *testing.T) {
	...
	got := reopenNotifCount(t, socialDB,
		`SELECT COUNT(*) FROM player_notifications WHERE xuid='x' AND id=1`)
	if got != 0 {
		t.Errorf("notif présente après Delete + reopen (got %d want 0)", got)
	}
}

// TestNotificationRead_LegacyNoCheckpoint_LeavesWAL : prouve que l'oracle WAL
// discrimine — un UPDATE read_at SANS CHECKPOINT (ancien chemin) laisse le WAL
// non-vide (donnée à risque de quarantaine #7659). Si ce test échoue (WAL==0),
// l'oracle ne discrimine pas et les tests positifs ne garantissent rien.
func TestNotificationRead_LegacyNoCheckpoint_LeavesWAL(t *testing.T) {
	socialDB := newNotifSocialDB(t)
	seedFlushedNotif(t, socialDB, 1)

	// Écriture SANS CHECKPOINT (reproduit l'ancien OpenReadWrite + Exec).
	if _, err := socialDB.SQLDb().Exec(
		`UPDATE player_notifications SET read_at = NOW() WHERE xuid='x' AND id=1`); err != nil {
		t.Fatalf("update sans checkpoint: %v", err)
	}
	if sz := walSize(t, socialDB.Path()); sz == 0 {
		t.Errorf("WAL vide après UPDATE non-checkpointé (size=0) — l'oracle WAL ne discrimine pas")
	}
}
