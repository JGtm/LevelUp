...
// Test interne (package duckdb) pour lire les accesseurs duckMemoryLimit()/duckThreads().
package duckdb
...
func TestOpenReadWrite_ResourceLimitsAppliedNoTZ(t *testing.T) {
	...
	// threads = preuve robuste que le hook d'init tourne (valeur lue par l'accesseur
	// du paquet, même lecture d'env que le hook → insensible à un override d'env).
	var threads string
	...
	t.Logf("memory_limit=%q threads=%s (config effective : %s / %d)", mem, threads, duckMemoryLimit(), duckThreads())
}
