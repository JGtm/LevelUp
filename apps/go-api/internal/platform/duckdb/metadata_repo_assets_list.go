...
// Retourne un slice vide (sans erreur) si le titre est inconnu ou sans images indexées.
func (r *MetadataRepo) ListMapsByTitle(
	...
	if err != nil {
		return nil, fmt.Errorf("ListMapsByTitle(%s): %w", titleID, err)
	}
	defer rows.Close()
