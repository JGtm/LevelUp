...
// génération append-only par (match_id, xuid) ; kill_positions_latest = dernière version
// append-only par (match_id, killer_xuid, time_ms) (G.2, 2026-08-30 — ADR 0026, la table
// n'était pas encore convertie et se lisait brute jusqu'ici) ; LEFT JOIN tolère un kill sans
// arme / sans position (event partiel) sans le perdre.
//
...
// `v_weapon_kills` / `kill_positions_latest` cessent d'apparier sur une chaîne vide partagée par
// tous les bots du match.
...
LEFT JOIN kill_positions_latest kp
