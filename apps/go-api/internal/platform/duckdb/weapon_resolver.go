...
func resolveOffArsenalKeys(ctx context.Context, meta *DB, titleSlug string, keys []string) map[string]offArsenalMeta {
	...
	query := "SELECT w.weapon_key, COALESCE(w.class, '') AS class, " + labelExpr + " AS label" +
		" FROM weapons w" +
	...
	defer rows.Close()
	for rows.Next() {
		var key, class, label string
		if err := rows.Scan(&key, &class, &label); err != nil {
			continue
		}
		out[key] = offArsenalMeta{class: class, label: label}
	}
	return out
}
