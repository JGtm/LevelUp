...
// Un admin accède à tout ; un utilisateur standard uniquement à son propre xuid.
func CanAccessPlayer(enforced bool, user *domain.User, profileXUID string) bool {
	if !enforced {
		return true
	}
	if user == nil {
	...
	return user.XUID != "" && user.XUID == profileXUID
}
