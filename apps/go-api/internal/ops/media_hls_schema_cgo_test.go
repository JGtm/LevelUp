...
// les colonnes HLS (hls_path, transcode_status) requises par le worker de
// transcoding. Garde-rail : ces colonnes sont dédiées — `status` (sémantique
// 'active' du rail home) n'est pas réutilisé.
func TestEnsureMediaTables_HLSColumns(t *testing.T) {
	...
	for _, col := range []string{"hls_path", "transcode_status"} {
		var cnt int
		err := db.QueryRow(
