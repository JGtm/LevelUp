...
var horsArsenalHINF = []string{
	...
	"hinf_gungoose",
	// Mutilator : RETIRE LE 2026-09-13, comme son entree du 2026-09-10 le prescrivait. Son
	// identifiant filmshell (0xd791556542c9679f, mesure depuis le seed de `labels.go`) est
	// desormais catalogue : c est une arme de l arsenal comme les autres, elle se resout par
	// weapon_id et n a plus rien a faire dans la liste des cles SANS identifiant.
	// DOUBLE COMPTAGE REVERIFIE, comme la ligne retiree l exigeait : il n y a plus qu UNE voie
	// de comptage depuis la decision D11 du 2026-09-01 — le filtre `wi.weapon_key IS NULL` de
	// l ex-`resolveOffArsenalKeys` a ete supprime avec sa fonction (cf. l en-tete de ce fichier
	// et `platform/duckdb/weapon_resolver.go`, `resolveWeaponKeyDimensions`). Aucune seconde
	// voie ne peut donc recompter ses frags.
	"hinf_pelican",
	"hinf_phantom",
	"hinf_repulsor",
	"hinf_rockethog",
