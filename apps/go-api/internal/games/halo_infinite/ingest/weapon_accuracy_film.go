...
// (grammar.PairWeaponHits, methode PAR LE TIR, NOTE_ATTRIBUTION_ARME_TIR_2026-08-31).
//
...
func MapWeaponAccuracyFilm(
	...
	for i := range stats {
		...
		a := acc[k]
		if a == nil {
			a = &filmAccuracyAgg{buckets: make([]int, grammar.WeaponHitBucketCount())}
			acc[k] = a
