...
func carry(t0, t1 int, xuid string) types.FlagSpan {
	return types.FlagSpan{State: FlagSpanCarried, StartMS: t0, EndMS: t1, XUID: xuid}
}

func carryOpen(t0, t1 int, xuid string) types.FlagSpan {
	return types.FlagSpan{State: FlagSpanCarriedOpen, StartMS: t0, EndMS: t1, XUID: xuid}
}

func dropped(t0, t1 int) types.FlagSpan {
	return types.FlagSpan{State: FlagSpanDropped, StartMS: t0, EndMS: t1}
}

func home(t0, t1 int) types.FlagSpan {
	return types.FlagSpan{State: FlagSpanHome, StartMS: t0, EndMS: t1}
}
...
func TestNetFlagGrabs_JonglageReplie(t *testing.T) {
	...
	tr := types.FlagTrack{Team: 0, Spans: []types.FlagSpan{
	...
	res := NetFlagGrabs([]types.FlagTrack{tr}, fenetreRef)
...
func TestNetFlagGrabs_PasseDeMainCompte(t *testing.T) {
	...
	tr := types.FlagTrack{Team: 1, Spans: []types.FlagSpan{
		carry(1_000, 4_000, "A"),
		carry(4_300, 6_000, "B"),
		carry(6_300, 9_000, "A"),
	}}
...
func TestNetFlagGrabs_RepriseApresRetourAuSocleCompte(t *testing.T) {
	...
	sans := types.FlagTrack{Team: 0, Spans: []types.FlagSpan{
		carry(1_000, 4_000, "A"),
		dropped(4_000, 4_400),
		carry(4_400, 8_000, "A"),
	}}
	res2 := NetFlagGrabs([]types.FlagTrack{sans}, fenetreRef)
...
func TestNetFlagGrabs_CoupureA1400Et1600(t *testing.T) {
	...
	for _, c := range cas {
		t.Run(c.nom, func(t *testing.T) {
			tr := types.FlagTrack{Spans: []types.FlagSpan{
				carry(1_000, 5_000, "A"),
...
func TestNetFlagGrabs_DeuxDrapeauxNeSeReplientPas(t *testing.T) {
	...
	a := types.FlagTrack{Team: 0, Spans: []types.FlagSpan{carry(1_000, 4_000, "A")}}
	b := types.FlagTrack{Team: 1, Spans: []types.FlagSpan{carry(4_200, 7_000, "A")}}
	res := NetFlagGrabs([]types.FlagTrack{a, b}, fenetreRef)
	if brut, net := netOf(t, res, "A"); brut != 2 || net != 2 {
...
func TestNetFlagGrabs_FenetreAbsenteNePubliePas(t *testing.T) {
	tr := types.FlagTrack{Spans: []types.FlagSpan{carry(1_000, 4_000, "A"), carry(4_100, 6_000, "A")}}
	res := NetFlagGrabs([]types.FlagTrack{tr}, 0)
	if res.Measured {
...
func TestNetFlagGrabs_OrdreDEntreeIndifferent(t *testing.T) {
	...
	spans := []types.FlagSpan{
		carry(15_000, 20_000, "A"),
		carry(10_800, 14_000, "A"),
		carry(5_000, 10_000, "A"),
	}
	res := NetFlagGrabs([]types.FlagTrack{{Spans: spans}}, fenetreRef)
...
func TestNetFlagGrabs_NetJamaisSuperieurAuBrut(t *testing.T) {
	...
	for _, w := range []time.Duration{time.Second, fenetreRef, 8 * time.Second} {
		res := NetFlagGrabs([]types.FlagTrack{tr}, w)
		for _, p := range res.Players {
			if p.Net > p.Raw {
