...
func TestGrammarRevSuitLaGrammaire(t *testing.T) {
	...
	case revFigee == GrammarRev && empFigee != empreinte:
		t.Errorf("LA GRAMMAIRE A CHANGE SANS MONTEE DE REVISION.\n"+
			...
			"Faire monter grammar.GrammarRev (forme grammar-AAAA-MM-JJ), puis regenerer par -update-grammar-rev.\n"+
...
func racinesGrammaire(t *testing.T) []string {
	...
	// .../internal/games/halo_infinite/film/internal/grammar -> .../internal
	internalDir := filepath.Dir(filepath.Dir(filepath.Dir(filepath.Dir(filepath.Dir(filepath.Dir(ici))))))
	filmDir := filepath.Join(internalDir, "games", "halo_infinite", "film", "internal")
	return []string{
