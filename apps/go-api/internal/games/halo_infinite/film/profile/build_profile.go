...
// EXTRAIT DE `grammar/build_profile.go` AU LOT 2.5.b : les TABLES descendent ici, avec leurs
// provenances et leurs deux erreurs sentinelles. Ce qui LIT un film pour composer un profil de
// build (`BuildProfileFromFilm`) ou qui INSTALLE un decoupage sur un contexte de film
// (`InstallFilmFormatMPP`), et le compteur expvar qui publie un build refuse, restent en
// `grammar` : la table dit ce qui varie, le lecteur va le chercher.
...
// erreurTableProfil : une erreur sentinelle CONSTANTE de la table de profil.
//
// MEME FORME QUE `grammar.chunk00Error`, ET CE N EST PAS UNE COPIE DE MECANISME : les deux
// familles d erreurs ne disent pas la meme chose et ne vivent pas dans la meme couche. Celles
// de `grammar` nomment un DEFAUT DE LECTURE (`chunk_00` tronque, section d identification
// absente) ; celles-ci nomment une CLE ABSENTE DE LA TABLE, c est-a-dire un trou du profil.
// Une constante typee, plutot qu `errors.New`, pour qu elle reste comparable et immuable.
type erreurTableProfil string

// Error implemente error.
func (e erreurTableProfil) Error() string { return string(e) }
...
const ErrUnknownFormat = erreurTableProfil("filmdec: version de format de chunk_00 absente de la table de profil")
...
const ErrUnknownBuild = erreurTableProfil("filmdec: build absent de la table de profil")
...
// MPPPourFormat rend le decoupage du bloc `object-multiplayer-properties`
...
func MPPPourFormat(format int) (MPPWidths, bool) {
	switch format {
