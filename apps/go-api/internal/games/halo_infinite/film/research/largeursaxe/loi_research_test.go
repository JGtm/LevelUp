...
// versionne et des constantes. Il tourne sous `go test -tags=research ./internal/games/halo_infinite/film/research/largeursaxe/`.
...
import (
	...
	"levelup/go-api/internal/games/halo_infinite/film/research/largeursaxe"
...
func TestLoiDuRemplisseurEgaleLeCatalogue(t *testing.T) {
	...
	for nom, e := range cat.Maps {
		got := largeursaxe.LargeursAxe(bornesDe(e), largeursaxe.NiveauPosition)
		if got != e.AxisWidths {
...
func TestTableDefautEgaleLeReleveMemoire(t *testing.T) {
	...
	for niveau, attendu := range attendus {
		if got := largeursaxe.LargeursAxe(largeursaxe.BornesParDefautDuBuild, niveau); got != attendu {
...
func TestTableParIndexEgaleBazaar(t *testing.T) {
	...
	for niveau, attendu := range attendus {
		if got := largeursaxe.LargeursAxe(bornesDe(e), niveau); got != attendu {
...
func TestLargeurIndexDePlageSuitLeCatalogue(t *testing.T) {
	...
	for nb, attendu := range cas {
		if got := largeursaxe.LargeurIndexDePlage(nb); got != attendu {
	...
	if e.EffectiveRegionIndexBits() != largeursaxe.LargeurIndexDePlage(4) {
		t.Errorf("live fire : catalogue %d bits, loi a 4 plages %d bits",
			e.EffectiveRegionIndexBits(), largeursaxe.LargeurIndexDePlage(4))
	}
}
