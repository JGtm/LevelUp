	...
	for _, c := range fc.ChunkNumbers() {
		...
		for _, pk := range paquets {
			...
			for _, b := range keyframeBornesToutes(pay) {
				...
				if idx, _, ok := lireEquipeDuRecord(pay, b.Bit, reg); ok {
					out = append(out, idx)
				}
			}
			if len(out) > 0 {
		...
		for _, c := range fc.ChunkNumbers() {
			...
			for _, pk := range paquets {
				...
				for _, bo := range keyframeBornesToutes(pay) {
					...
					_, brut, ok := lireEquipeDuRecord(pay, bo.Bit, reg)
					if !ok {
						continue
					}
					bonnes++
					tr := WalkKeyframeFullState(pay, bo.Bit, reg)
...
func TestScanPlayerTeamsEntreeTronquee(t *testing.T) {
	...
	for _, n := range coupes {
		...
		for _, b := range bornes {
			if idx, brut, ok := lireEquipeDuRecord(tronque, b, reg); ok {
				lus++
