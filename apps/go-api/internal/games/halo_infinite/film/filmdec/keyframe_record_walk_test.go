...
// kfGramLayout porte les TROIS BASCULES de grammaire du record NEW dont les defauts viennent
// du chemin DELTA et n'ont jamais ete calibrees sur la table d'image-cle. Les exposer permet a
// la mesure de trancher — c'est l'oracle qui choisit, pas le lecteur. Les trois interrupteurs
// eux-memes vivent dans traverse.go (globaux de paquet) : ce type ne fait que les combiner, et
...
type kfGramLayout struct {
	...
	TailBits int
	// ArchDefaultState : router l'archetype vers son deserialiseur d'etat par defaut (true) ou
	// vers le Skip(0) historique (false). A false, aucune valeur n'est publiee — c'est le
	// temoin negatif de la voie elle-meme.
	ArchDefaultState bool
}

// kfGramLayouts est la matrice PROBEE par le balayage : les huit combinaisons des trois
// bascules. Le premier element est le defaut de production.
var kfGramLayouts = []kfGramLayout{
...
// String rend une etiquette lisible d'une combinaison.
func (l kfGramLayout) String() string {
...
func kfGramOne(reg *Registry, pay []byte, r KeyframeRec, want int, tal *kfGramTally) {
	...
	ch := ChainKeyframeRecords(pay, reg, tr.EndBit, want, r.Slot, contexteDInstrument())
	if !ch.Reached {
		tal.lost++
		tal.stops[ch.Stop]++
		return
...
func kfGramGlobalPass(reg *Registry, pays [][]byte) (parsed, hit, oracle int, stops [5]int) {
	for _, pay := range pays {
		recs, stop := WalkKeyframeRecords(pay, reg, contexteDInstrument())
		stops[stop]++
		parsed += len(recs)
