...
func TestScanBipedRecords_CaptureDirs(t *testing.T) {
	...
	got := ScanBipedRecords(w.buf, NewSlotBand(map[uint32]bool{slot: true}), cliffLayout, opt, ContexteParDefaut())
	if len(got) != 1 {
	...
	off := scanOptWorld()
	plain := ScanBipedRecords(w.buf, NewSlotBand(map[uint32]bool{slot: true}), cliffLayout, off, ContexteParDefaut())
...
func TestScanRecordDirs_StopsOnUnknownComponent(t *testing.T) {
	pay := make([]byte, 64)
	out, vit := scanRecordDirs(NewBitReader(pay), 0, len(pay)*8, []int{0, 7, 21}, dirsGrammar{})
...
func TestScanRecordVitals_ReachedThroughAngularVelocity(t *testing.T) {
	...
	_, vit := scanRecordDirs(NewBitReader(w.buf), 0, len(w.buf)*8, []int{0, 3, 4, 5}, dirsGrammar{})
	if !vit.HasBody || !vit.HasShield {
