...
func TestScanBipedAimRecordsGrammaire(t *testing.T) {
	...
	got := ScanBipedAimRecords(aimRecord(slot, 1, 0, []int{21, 25}, 1234, 1100), bande, ContexteParDefaut())
	if len(got) != 1 {
...
func TestScanBipedAimRecordsTemoins(t *testing.T) {
	...
	for _, cas := range []struct {
		...
		{"slot hors bande", aimRecord(slot, 1, 0, []int{21, 25}, 1234, 1100), NewSlotBand(map[uint32]bool{999: true})},
		...
		if got := ScanBipedAimRecords(cas.pay, cas.band, ContexteParDefaut()); len(got) != 0 {
...
func TestScanBipedAimRecordsPrecede(t *testing.T) {
	...
	w2.put(0, 64)
	got := ScanBipedAimRecords(w2.b, NewSlotBand(map[uint32]bool{slot: true}), ContexteParDefaut())
