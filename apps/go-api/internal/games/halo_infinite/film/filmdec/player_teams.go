...
func ScanPlayerTeams(fc *FilmContext) (map[int]int, TeamScanReport) {
	...
	for _, c := range fc.ChunkNumbers() {
		...
		for _, pk := range paquets {
			...
			scanPaquetEquipes(pk.Payload(raw), reg, &rep, parEntite, parIndex)
		}
	}
...
func scanPaquetEquipes(pay []byte, reg *Registry, rep *TeamScanReport,
	parEntite, parIndex map[int]map[int]int) {
	porteur := false
...
func lireEquipeDuRecord(pay []byte, recBit int, reg *Registry) (idx, brut int, ok bool) {
	tr := WalkKeyframeFullState(pay, recBit, reg)
