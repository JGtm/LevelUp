...
func v6TryVehicleEventAt(
	pay []byte, b, shift int, base uint32, band SlotBand,
) (v6Candidate, bool) {
	total := len(pay) * 8
...
func v6BandOf(dir string, n int) (SlotBand, uint32) {
	chunks := make([]int, 0, n)
	for i := 1; i <= n; i++ {
...
func (m *v6MarcheStats) scanPacket(pay []byte, id string, base uint32, band SlotBand) {
	m.packets++
	hits := 0
	maxBit := v6ScanMaxBit
