...
func ScanBipedAimOnly(fc *FilmContext) ([]BipedAim, error) {
	...
	for _, c := range nums {
		...
		for _, pk := range pks {
			...
			for _, a := range ScanBipedAimRecords(pk.Payload(data), band, fc.ProfilDeBalayage()) {
...
func ScanBipedAimRecords(payload []byte, slots SlotBand, prof ProfilDeBalayage) []BipedAim {
	total := len(payload) * 8
	var out []BipedAim
