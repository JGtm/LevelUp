...
// cache).
func ReadFilmIdentity(chunk0 []byte) (profile.FilmIdentity, error) {
	if looksZlib(chunk0) {
		return profile.FilmIdentity{}, ErrRegistryStillCompressed
	}
	reg := parseRegistry(chunk0)
	if reg.Truncated {
		return profile.FilmIdentity{}, ErrChunk00Truncated
	}
	finRegistre := registryEntryBase + len(reg.Archetypes)*archetypeBlockSize
	buildOff, ok := chercherChaineBuild(chunk0, finRegistre)
	if !ok {
		return profile.FilmIdentity{}, ErrNoFilmIdentity
	}
