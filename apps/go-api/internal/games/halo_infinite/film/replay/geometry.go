...
//
// UNE VALEUR ILLISIBLE REND UNE [LigneDeGeometrieError] (fichier, ligne, colonne) au lieu d'être
// sautée ou lue 0 : c'est l'appelant qui journalise, puis dégrade en « sans props ».
func LoadGeometry(mapDir, typesDir string) ([]MapObject, int, error) {
	...
	lignes, err := readCSV(filepath.Join(mapDir, MapObjectsFile))
	...
	for _, l := range lignes {
		// UNE VALEUR ILLISIBLE EST UNE ERREUR NOMMEE (lot J2.10, constat RA1-7) : la ligne n est
		// plus sautee en silence, ni sa coordonnee lue 0. Les coordonnees ne sont lues que pour
		// un type dessinable, comme avant : l ordre des controles n a pas change.
		...
		o := MapObject{TypeID: typeID, DX: round2(ext[0]), DY: round2(ext[1])}
		if err := l.position(&o); err != nil {
			return nil, 0, err
		}
		out = append(out, o)
	}
