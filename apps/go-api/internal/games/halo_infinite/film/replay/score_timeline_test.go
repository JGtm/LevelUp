...
func statRec(timeMS, slot, round int, comps map[int]objectives.StatValue) objectives.StatRecord {
	return objectives.StatRecord{TimeMS: timeMS, Slot: slot, Round: round, Comps: comps}
}
...
func modeRamp(slot, round, startMS, stepMS int, values ...int64) []objectives.StatRecord {
	out := make([]objectives.StatRecord, 0, len(values))
	for i, v := range values {
		out = append(out, statRec(startMS+i*stepMS, slot, round,
			map[int]objectives.StatValue{0: {A: v}}))
	}
	return out
}
...
func coreLine(slot, round, timeMS int, kills, deaths, assists, personal int64) []objectives.StatRecord {
	return []objectives.StatRecord{
		statRec(timeMS, slot, round, map[int]objectives.StatValue{
			1: {B: personal},
			2: {A: kills, B: deaths},
			3: {A: assists},
		}),
...
func TestScoreTimelineRoundsAndTotal(t *testing.T) {
	...
	tl, cov := buildScoreTimeline(&ScoreInput{Records: recs}, nil, testClock(), nil)
...
func TestScoreTimelineSubtractsOrigin(t *testing.T) {
	...
	tl, _ := buildScoreTimeline(&ScoreInput{Records: recs}, nil, testClock(), nil)
	if tl == nil || len(tl.Teams) != 1 {
...
func TestScoreTimelineTargetScore(t *testing.T) {
	recs := func() []objectives.StatRecord {
	...
	tl, _ := buildScoreTimeline(&ScoreInput{Records: recs(), TargetScore: 3}, nil, testClock(), nil)
	if tl.TargetScore == nil || *tl.TargetScore != 3 {
		t.Fatalf("cible = %v, attendu 3 (finals 3 et 2, cible respectee)", tl.TargetScore)
	}

	tl, _ = buildScoreTimeline(&ScoreInput{Records: recs(), TargetScore: 2}, nil, testClock(), nil)
...
func TestScoreTimelinePublishesOnlyPairedPlayers(t *testing.T) {
	...
	tl, _ := buildScoreTimeline(in, nil, testClock(), nil)
	if len(tl.Players) != 3 {
...
func TestScoreTimelinePlayerCounters(t *testing.T) {
	tl, _ := buildScoreTimeline(fragsIdentityInput(), nil, testClock(), nil)
	var p *PlayerScore
...
func TestScoreTimelinePropagatesTruncation(t *testing.T) {
	recs := modeRamp(6, 0, 2_000, 1_000, 1, 2, 3)
	_, cov := buildScoreTimeline(&ScoreInput{Records: recs, Truncated: true}, nil, testClock(), nil)
	if !cov.Truncated {
...
func TestScoreTimelineWithoutInput(t *testing.T) {
	tl, cov := buildScoreTimeline(nil, nil, testClock(), nil)
	if tl != nil || cov != nil {
...
func TestScoreTimelineEmptyFilmKeepsCoverage(t *testing.T) {
	tl, cov := buildScoreTimeline(&ScoreInput{}, nil, testClock(), nil)
	if tl != nil {
...
func fragsIdentityInput() *ScoreInput {
	...
	recs = append(recs, statRec(5_000, 6, 0, map[int]objectives.StatValue{2: {A: 5}}))
	recs = append(recs, statRec(5_000, 8, 0, map[int]objectives.StatValue{2: {A: 7}}))
...
func TestScoreTimelinePlayerCountersHaveNoRepeats(t *testing.T) {
	tl, cov := buildScoreTimeline(repeatedCountersInput(), nil, testClock(), nil)
...
func repeatedCountersInput() *ScoreInput {
	...
	for i, kills := range []int64{1, 2, 3} {
		recs = append(recs, statRec(3_000+i*500, 10, 0, map[int]objectives.StatValue{
			1: {B: int64(100 * (i + 1))},
	...
	return &ScoreInput{
		Records: recs,
		Lines:   []objectives.PlayerLine{{XUID: "x10", Kills: 3, Deaths: 2, Assists: 1}},
	}
}
