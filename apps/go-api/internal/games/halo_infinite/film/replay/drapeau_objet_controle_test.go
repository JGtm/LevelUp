...
//	  go test ./internal/games/halo_infinite/film/replay/ -run DrapeauObjetControle -v -timeout 60m
...
func objDrapeauRefs(t *testing.T, id string, d objDoc, step uint64) []objDrapeauRef {
	...
	// ajoute.
	idx, _ := tracksByXUID(d.doc.Tracks, objDrapeauPontDuDocument(d.doc.Tracks), nil, nil)
