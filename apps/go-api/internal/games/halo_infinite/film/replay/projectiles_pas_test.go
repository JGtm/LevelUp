...
func projPas(origine uint64, repos bool, xs ...float32) grammar.ProjectileTrack {
	pts := make([]grammar.ProjectileSample, len(xs))
	for i, x := range xs {
		pts[i] = grammar.ProjectileSample{
			TimestampUS: origine + uint64(i)*100_000, X: x, Y: 0, Z: 0,
		}
	}
	pts[len(pts)-1].AtRest = repos
	return grammar.ProjectileTrack{Slot: 4096, Gen: 1, Pts: pts}
}
...
func TestVolCoupeAuPremierPasImpossible(t *testing.T) {
	...
	tracks := []grammar.ProjectileTrack{projPas(1_000_000, true, 0, 1, 2, 42, 43, 44)}
...
func TestVolLisibleResteIntactEtGardeSonRepos(t *testing.T) {
	...
	tracks := []grammar.ProjectileTrack{projPas(1_000_000, true, 0, 2, 4, 6, 8)}
	out, _, tronquees := buildProjectiles(tracks, 1_000_000, 100_000)
...
func TestVolCoupeTropTotNEstPasPublie(t *testing.T) {
	...
	tracks := []grammar.ProjectileTrack{projPas(1_000_000, true, 0, 60, 61, 62)}
	out, _, tronquees := buildProjectiles(tracks, 1_000_000, 100_000)
	if len(out) != 0 {
...
func TestCouvertureDesProjectilesRemonteLesTronquees(t *testing.T) {
	...
	opt := Options{Projectiles: []grammar.ProjectileTrack{
