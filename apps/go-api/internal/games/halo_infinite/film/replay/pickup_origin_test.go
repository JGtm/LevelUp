...
func poJuge(points []MapSpawnPoint, etat string, pos []grammar.BipedPosition,
	dropped []droppedSpot,
) *pickupOriginJudge {
	j := &pickupOriginJudge{
...
func poPos(slot uint32, ts uint64, x, y, z float32) grammar.BipedPosition {
	return grammar.BipedPosition{Slot: slot, TimestampUS: ts, X: x, Y: y, Z: z, HasWorld: true}
}
