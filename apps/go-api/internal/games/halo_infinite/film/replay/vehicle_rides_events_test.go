...
func TestVehicleRideFromEpisodeAnchors(t *testing.T) {
	...
	cases := []struct {
		name       string
		start, end grammar.BipedPosition
		open       bool
		want       bool
	}{
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			bySlot := map[uint32][]grammar.BipedPosition{occ: {c.start, c.end}}
...
func TestVehicleRideTerminalSilenceClosesAtLifeEnd(t *testing.T) {
	...
	bySlot := map[uint32][]grammar.BipedPosition{occ: {vehPos(occ, 2_000_000, 10.5, 10)}}
...
func TestVehicleEpisodeReappearanceClosesOpenEnd(t *testing.T) {
	...
	_, r, resolved, ok := vehicleRideFromEpisode(eps[0], map[uint32][]grammar.BipedPosition{occ: pts}, in)
	if !ok {
...
// TestOwnerReportXuidAtSuitLOccupantDansLeTemps — LE CORRECTIF P1-7 (audit du 2026-09-06).
//
// `SlotXUID` est une identite UNIQUE PAR SLOT pour tout le match : `ownersFromLives` garde la
// PREMIERE vie nommee et jette les suivantes en collision, et `buildLifeSpans` trie par slot
// puis chronologiquement. Un lecteur qui interroge le pont SANS son instant credite donc le
// PREMIER occupant — et `VehicleRide.XUID` est ce qui donne sa COULEUR au vehicule (le client
// joint xuid -> equipe -> couleur) : le sprite prenait l equipe du mauvais joueur, la fiche
...
func TestOwnerReportXuidAtSuitLOccupantDansLeTemps(t *testing.T) {
	own := OwnerReport{
		...
		lives: []lifeSpan{
			{slot: 7, from: 1_000_000, to: 10_000_000, xuid: 111},
