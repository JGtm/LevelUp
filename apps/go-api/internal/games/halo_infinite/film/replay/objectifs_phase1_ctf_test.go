...
// le mode (`document.go`, en-tete) : `objectives.ObjectiveTypeOf` prend un
...
// (`objectives.FlagFilmSignals`) : bursts > 0, captures > 0, captures <= bursts, vols > 0.
// La regle et son corpus gele vivent dans `objectives/flagfilm.go` ; cet instrument est ce
...
// les tests de verite terrain d'`objectives` (extract_test.go, named_test.go) et dans le
...
var objModeCorpus = []struct {
	...
	{"64e8adfa", objectives.ObjectiveTypeFlag},
	{"530820e5", objectives.ObjectiveTypeFlag},
	{"53ce4390", objectives.ObjectiveTypeFlag},
	{"0f9550e5", objectives.ObjectiveTypeFlag},
	{"1bc77d2e", objectives.ObjectiveTypeFlag},
	{"24dbb67d", objectives.ObjectiveTypeSkull},
	{"696a9d7c", objectives.ObjectiveTypeZone},
	{"7344d24f", objectives.ObjectiveTypeZone},
	{"0a247154", objectives.ObjectiveTypeHill},
	{"01e1f945", objectives.ObjectiveTypeHill},
	{"606d9844", objectives.ObjectiveTypeHill},
	{"8076f97f", objectives.ObjectiveTypeHill},
	{"bcb6d393", objectives.ObjectiveTypeFlag},
	{"000d5950", ""},
	{"00162144", ""},
}
...
func TestObjectifsPhase1DiscriminantCTF(t *testing.T) {
	...
	for _, f := range objModeCorpus {
		...
		t.Logf("%s (mode %q) : bursts %d ; table DRAPEAU appliquee -> grabs %d, steals %d, "+
			...
			f.ID, f.Mode, sig.Bursts, compte[objectives.StatFlagGrabs],
			compte[objectives.StatFlagSteals], compte[objectives.StatFlagCaptures],
			compte[objectives.StatFlagReturns], compte[objectives.StatFlagCarriersKilled],
			verdict, attendu)
		switch {
...
func objCompteStats(evs []objectives.NamedEvent) map[string]int {
	out := map[string]int{}
	for _, e := range evs {
		out[e.Stat]++
	}
