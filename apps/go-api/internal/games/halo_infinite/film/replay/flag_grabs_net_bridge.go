...
// nettes (`objectiveevents.NetFlagGrabs`).
//
...
// [objectiveevents.NetFlagGrabs], en MILLISECONDES.
//
...
//	                                  trompeur, cf. objectiveevents/flagfilm.go
...
func FlagTracksOf(doc *ReplayDocument) (tracks []objectiveevents.FlagTrack, openings int, ok bool) {
	...
	out := make([]objectiveevents.FlagTrack, 0, len(doc.FlagCarries))
	for _, fc := range doc.FlagCarries {
		tr := objectiveevents.FlagTrack{Team: fc.Team, Spans: make([]objectiveevents.FlagSpan, 0, len(fc.Spans))}
		for _, sp := range fc.Spans {
			x := ""
			if sp.XUID != nil {
				x = *sp.XUID
			}
			tr.Spans = append(tr.Spans, objectiveevents.FlagSpan{
