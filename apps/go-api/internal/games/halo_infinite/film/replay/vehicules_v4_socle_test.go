...
func v4Decode(t *testing.T, root string, f v0Film) (v4Ctx, bool) {
	...
	_, pst := decodeFilmPlacementsDir(ctx.dir, &wr)
	ctx.scan = decodeFilmVehicleScanDir(ctx.dir, &wr, pst.Calibration.Widths)
	if !ctx.scan.Scanned {
	...
	ctx.lives = vehicleLives(ctx.scan.Keyframes, nil)
