...
// equipment_placement_ends_test.go — la fin d'affichage observée des poses (schéma 28).
//
...
func pePose(life grammar.EquipmentLifeKey, t0US uint64) grammar.EquipmentPlacement {
	return grammar.EquipmentPlacement{Life: life, T0US: t0US, T1US: t0US}
}

func TestPlacementEndsDisparitionBornee(t *testing.T) {
	life := grammar.EquipmentLifeKey{Slot: 900, Gen: 1}
	raw := []grammar.EquipmentPlacement{pePose(life, 2_000_000)}
	census := grammar.WorldObjectKeyframes{
		TimesUS: []uint64{5_000_000, 9_000_000, 13_000_000},
		SeenUS:  map[grammar.EquipmentLifeKey][]uint64{life: {5_000_000}},
	}
...
func TestPlacementEndsEncoreRecenseeALaFin(t *testing.T) {
	...
	census := grammar.WorldObjectKeyframes{
		...
		SeenUS:  map[grammar.EquipmentLifeKey][]uint64{life: {5_000_000, 9_000_000}},
	}
	ends := placementEnds(raw, census, peClock())
...
func TestPlacementEndsLaPoseSuivanteFermeLaVie(t *testing.T) {
	...
	census := grammar.WorldObjectKeyframes{
		...
		SeenUS: map[grammar.EquipmentLifeKey][]uint64{life: {5_000_000, 12_000_000, 16_000_000}},
	}
	ends := placementEnds(raw, census, peClock())
