...
func TestPlanHLS_MultiTrackUniformAAC(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			plan, err := planHLS(tc.streams, false)
			if err != nil {
				t.Fatalf("planHLS: %v", err)
			}
			if len(plan.Audios) != 3 {
...
func TestPlanHLS_Errors(t *testing.T) {
	if _, err := planHLS([]AVStreamDetail{{CodecType: "audio", CodecName: "opus"}}, false); err == nil {
		t.Error("planHLS sans vidéo: erreur attendue")
	}
	if _, err := planHLS([]AVStreamDetail{{CodecType: "video", CodecName: "h264"}}, false); err == nil {
