...
func TestCareerLiveCache_ProgressMiss_ThenHit(t *testing.T) {
	...
	if _, hit := cache.GetProgress("xuid1"); hit {
		t.Fatal("miss expected on first Get")
	}
	cache.PutProgress("xuid1", &syncpkg.CareerRankData{CurrentRank: 42, CurrentXP: 1000})
	got, hit := cache.GetProgress("xuid1")
	if !hit {
...
func TestCareerLiveCache_ProgressExpires(t *testing.T) {
	...
	cache.PutProgress("xuid1", &syncpkg.CareerRankData{CurrentRank: 1})
...
func TestCareerLiveCache_CustomizationExpires(t *testing.T) {
	cache, clk := newTestCache(t, 5*time.Minute, 6*time.Hour)
	cache.PutCustomization("xuid1", &syncpkg.SpartanCustomizationData{SpartanID: "SR-001"})

	clk.Advance(5 * time.Hour)
	if _, hit := cache.GetCustomization("xuid1"); !hit {
...
func TestCareerLiveCache_DistinctXUIDs(t *testing.T) {
	cache, _ := newTestCache(t, 5*time.Minute, 6*time.Hour)
	cache.PutProgress("a", &syncpkg.CareerRankData{CurrentRank: 1})
	cache.PutProgress("b", &syncpkg.CareerRankData{CurrentRank: 2})
	if _, hit := cache.GetProgress("c"); hit {
		t.Fatal("miss expected for unknown xuid")
	}
	a, hitA := cache.GetProgress("a")
	b, hitB := cache.GetProgress("b")
...
// goroutines appelant DoProgress simultanément pour le même xuid voient
...
func TestCareerLiveCache_DoProgress_SingleflightDedupes(t *testing.T) {
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(idx int) {
			defer done.Done()
			start.Wait()
			results[idx], errs[idx] = cache.DoProgress("xuid-sf", func() (*syncpkg.CareerRankData, error) {
...
func TestCareerLiveCache_DoProgress_ErrorNotCached(t *testing.T) {
	...
	_, err := cache.DoProgress("xuid-err", func() (*syncpkg.CareerRankData, error) {
		calls++
		return nil, wantErr
	})
	if err != wantErr {
	...
	got, err := cache.DoProgress("xuid-err", func() (*syncpkg.CareerRankData, error) {
		calls++
...
func TestCareerLiveCache_DoCustomization_SingleflightDedupes(t *testing.T) {
	...
	for i := 0; i < concurrency; i++ {
		...
		go func() {
			defer done.Done()
			start.Wait()
			_, _ = cache.DoCustomization("xuid-sf-c", func() (*syncpkg.SpartanCustomizationData, error) {
				atomic.AddInt32(&calls, 1)
				time.Sleep(10 * time.Millisecond)
