...
// TestKillSource_LabelENPropagates (V2.1, D2+D3, 2026-08-29) : LabelEN suit EXACTEMENT
// le même trajet que Label sur la 3e provenance (KillSourceClassRow.LabelEN ->
// acc.labelsEN -> perWeaponRoles -> FragRoleEntry.LabelEN), indépendamment de Label —
// un objet peut porter les deux, l'un des deux, ou aucun (le repli croisé entre les deux
// locales est un choix WEB, cf. fragRoleLabel.ts, jamais fait côté Go).
func TestKillSource_LabelENPropagates(t *testing.T) {
	sources := []port.KillSourceClassRow{
		{XUID: "x1", WeaponKey: "hinf_coil_kinetic", Class: domain.FragClassEnvironmental,
			Label: "Bobine à fusion UNSC", LabelEN: "UNSC Fusion Coil", Kills: 3},
		// LabelEN seedé sans Label (weapon_names.toml partiellement traduit) : les deux
		// champs restent INDÉPENDANTS, aucun repli côté Go.
		{XUID: "x1", WeaponKey: "hinf_coil_plasma", Class: domain.FragClassEnvironmental,
			Label: "", LabelEN: "Plasma Coil", Kills: 1},
	}
	fd := Build(nil, sources, domain.FragKillTypeCounts{Total: 10}, false)
	env := classByKey(fd)[domain.FragClassEnvironmental]
	if len(env.Roles) != 2 {
		t.Fatalf("%d objets au niveau 2, want 2 : %+v", len(env.Roles), env.Roles)
	}
	byRole := map[string]domain.FragRoleEntry{}
	for _, r := range env.Roles {
		byRole[r.Role] = r
	}
	kinetic, ok := byRole["hinf_coil_kinetic"]
	if !ok || kinetic.Label != "Bobine à fusion UNSC" || kinetic.LabelEN != "UNSC Fusion Coil" {
		t.Errorf("hinf_coil_kinetic = %+v, want Label=Bobine à fusion UNSC / LabelEN=UNSC Fusion Coil", kinetic)
	}
	plasma, ok := byRole["hinf_coil_plasma"]
	if !ok || plasma.Label != "" || plasma.LabelEN != "Plasma Coil" {
		t.Errorf("hinf_coil_plasma = %+v, want Label vide / LabelEN=Plasma Coil (pas de repli côté Go)", plasma)
	}
}
