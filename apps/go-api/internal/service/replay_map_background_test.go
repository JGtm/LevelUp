...
// grammar.NormalizeMapName, comme le build. CE RABOTAGE A ÉTÉ RETIRÉ, et pas par confort — il
...
func TestMapBackground_TousLesModulesDuCatalogue(t *testing.T) {
	...
	res := title.NewPathResolver(root)
	cat, err := grammar.LoadMapQuantCatalog(res.MapQuantBoundsPath(title.DefaultSlug))
	if err != nil {
