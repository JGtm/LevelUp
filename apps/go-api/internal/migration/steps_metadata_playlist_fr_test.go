...
// asset_id qui n'avaient qu'une ligne en-US (cas où le sync n'a stocké qu'une
// langue).
...
// TestApplyPlaylistFRSeeds_Idempotent : exécuter deux fois donne le même
// résultat.
...
// TestApplyPlaylistFRSeeds_NoAssetTranslationsTable : robustesse — la migration
// doit échouer proprement (ou être no-op) si la table n'existe pas encore.
func TestApplyPlaylistFRSeeds_NoAssetTranslationsTable(t *testing.T) {
	...
	defer db.Close()
	// Pas de CREATE TABLE — la migration doit retourner une erreur descriptive.
