...
// setupStreakTestDB crée une player DB avec (optionnellement) la table `streak`
// legacy + N rows, puis laisse le caller appliquer les migrations TargetPlayer.
func setupStreakTestDB(t *testing.T, withLegacyData bool) *sql.DB {
	...
	if withLegacyData {
		// Schéma legacy identique à create_progression_player_schema.
		if _, err := db.Exec(`
			CREATE TABLE streak (
...
// TestMigration_StreakAppendOnly_LatestViewTieBreak : 2 versions du même id avec
// le MÊME written_at → la vue doit retourner celle au tech_id le plus grand
// (le dernier insert). C'est la raison d'être du tie-break `tech_id DESC`.
func TestMigration_StreakAppendOnly_LatestViewTieBreak(t *testing.T) {
	...
	for _, length := range []int{1, 2} { // 2e insert = tech_id plus grand
