...
func TestNoARTSurfaceIndexInMigrations(t *testing.T) {
	...
	internalDir := filepath.Dir(dir) // apps/go-api/internal/migration -> apps/go-api/internal
	...
	// Scanne le registre GLOBAL (internal/migration) ET les migrations TITLE-OWNED
	...
	for _, root := range roots {
		if _, statErr := os.Stat(root); statErr != nil {
			continue // racine absente → skip
		}
		if walkErr := filepath.Walk(root, scan); walkErr != nil {
