 ...
 * data-driven via weaponRoleInsight, qui lit les rôles registre (kills_by_role) — donnée
 * inchangée. Le sunburst et le breakdown vivent dans components/charts (partagés) ; cette
 * carte porte uniquement la composition + le coach spécifiques à Synthesis.
 */
...
import type { FragDistribution, SynthesisRoleKillEntry, SynthesisWeaponKillEntry } from '@/lib/api/types'
...
export function SynthesisFragCard({ distribution, killsByRole, weapons }: Props) {
  const appLocale = useAppShellStore((s) => s.locale)

  // Insight coach data-driven (angle mort armes lourdes / sur-dépendance). Lit les
  // rôles registre (kills_by_role) — inchangé, sur l'arsenal réel (weaponRoleInsight
  // exclut déjà les rôles hors-combat).
  const insight = weaponRoleInsight(killsByRole)
