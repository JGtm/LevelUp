...
describe('buildIssueUrl — base', () => {
  ...
  // Bilan fork ChaseWoodhams 2026-09-11, point 6 : le corps de l'issue était
  // entièrement en dur en FR — un utilisateur en locale EN recevait un corps
  // d'issue GitHub en français. Garde-rail de non-régression.
  it("locale en → corps de l'issue en anglais (titre, sections, footer)", () => {
    const r = buildIssueUrl({
      title: 'broken thing',
      description: '',
      context: baseContext,
    ...
    const url = new URL(r.url)
    expect(url.searchParams.get('title')).toBe('[Bug] broken thing')
    expect(r.body).toContain('## Description')
    ...
    expect(r.body).toContain('## Recent failed requests')
    expect(r.body).toContain('_(no description provided)_')
    expect(r.body).toContain('Auto-generated by the feedback drawer')
    // Aucun littéral FR résiduel des en-têtes/labels traduits.
    expect(r.body).not.toContain('Contexte')
    expect(r.body).not.toContain('Environnement client')
    expect(r.body).not.toContain('Filtres actifs')
    expect(r.body).not.toContain('Auto-généré')
  })
})
