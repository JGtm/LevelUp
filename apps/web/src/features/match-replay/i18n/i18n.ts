...
export const REPLAY_TEXT: Record<ReplayLocale, ReplayText> = {
  fr: {
    ...
    seatVacant: 'Place libre',
    seatVacantHint: "Personne ne tient cette place à cet instant : elle attend son prochain occupant.",
    seatNotSpawned: 'Pas encore apparu',
    ...
    exportFormat: 'Format',
    exportFormatOptionFmt: (name, width, height) => `${name} (${width} × ${height})`,
    exportFraming: 'Cadrage',
    exportFramingWhole: 'Carte entière',
    exportFramingCurrentFmt: (zoom) => `Cadrage actuel (${String(zoom).replace('.', ',')}x)`,
    exportStart: 'Exporter',
    exportCancel: 'Annuler',
    exportClose: 'Fermer',
    exportRunningHint: 'Le terrain défile dans le cadre de la vidéo pendant le calcul : le rejeu revient à sa position à la fin.',
  ...
  en: {
    ...
    seatVacant: 'Open slot',
    seatVacantHint: 'Nobody holds this slot at this moment: it is waiting for its next occupant.',
    seatNotSpawned: 'Not spawned yet',
    ...
    exportFramingWhole: 'Whole map',
    exportFramingCurrentFmt: (zoom) => `Current view (${zoom}x)`,
    exportStart: 'Export',
    exportCancel: 'Cancel',
    exportClose: 'Close',
    exportRunningHint: 'The field scrolls inside the video frame while it computes: the replay returns to its position at the end.',
