...
const indexOfOp = (ops: CanvasOp[], op: string): number =>
  ops.findIndex((o) => o.op === op);
...
describe("étiquette de nom (D4)", () => {
  it("cerne le nom AVANT de le remplir, avec des jonctions rondes", () => {
    ...
    const joins = ops.filter(
      (o) => o.op === "set lineJoin" && o.args[0] === "round",
    );
  ...
  it("centre le texte sous le point et l aligne par son HAUT (il descend, jamais il ne remonte)", () => {
    const ops = trace();
    expect(
      ops.some((o) => o.op === "set textAlign" && o.args[0] === "center"),
    ).toBe(true);
    expect(
      ops.some((o) => o.op === "set textBaseline" && o.args[0] === "top"),
    ).toBe(true);
...
describe("élévation de visée (schéma 13)", () => {
  ...
  const inclinee = (p?: number) =>
    singlePointTrack(512, {
      points: [
        { t: 50, x: 5, y: 5, z: 0, h: 360, ...(p === undefined ? {} : { p }) },
      ],
    });
  ...
  it("prend l élévation du MÊME instant que le cap, jamais une plus ancienne", () => {
    ...
    const secteur = ops.find(
      (o) => o.op === "arc" && (o.args as number[]).length === 5,
    )!;
...
describe("traînée en option et joueur de la page (V1, 2026-08-18)", () => {
  ...
  it("joueur de la page : DEUX anneaux et un halo, à l encre dédiée", () => {
    ...
    const strokeStyles = mine
      ...
      .map((o) => o.args[0]);
    expect(
      strokeStyles.filter((s) => s === "rgb(7 7 7)").length,
  ...
  it("LA COULEUR N EST JAMAIS SEULE : le noyau du joueur de la page garde sa couleur d équipe", () => {
    ...
    expect(
      mine.filter((o) => o.op === "set fillStyle").map((o) => o.args[0]),
    ).toContain("rgb(1 2 3)");
  });
  ...
  it("un joueur ORDINAIRE n a ni double anneau ni halo", () => {
    ...
    expect(
      autre.filter((o) => o.op === "set strokeStyle").map((o) => o.args[0]),
