...
import { VEHICLE_SHOT_HELD_LOOPS, VEHICLE_SHOT_SOUND_VARIANTS } from './vehicleShotSound'
...
describe('garde-rail : manifeste sonore = dossier d assets', () => {
  const referenced = new Set([
    ...
    ...allEngineStems(),
    // Les BOUCLES DE TIR TENU (lot M6.1, 2026-09-24) : le corps de rafale d'une arme a tir
    // continu, declare des la livraison, cable par le lot M4b (cf. `vehicleShotSound.ts`).
    ...Object.values(VEHICLE_SHOT_HELD_LOOPS),
  ])
...
describe('garde-rail : tirs d armes de vehicule (lot du 2026-09-04)', () => {
  ...
  it('boucles de tir tenu : rattachees a un coup declare, jamais a la coupe des armes', () => {
    ...
    expect(Object.keys(VEHICLE_SHOT_HELD_LOOPS).length).toBeGreaterThan(0)
    for (const [coup, boucle] of Object.entries(VEHICLE_SHOT_HELD_LOOPS)) {
