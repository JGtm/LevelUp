 ...
 * `replayOverlayStyles.ts`, et le titre du verdict de fin ne doit pas se re-écrire À LA MAIN
 * ailleurs dans la feature.
 *
 * POURQUOI. Le message inter-manche a été aligné sur « le même style que le texte de défaite ou
 * victoire » (retour utilisateur du 2026-08-28). C'était la 3e copie du même littéral de titre
...
import { OVERLAY_TITLE } from './replayOverlayStyles'
...
describe('garde-rail : le titre de panneau d overlay est centralisé', () => {
  it('le littéral exact du titre ne vit QUE dans replayOverlayStyles.ts', () => {
    const coupables = sourceFiles().filter(
      (f) => f !== STYLES_FILE && readFileSync(resolve(FEATURE_DIR, f), 'utf8').includes(OVERLAY_TITLE),
    )
    expect(coupables).toEqual([])
  })

  it('la constante de titre est bien celle attendue (le style « victoire/défaite »)', () => {
    ...
    expect(OVERLAY_TITLE).toBe('text-2xl font-bold uppercase tracking-wide text-foreground')
  })
})
