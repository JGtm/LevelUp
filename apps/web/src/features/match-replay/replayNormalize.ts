...
> & {
  ...
  /**
   * LES RAMASSAGES ET LES CONSOMMATIONS D'ÉQUIPEMENT (schéma 25) : ce qui ARRIVE à un joueur,
  ...
  /**
   * LES ARMES AU SOL individuelles (schéma 26) : une entrée par objet qui a BOUGÉ, avec sa
  ...
  /**
   * LES PRISES ET LES LÂCHERS D'ARME (schéma 24) : qui, quand, quelle arme. Datés à la
   * milliseconde — c'est la source FINE qui affine `loadoutAt` entre deux images-clés. Vide =
   * artefact antérieur au schéma 24, ou film qui n'en porte aucun.
   */
...
export function normalizeReplayDocument(raw: ReplayDocument): ReplayDocumentReady {
  return {
    ...
    // LES RAMASSAGES ET LES CONSOMMATIONS d'équipement (schéma 25) : la source FINE de datation
    ...
    // LES PRISES ET LES LÂCHERS D'ARME (schéma 24) : même rapport à `loadouts` que ci-dessus —
    ...
    // LES ARMES AU SOL individuelles (schéma 26) : une entrée par objet qui a bougé, bornée par
