...
/** Ce que dit la phrase de lecteur de la carte « Appui » : qui prépare, et pour qui. */
export interface AppuiDominant {
  /** L'assistant qui a préparé le plus de frags de l'escouade. */
  joueur: string
  /** Sa part dans les appuis MESURÉS de l'escouade, unité 0..1. */
  part: number
  /** Celui de ses bénéficiaires qui en a le plus profité. */
  beneficiaire: string
}

/**
 * appuiDominant désigne l'assistant le plus prolifique et son principal bénéficiaire.
 *
 * LE DÉNOMINATEUR EST LE TOTAL SERVEUR (`total_assists`), comme `assistPartParCouple` : le
 * dériver des barres visibles le ferait mentir le jour où une paire serait filtrée.
 *
 * `null` quand rien n'est mesuré — la phrase n'est alors pas rendue plutôt que de désigner
 * quelqu'un au hasard sur un total nul.
 */
export function appuiDominant(block: SquadAssistPairs): AppuiDominant | null {
  const pairs = block.pairs ?? []
  if (pairs.length === 0 || block.total_assists <= 0) return null

  const totalParAssistant = new Map<string, number>()
  for (const p of pairs) {
    totalParAssistant.set(
      p.assist_gamertag,
      (totalParAssistant.get(p.assist_gamertag) ?? 0) + p.assist_count,
    )
  }
  let joueur = ''
  let meilleur = -1
  for (const [gt, n] of totalParAssistant) {
    if (n > meilleur) {
      meilleur = n
      joueur = gt
    }
  ...
  const siennes = pairs.filter((p) => p.assist_gamertag === joueur)
  const beneficiaire = siennes.reduce((a, b) => (b.assist_count > a.assist_count ? b : a))
  return { joueur, part: meilleur / block.total_assists, beneficiaire: beneficiaire.killer_gamertag }
}
