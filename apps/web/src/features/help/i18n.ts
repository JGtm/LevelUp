        ...
        entries: [
          {
            term: 'LUSR',
            definition:
              "Indice de compétence fondé sur TrueSkill 2 (Microsoft Research), adapté à Halo Infinite. C'est l'équivalent du CSR (rang classé officiel) pour les modes non classés : là où le CSR est attribué par Halo pour les listes de jeu compétitives, le LUSR est calculé localement par LevelUp sur tous les autres modes (arène, grande équipe, Firefight, parties personnalisées…), en appliquant la même logique TrueSkill.\n\nChaque joueur est représenté par deux nombres : sa moyenne estimée μ (talent supposé) et son incertitude σ (à quel point on est sûr de cette estimation). L'affichage public est μ − 3σ : on retire volontairement l'incertitude pour éviter de gonfler le rang d'un joueur dont on connaît mal le niveau.\n\nLe calcul est séparé par famille de listes de jeu (compétitif, arène, grande équipe, Firefight, parties personnalisées). Une famille = un état μ/σ indépendant, pour qu'une mauvaise série en social ne pollue pas ton niveau classé.\n\nÀ chaque match, LevelUp calcule un score composite [0 ; 1] qui mesure ta performance « objective », puis l'injecte dans la formule TrueSkill pour mettre à jour μ et σ. Le score composite agrège 8 mesures (voir le tableau dans la formule) avec deux particularités importantes :\n\n• Cinq de ces mesures (précision, efficacité de dégâts, exploit médailles, conversion offensive, résistance défensive) sont comparées à ta propre moyenne glissante sur les 50 derniers matchs — et non à une référence absolue. Le score s'ajuste donc à ton niveau actuel : progresser devient plus difficile à mesure que tu progresses.\n\n• Le niveau adverse est estimé via le ratio des « kills attendus » (mesure de niveau retournée par l'API Halo) entre les deux équipes, plafonné dans [0,5 × ; 2,0 ×] ton μ. Battre une équipe plus forte te fait monter davantage qu'écraser une équipe faible.\n\nRésultats pris en compte : Victoire (poids 1,0), Égalité (0,5), Défaite (0,0). Les abandons comptent faiblement (0,15) pour ne pas te pénaliser sur une déconnexion subie sans pour autant les ignorer totalement.\n\nDérive d'inactivité : au-delà d'une journée sans jouer dans la famille concernée, σ remonte d'1 point par jour (plafonné à 14 jours). Conséquence : le match suivant a plus d'impact sur μ (positif comme négatif), pour rattraper l'éventuelle évolution de ton vrai niveau pendant la pause.",
            formula:
          ...
          },
          {
            ...
            definition:
              "Mesure la quantité de dégâts que tu absorbes par mort. Repose sur le même barème : {{HP}} points de vie pour abattre un Spartan dans le titre courant (sur Halo Infinite : 90 de base + 135 de bouclier = 225).\n\nAu-dessus de 1,0 : tu meurs après avoir encaissé plus qu'une vie de Spartan (bonne résistance, tu obliges les adversaires à vider leur chargeur).\nEn dessous de 1,0 : tu meurs rapidement, souvent surpris ou mal positionné.",
            formula:
          ...
          },
          {
            ...
            definition:
              "Mesure ton dynamisme dans un match : écart entre ton engagement observé et ton engagement attendu, normalisé en percentile vs ton propre historique. 50 = comme d'habitude. >50 = plus engagé. <50 = moins engagé.\n\nL'attendu est ancré sur le lobby : c'est ta réponse HABITUELLE à un match d'intensité similaire (calme / standard / chaotique), pas ta part relative à ta seule équipe. Un joueur qui réagit mal aux matchs intenses aura donc un attendu bas dans un match intense — le trait structurel se lit sur la courbe « Joueur attendu » (basse vs l'équipe réelle), la forme du jour se lit sur l'écart réel − attendu.\n\nDistinct du Score de performance : le PerfScore mesure la qualité de tes actions, l'Engagement mesure leur quantité, leur régularité et ton état d'esprit. Les deux axes sont indépendants.\n\nCalcul intra-match : pour chaque instant t (échantillonné toutes les 10s, fenêtre glissante 90s), on compare ton pace d'événements pondérés (objectif ×1.5, assistance ×0.5, mort ×0 — chaque affrontement compte une fois, côté acteur) au pace du lobby par joueur multiplié par ton coefficient de réponse du bin d'intensité du match. La moyenne des écarts donne le résidu brut, normalisé en percentile vs tes 200 derniers matchs du même mode.\n\nNiveaux de confiance (qualité de l'HISTORIQUE du percentile) : insufficient_history (<10 matchs, score nil), partial (10-29 matchs), full (≥30 matchs). Base de l'attendu (signal distinct) : bin (coef du bin, ≥10 matchs dans le bin), global (coef lobby global), cold_start (aucun historique → attendu masqué). PvE non couvert v1.",
            formula:
            ...
            example:
              "Sur ce match, le lobby produit en moyenne 2.9 events/min/joueur (I = 2.9) → bin chaotique. Ton coef de réponse dans les matchs chaotiques est 0.93 (tu réagis un peu moins fort quand ça s'emballe). Ton pace attendu = 0.93 × 2.9 = 2.70 events/min. Si tu fais 2.43 events/min, tu es sous l'attendu de 0.27 → écart moyen négatif → vs ton historique, cela te place par exemple au P42 (sous ton habitude). Note : l'attendu (2.70) n'est plus collé à l'équipe réelle (2.60) — les deux courbes ont des définitions indépendantes.",
          },
          {
            term: "Coefficient de réponse (bins d'intensité + coef lobby)",
            definition:
              "Caractérise ta réponse habituelle sur la durée, ancrée sur le lobby. Médianes glissantes sur tes 200 derniers matchs par catégorie de mode (PvP_ranked, PvP_unranked).\n\nBins d'intensité : tes matchs sont classés en 3 terciles d'intensité (calme / standard / chaotique) selon le pace du lobby ; pour chacun, on garde la médiane de (ton pace / pace lobby par joueur). Un coef décroissant avec l'intensité = tu réagis moins bien aux matchs qui s'emballent ; croissant = tu montes en puissance dans le chaos.\n\nCoef lobby global : ta part historique de l'action totale du lobby, toutes intensités confondues. Sert de repli quand un bin n'a pas assez d'échantillons (<10). Comparable inter-joueurs.\n\nRemplace l'ancien coef_team_share (part intra-équipe), abandonné : l'attendu n'est plus une part relative à l'équipe mais la réponse habituelle du joueur au contexte total du match.",
            formula:
            ...
            example:
              "Sur tes 200 derniers matchs PvP non classés, tes 3 bins donnent coef 0.94 (calme) / 0.96 (standard) / 0.81 (chaotique) : tu tiens ton rythme dans les matchs calmes/standards mais tu décroches un peu dans les matchs chaotiques. Chaque bin porte ~66 matchs (terciles de 198 matchs valides). Dans un match chaotique, ton attendu utilisera 0.81 × pace_lobby ; dans un match calme, 0.94 × pace_lobby.",
          },
          {
      ...
      },
      {
        ...
        entries: [
          ...
          },
          {
            term: 'Survie',
            definition:
              'Résistance défensive normalisée sur les matchs partagés. Mesure la capacité à encaisser des dégâts avant de mourir. Repose sur le même barème que l\'Impact : {{HP}} points de vie pour abattre un Spartan dans le titre courant (225 sur Halo Infinite). Au-dessus de 1,65 (repère élite) : tu encaisses bien plus d\'une vie complète. En dessous de 1,0 : tu meurs tôt dans les engagements.',
      ...
      },
      {
        ...
        entries: [
          ...
          },
          {
            term: 'Domination',
            definition:
              "Victoire dans laquelle ton équipe a maintenu une avance importante et constante tout au long du match, sans jamais être réellement menacée. Badge de maîtrise totale.",
            formula:
              "Conditions : Victoire + avance maximale du joueur ≥ seuil d'avance\n(et aucun retournement ≥ seuil de retournement)",
            example:
              "Score final 50–28. Ton équipe a toujours mené d'au moins 20 éliminations → Domination. Si à un moment l'adversaire avait remonté à 5 éliminations d'écart, c'est trop peu pour déclencher un autre badge.",
          },
          {
            term: 'Humiliation',
            definition:
              "Défaite dans laquelle l'adversaire a maintenu une avance écrasante du début à la fin. Ton équipe n'a jamais vraiment été en position de renverser la tendance.",
            formula:
          ...
          },
          {
            term: 'Remontada',
            definition:
              "Victoire arrachée après avoir été nettement mené. Ton équipe était en position défavorable (retard ≥ seuil) à un moment du match avant de renverser la situation et de l'emporter.",
            formula:
              "Conditions : Victoire + l'adversaire avait une avance ≥ seuil de retournement avant la fin du match",
            example:
              "Score final 50–45. À mi-match ton équipe était en retard de 18 éliminations. Elle a renversé la tendance → Remontada.",
          },
          {
            term: 'Débandade',
            definition:
              "Défaite après avoir été en position de force. Ton équipe menait nettement à un moment du match avant de s'effondrer et de perdre.",
            formula:
              'Conditions : Défaite + ton équipe avait une avance ≥ seuil de retournement avant la fin du match',
            example:
              "Score final 45–50. À mi-match ton équipe menait de 18 éliminations. L'adversaire a rattrapé → Débandade.",
          },
          {
            term: 'Contre-Remontada',
            definition:
              "Le badge le plus rare : victoire après un double retournement. Ton équipe a d'abord mené, puis a été rattrapée et dépassée, avant de reprendre l'avantage et de gagner.\n\nLes deux équipes ont eu une avance significative à un moment du match.",
            formula:
              "Conditions : Victoire + ton équipe avait une avance ≥ seuil de retournement à un moment\n+ l'adversaire avait aussi une avance ≥ seuil de retournement à un autre moment\n(priorité maximale — détectée avant Remontada)",
            example:
              "Ton équipe menait de 20 éliminations en première moitié. L'adversaire est remonté et l'a dépassée de 18. Elle est repartie et a gagné 50–47 → Contre-Remontada.",
          },
        ],
      },
      {
        ...
        entries: [
          {
            ...
            definition:
              "Processus de récupération des matchs depuis l'interface Halo Waypoint et d'écriture dans la base DuckDB locale. En mode incrémental (par défaut), seuls les nouveaux matchs non encore enregistrés sont récupérés ; une synchronisation complète peut être forcée depuis les Paramètres.\n\nTrois déclencheurs coexistent :\n\n• Synchronisation manuelle — tu appuies sur le bouton « Synchroniser » dans les Paramètres. C'est la méthode de référence après une longue session ou pour rattraper un retard ponctuel.\n\n• Synchronisation planifiée — un ordonnanceur interne déclenche une synchronisation incrémentale à intervalle fixe (par exemple toutes les 30 minutes). Activable et configurable depuis Paramètres › Synchronisation. Utile pour garder la base à jour en arrière-plan tant que l'application tourne, indépendamment de ton activité de jeu.\n\n• Détection de présence — un veilleur écoute les notifications Xbox en temps réel via un canal RTA (authentifié par jeton XSTS) et déclenche automatiquement une synchronisation dès qu'il détecte la fin d'un de tes matchs. Réaction quasi immédiate, sans attendre le prochain tour de l'ordonnanceur.\n\nPrérequis côté Xbox : pour que les notifications RTA soient effectivement reçues, ton profil Xbox doit autoriser la lecture publique de ton statut de connexion et de ton activité de jeu en cours (paramètres « Vous voir en ligne » et « Vous voir jouer à un jeu » réglés sur « Tout le monde »). Si ces paramètres sont restreints (Amis uniquement, Bloqué), le canal RTA refuse l'abonnement et la détection de présence reste silencieuse — la synchronisation planifiée et la synchronisation manuelle continuent en revanche de fonctionner normalement.",
            example:
              "Tu lances Halo, tu joues 5 parties, puis tu fermes le jeu :\n• Détection de présence active → chaque fin de match déclenche une synchronisation, tes 5 matchs sont remontés au fil de l'eau.\n• Synchronisation planifiée seule (toutes les 30 min) → tes 5 matchs sont remontés en 1 ou 2 passes, avec un délai maximum d'environ 30 minutes.\n• Aucune des deux activée → tu ouvres LevelUp plus tard et tu appuies sur « Synchroniser » : les 5 matchs sont récupérés d'un coup, sans retoucher aux 500 déjà stockés.",
          },
          {
          ...
          },
          {
            ...
            definition:
              "Résolution d'un nom d'affichage unique et lisible en français à partir des variantes brutes renvoyées par l'interface Halo Waypoint. Une même playlist peut remonter sous plusieurs intitulés selon la version du jeu, la saison ou le canal d'où elle est lue.\n\nLevelUp consolide ces variantes en un seul nom français via une table de correspondance et applique le même traitement aux noms de cartes lorsque c'est nécessaire. Cela garantit qu'un match « Capture du drapeau » sur « Aquarius » apparaît toujours sous le même libellé dans tes statistiques, ton Explorer et ton Palmarès, peu importe sous quel intitulé brut Halo l'a renvoyée.",
            example:
        ...
        entries: [
          {
            ...
            example:
              "Tu joues 7 jours d'affilée → série « daily_play » à 7. Multiplicateur PP passé de ×1.0 à ×1.3. Tu sautes un jour sans utiliser un bouclier → la série casse et redémarre à 0.",
          },
          {
            term: 'Record personnel (Personal Best)',
            definition:
              "Meilleure valeur jamais atteinte sur une métrique donnée, dans une fenêtre temporelle (30 jours, 90 jours, ou tout-temps). Mis à jour automatiquement à chaque match : si tu bats ton record, l'ancien est archivé dans l'historique.",
            example:
              "Ton record « frags par match » sur 30 jours était 22. Tu fais 25 → nouveau record à 25, l'ancien (22) glisse dans l'historique avec la date.",
          },
          {
            term: 'Palier (Milestone)',
            definition:
              "Seuil d'accomplissement franchi automatiquement et conservé à vie (ex : 1 000 frags cumulés, 100 victoires, 50 médailles Killing Spree). Contrairement aux objectifs, tu n'as rien à activer — le palier s'auto-attribue dès que la condition est remplie.",
            example:
              "Tu atteins tes 10 000 frags au total → le palier « Vétéran 10K » est marqué obtenu, avec la date et le match déclencheur.",
          },
          {
          ...
          },
          {
            term: 'Levier LUSR',
            definition:
              "Composante LUSR identifiée comme ayant la plus grande marge de progression personnelle (écart entre ta moyenne actuelle et tes 20 % supérieurs ou la cible du palier suivant). Travailler ce levier maximise l'impact sur ton LUSR global.",
            example:
              "Ta composante « précision » est à 0.42 alors que tes 20 % supérieurs personnels atteignent 0.58 — c'est un levier fort (+38 % de marge). Le bouton « Démarrer une campagne » lance un objectif ciblé sur cet axe.",
          },
          {
          ...
          },
          {
            ...
            example:
              "Tu complètes un objectif Heroic (200 PP de base) alors que ta série quotidienne est à 14 → multiplicateur ×1.5 → 300 PP crédités.",
          },
          {
            term: 'Mode pilote',
            definition:
      ...
      },
      {
        ...
        entries: [
          ...
          },
          {
            term: 'Escouade',
            definition:
              "Composition de coéquipiers que tu analyses ensemble : les pages Escouade calculent des statistiques agrégées sur tes matchs joués en commun (synergies, contributions, carte de chaleur d'intensité). Tu peux sauvegarder, nommer et recharger plusieurs escouades.\n\nÀ ne pas confondre avec les Groupes : une escouade est une composition d'analyse, elle ne donne aucun accès aux données.",
            example:
              "Si trois de tes coéquipiers réguliers utilisent LevelUp, ta page Escouade affichera tes matchs joués avec eux et leurs statistiques sur cet échantillon commun.",
          },
          {
            term: 'Groupes',
            definition:
              "Cercles familles/amis qui définissent à quelles bases de données tu as accès pour explorer les stats. Inviter un membre (via sa connexion Xbox) ouvre l'accès mutuel à tes données.\n\nÀ ne pas confondre avec l'Escouade : un groupe gère l'ACCÈS aux données ; l'escouade est une composition d'analyse de coéquipiers.",
            example:
              "Crée un groupe « Famille » et invite un proche : il pourra explorer tes stats et toi les siennes. Pour analyser tes parties jouées ensemble, sélectionne-le ensuite comme coéquipier sur la page Escouade.",
          },
          {
            term: 'Explorer',
            definition:
              "Vue d'exploration détaillée de tous tes matchs avec filtres en cascade : carte, mode, liste de jeu, résultat, date, session. Permet une analyse fine et la navigation vers la vue détaillée de chaque match.",
            example:
              "Filtrer par carte « Streets », mode « Capture du drapeau », résultat « Victoire » sur les 30 derniers jours pour isoler tes matchs gagnés récemment dans cette configuration.",
          },
          {
            ...
            example:
              "Si les barres de performance rouge/vert se ressemblent sur ton écran, active la palette Okabe-Ito : les mêmes niveaux de performance sont alors encodés en bleu ciel, bleu-vert, jaune, orange et vermillon — distincts pour tous les types de vision.",
          },
          {
            term: 'Palmarès',
            definition:
              "Section regroupant tes classements (tableau local par liste de jeu et par saison), tes relations (alliés fréquents, ennemis récurrents, victimes habituelles), la comparaison face-à-face avec un autre joueur, et ton passe saisonnier Halo.",
            example:
              "Le sous-onglet Relations met en évidence tes cinq alliés les plus fréquents et leur taux de victoire commun, ainsi que tes cinq ennemis récurrents et le solde des affrontements.",
          },
        ],
      },
    ],
