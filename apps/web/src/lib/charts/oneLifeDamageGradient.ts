 ...
 */
export function offensiveDamageGradient(values: Array<number | null>): LinearGradient {
  const pos = resolveToken('divergent-pos')
  const neg = resolveToken('divergent-neg')
  const r = offsetOf(values, ONE_LIFE_DAMAGE)
 ...
 */
export function defensiveDamageGradient(values: Array<number | null>): LinearGradient {
  const pos = resolveToken('divergent-pos')
...
export function damageAxisBounds(values: Array<number | null>): { min: number; max: number } {
  const nums = values.filter((v): v is number => v != null)
  if (nums.length === 0) return { min: 0, max: ONE_LIFE_DAMAGE * 2 }
  const lo = Math.min(...nums, ONE_LIFE_DAMAGE)
  const hi = Math.max(...nums, ONE_LIFE_DAMAGE)
