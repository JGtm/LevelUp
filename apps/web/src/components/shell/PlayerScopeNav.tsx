...
function PlayerNavLink({ item, playerSlug, tone }: PlayerNavLinkProps) {
  const base =
    tone === 'primary'
      ? 'group flex min-w-[15rem] shrink-0 flex-col rounded-2xl border border-border bg-muted/90 px-4 py-3 text-left transition hover:border-border hover:bg-primary/10 [&.active]:bg-primary [&.active]:text-primary-foreground'
      : 'group inline-flex items-start gap-3 rounded-full border border-border bg-background px-4 py-2 text-left text-sm font-medium text-foreground transition hover:border-border hover:bg-primary/10 hover:text-primary [&.active]:bg-primary [&.active]:text-primary-foreground'
    ...
    <Link to={item.to} params={{ playerSlug }} className={base}>
      {tone === 'primary' ? (
        <>
          ...
          <span className="mt-1 text-sm leading-5 text-muted-foreground transition group-[&.active]:text-primary-foreground/72">
            {item.description}
          </span>
        </>
      ) : (
